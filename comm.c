...
#include "ban.h"
#include "fight.h"
#include "psionics.h" /* for affect_update */
#include "modify.h"
#include "quest.h"
  ...
  while (!circle_shutdown) {
    ...
    for (d = descriptor_list; d; d = next_d) {
      ...
      if (d->character) {
		...
		GET_DELAY_HIT_WAIT(d->character) -= (GET_DELAY_HIT_WAIT(d->character) > 0);

		if (GET_DELAY_HIT_WAIT(d->character) == 0 && GET_DELAY_HIT(d->character) != NULL) {
			GET_DELAY_HIT_WAIT(d->character) = -1;
			exec_delayed(d->character);
		}
...
void heartbeat(int heart_pulse)
{
  ...
  if (!(heart_pulse % PULSE_IDLEPWD))		/* 15 seconds */
        room_update_turret(DO_BURN);

  if (!(heart_pulse % PULSE_MOBILE))
    mobile_activity();

    if (!(heart_pulse % PULSE_ROOM))
        room_update(DO_NOT_BURN);
    if (!(heart_pulse % (30 * PASSES_PER_SEC)))
        room_update(DO_BURN);
    if (!(heart_pulse % PULSE_EXPLOSIVE))
        explosive_update();
    if (!(heart_pulse % PULSE_REGEN)) /* 4 seconds */
        hpv_update();
	if (!(heart_pulse % PULSE_ENCUMBRANCE)) /* 4 seconds */
		encumb_update();

    if ((heart_pulse % (2 * (3600 * PASSES_PER_SEC))) == 0) /* 2 hours */
        saber_part_load(1);
    if ((heart_pulse % (3600 * PASSES_PER_SEC)) == 0) { /* 1 hour */
		update_glist();
		world_event_loader(FALSE);
        rainbow_token_load(0);
	}
	if (!(heart_pulse % PULSE_FIGHT))// New fight system TAG: NEWFIGHT
		execute_combat();
	if (!(heart_pulse % PULSE_SECOND)) {
		events_per_second();
		execute_special_psionics();
		execute_special_psionics_caller();
	}
	...
	if (!(heart_pulse % (SECS_PER_MUD_HOUR * PASSES_PER_SEC))) {  /* Tick ! */
		...
		if (EVENT_LIVE)
			update_world_event();
		// halloween event!
//		reset_zone(real_zone(41), FALSE);
//		reset_zone(real_zone(216), FALSE);
//		reset_zone(real_zone(11), FALSE);
	}
...
char *prompt_str(struct char_data *ch)
{
    ...
    // lets try putting colors here instead of up there.. not to confuse them!

    // if there is nothing in the prompt to parse, convert colors and return
    if (!strchr(str, '%')) {
        proc_colors(str, sizeof(str), COLOR_ON(ch));
        return str;
    }
    ...
    for (;;) {
        if (*str == '%') {
            ...
            switch (*(++str)) {
                ...
                case 'p': // current psi
                    sprintf(i, "%d", GET_PSI(ch));
                    tmp = i;
                    break;

                case 'P': // maximum psi
                    sprintf(i, "%d", GET_MAX_PSI(ch));
                    tmp = i;
                    break;
                ...
                case 'q': // current qp
                    sprintf(i, "%d", GET_QUESTPOINTS(ch));
                    tmp = i;
                    break;
                    ...
                    if (ch->equipment[WEAR_WIELD]) {
                        if (IS_GUN(ch->equipment[WEAR_WIELD])) {
                            if (IS_LOADED(ch->equipment[WEAR_WIELD]))
                                sprintf(i, "%d", GET_LOADED_AMMO_COUNT(ch->equipment[WEAR_WIELD]));
                            else
                                sprintf(i, "%s", "EMPTY");
                        }
                    }
                ...
                case 'i': // invisibility level - immort only
                     if (GET_LEVEL(ch) >= LVL_IMMORT)
                        sprintf(i, "%d", GET_INVIS_LEV(ch));
                    else
                        sprintf(i, " ");
                    tmp = i;
                    break;

                case 'r': // current room number - immort only
                     if (GET_LEVEL(ch) >= LVL_IMMORT)
                        sprintf(i, "%d", GET_ROOM_VNUM(IN_ROOM(ch)));
                    else
                        sprintf(i, " ");
                    tmp = i;
                    break;

                case 'Q': // percentage of hitp/move/psi
                    str++;
                    switch (LOWER(*str)) {
                        case 'h':
                            ...
                            break;
                        case 'p':
                            perc = (100 * GET_PSI(ch)) / GET_MAX_PSI(ch);
                            break;
                        case 'v':
                ...
                case 'o': // opponent
// TAG: NEWSERVER                   if (vict && GET_POS(vict) < POS_MORTALLYW)
//						sprintf(i, "");
					if (vict && GET_POS(vict) >= POS_MORTALLYW) {
                        perc = (100*GET_HIT(vict)) / GET_MAX_HIT(vict);
                        sprintf(i, "%s @c(%s@c)@w", PERS(vict, ch),
                            (perc >= 90 ? "\x1B[0;32mExcellent condition\x1B[0;0m" :
                             perc >= 75 ? "\x1B[0;32mA few scratches\x1B[0;0m" :
                             perc >= 50 ? "\x1B[0;32mSmall wounds and bruises\x1B[0;0m" :
                             perc >= 30 ? "\x1B[0;0mQuite a few wounds\x1B[0;0m" :
                             perc >= 15 ? "\x1B[1;31mPretty hurt\x1B[0;0m" :
                             perc >= 0 ? "\x1B[1;31mAwful condition\x1B[0;0m" :
                             "\x1B[1;31mBleeding awfully from big wounds\x1B[0;0m")
                         );
                         tmp = i;
                     }
                     else {
                ...
                case 'X':
                    sprintf(i, "%d", (exptolevel(ch, GET_LEVEL(ch) +1) - GET_EXP(ch)));
                    tmp = i;
                    break;
				
				case 'Z':
					if (GET_LEVEL(ch) < LVL_IMMORT) {
						xp2lvl = (100 * (GET_EXP(ch))) / (exptolevel(ch, GET_LEVEL(ch) +1));
						sprintf(i, "%ld", xp2lvl);
					}
					else
						sprintf(i, "0");
                ...
                case 'u': // units on hand
                    sprintf(i, "%d", GET_UNITS(ch));
                    tmp = i;
                    break;

                case 'U': // units in bank
                    sprintf(i, "%d", GET_BANK_UNITS(ch));
                    tmp = i;
                    break;

                case 'T':
                    if (vict && (tank = FIGHTING(vict)) && tank != ch) {
                        sprintf(i, "%s", GET_NAME(tank));
                        tmp = i;
                    }
                    else {
                        str++;
    ...
    strcat(pbuf, "@n");
    proc_colors(pbuf, sizeof(pbuf), COLOR_ON(ch));
    return pbuf;
}
char *make_prompt(struct descriptor_data *d)
{
    ...
    if (d->showstr_count) {
        ...
        strcpy(prompt, "] ");
    else if (STATE(d) == CON_PLAYING && !IS_NPC(d->character) && GET_PROMPT(d->character) != NULL) {

        if (PRF_FLAGGED(d->character, PRF_BUILDWALK))
            sprintf(prompt, "%sBuildWalking]%s %s", CCRED(d->character, C_NRM), CCNRM(d->character, C_NRM), prompt_str(d->character));
        else if (PRF_FLAGGED(d->character, PRF_AFK))
            sprintf(prompt, "%s[AFK]%s %s", CCRED(d->character, C_NRM), CCNRM(d->character, C_NRM), prompt_str(d->character));
		else if (!PRF_FLAGGED(d->character, PRF_BLIND) && IN_VEHICLE(d->character))
			sprintf(prompt, "%s[%sHP: %s%d%s][%sFuel: %s%d%s][%sAC: %s%d%s]%s ",BWHT,KGRN,BGRN, (GET_OBJ_VAL(VEHICLE(d->character), 2) - VEHICLE(d->character)->damage),BWHT,KRED,BRED, VEHICLE(d->character)->fuel,BWHT,KCYN,BCYN, GET_OBJ_VAL(VEHICLE(d->character), 4),BWHT,CNRM);
        else
            sprintf(prompt, "%s", prompt_str(d->character));


    }
    else if (STATE(d) == CON_PLAYING && !IS_NPC(d->character) && GET_PROMPT(d->character) == NULL && !IN_VEHICLE(d->character)) {
        ...
        *prompt = '\0';
        ...
        if (GET_LEVEL(d->character) > 30) {
            count = snprintf(prompt + len, sizeof(prompt) - len, "[%d] ", world[IN_ROOM(d->character)].number);
            if (count >= 0)
                len += count;
        }
        ...
        if (PRF_FLAGGED(d->character, PRF_DISPHP) && len < sizeof(prompt)) {
            ...
            // only when below 25%
            if (GET_HIT(d->character) << 2 < GET_MAX_HIT(d->character) ) {
                count = snprintf(prompt + len, sizeof(prompt) - len, "%s%dH%s ",
                    CCRED(d->character, C_NRM), GET_HIT(d->character), CCNRM(d->character, C_NRM));
                if (count >= 0)
                    len += count;
            }
        }

        if (PRF_FLAGGED(d->character, PRF_DISPPSI) && len < sizeof(prompt)) {
            count = snprintf(prompt + len, sizeof(prompt) - len, "%dM ", GET_PSI(d->character));
            if (count >= 0)
                len += count;
        }
        else if (PRF_FLAGGED(d->character, PRF_DISPAUTO) && len < sizeof(prompt)) {

            if (GET_PSI(d->character) << 2 < GET_MAX_PSI(d->character) && len < sizeof(prompt)) {
                count = snprintf(prompt + len, sizeof(prompt) - len, "%s%dM%s ",
                    CCRED(d->character, C_NRM), GET_PSI(d->character), CCNRM(d->character, C_NRM));
                if (count >= 0)
                    len += count;
            }
        }
        ...
        if (PRF_FLAGGED(d->character, PRF_DISPMOVE) && len < sizeof(prompt)) {
            ...
            if (GET_MOVE(d->character) << 2 < GET_MAX_MOVE(d->character) && len < sizeof(prompt)) {
                count = snprintf(prompt + len, sizeof(prompt) - len, "%s%dV%s ",
                    CCRED(d->character, C_NRM), GET_MOVE(d->character), CCNRM(d->character, C_NRM));
                if (count >= 0)
                    len += count;
            }
        }
        ...
        if (PRF_FLAGGED(d->character, PRF_DISPAMMO) && len < sizeof(prompt)) {
            if (d->character->equipment[WEAR_WIELD]) {
                if (IS_GUN(d->character->equipment[WEAR_WIELD])) {
                    if (IS_LOADED(d->character->equipment[WEAR_WIELD])) {
                        count = snprintf(prompt + len, sizeof(prompt) - len, "[%dA] ", GET_LOADED_AMMO_COUNT(d->character->equipment[WEAR_WIELD]));
                    }
                    else {
                        count = snprintf(prompt + len, sizeof(prompt) - len, "[EMPTY] ");
                    }
...
static int new_descriptor(socket_t s)
{
  ...
  setsockopt(desc, SOL_SOCKET, SO_KEEPALIVE, val, sizeof(int));
...
static ssize_t perform_socket_read(socket_t desc, char *read_point, size_t space_left)
{
  ...
  if (ret < 0) {
	  log("WARNING: Value of socket read was negative, dumping the socket. TAG: GAHAN");
	  return (-1);
  }
...
void perform_act(const char *orig, struct char_data *ch, struct obj_data *obj, void *vict_obj, struct char_data *to, int message)
{
  const char *i = NULL;
  ...
  *(++buf) = '\0';
	...
	if (to->desc && message == 0) {
		if (to->desc)
		    write_to_output(to->desc, "%s", CAP(lbuf));
	}
	else if (to->desc && message == 1 && !PRF_FLAGGED(to->desc->character, PRF_BLIND)) {
	    if (to->desc)
		    write_to_output(to->desc, "%s", CAP(lbuf));
	}
	else if (to->desc && message == 2 && !PRF_FLAGGED(to->desc->character, PRF_SPAM)) {
	    if (to->desc)
		    write_to_output(to->desc, "%s", CAP(lbuf));
	}
	else if (to->desc && message == 3 && !PRF_FLAGGED(to->desc->character, PRF_NOFIGHTSPAM)) {
	    if (to->desc)
		    write_to_output(to->desc, "%s", CAP(lbuf));
	}
...
char *act(const char *str, int hide_invisible, struct char_data *ch, struct obj_data *obj, void *vict_obj, int type, ...)
{
  ...
  int message = 0;
  va_list exargs;
  va_start(exargs,type);
  iptr=va_arg(exargs,int*);
  if(iptr!=NULL)
    message = (int)iptr;
  ...
  if (type == TO_CHAR) {
    if (ch && SENDOK(ch)) {
      perform_act(str, ch, obj, vict_obj, ch, message);
  ...
  if (type == TO_VICT) {
    if ((to = vict_obj) != NULL && SENDOK(to)) {
      perform_act(str, ch, obj, vict_obj, to, message);
      return last_act_message;
    }
    return NULL;
  }
  ...
  if (type == TO_GMOTE) {
    ...
    for (i = descriptor_list; i; i = i->next) {
      if (!i->connected && i->character &&
        ...
        perform_act(buf, ch, obj, vict_obj, i->character, message);
      }
    }
    return last_act_message;
  }
...
static void msdp_update( void )
{
  ...
  for (d = descriptor_list; d; d = d->next)
  {
    ...
    if ( ch && !IS_NPC(ch) && d->connected == CON_PLAYING )
    {
      ...
      MSDPSetNumber( d, eMSDP_EXPERIENCE_TNL,  exptolevel(ch, GET_LEVEL(ch) +1) - GET_EXP(ch));
      ...
      MSDPSetString( d, eMSDP_CLASS, pc_class[GET_CLASS(ch)].class_name );

      MSDPSetNumber( d, eMSDP_PSI, GET_PSI(ch) );
      MSDPSetNumber( d, eMSDP_PSI_MAX, GET_MAX_PSI(ch) );
      MSDPSetNumber( d, eMSDP_WIMPY, GET_WIMP_LEV(ch) );
      MSDPSetNumber( d, eMSDP_MONEY, GET_UNITS(ch) );
/* New Variables 10/14/13 */
      MSDPSetString( d, eMSDP_ROOM_NAME, world[IN_ROOM(ch)].name);
      MSDPSetString( d, eMSDP_ROOM_EXITS, GET_EXITS(ch));
      MSDPSetString( d, eMSDP_AREA_NAME, zone_table[(world[IN_ROOM(ch)].zone)].name);
      MSDPSetNumber( d, eMSDP_QUESTPOINTS, GET_QUESTPOINTS(ch) );
      MSDPSetNumber( d, eMSDP_PROGRESS, GET_PROGRESS(ch) );
      MSDPSetNumber( d, eMSDP_TIER, GET_TIER(ch) );
      MSDPSetNumber( d, eMSDP_FAME, GET_FAME(ch) );
      MSDPSetNumber( d, eMSDP_PROGRESSPCNT, GET_PROGRESSPCNT(ch) );
      MSDPSetNumber( d, eMSDP_CAN_CARRY_W, CAN_CARRY_W(ch) );
      MSDPSetNumber( d, eMSDP_IS_CARRYING_W, IS_CARRYING_W(ch) );
      MSDPSetNumber( d, eMSDP_IS_WEARING_W, IS_WEARING_W(ch) );
      MSDPSetNumber( d, eMSDP_ENCUMBRANCE, ENCUMBRANCE(ch) );
      MSDPSetNumber( d, eMSDP_GREEN_SHARDS, GET_GREEN_SHARDS(ch) );
      MSDPSetNumber( d, eMSDP_BLUE_SHARDS, GET_BLUE_SHARDS(ch) );
      MSDPSetNumber( d, eMSDP_BLACK_SHARDS, GET_BLACK_SHARDS(ch) );
/* end new variables - bosstone */
	  ...
	  MSDPSetNumber( d, eMSDP_PCN, GET_PCN(ch));
	  MSDPSetNumber( d, eMSDP_CHA, GET_CHA(ch));
	  MSDPSetNumber( d, eMSDP_STR_PERM, GET_REAL_STR(ch));
	  MSDPSetNumber( d, eMSDP_DEX_PERM, GET_REAL_DEX(ch));
	  MSDPSetNumber( d, eMSDP_CON_PERM, GET_REAL_CON(ch));
	  MSDPSetNumber( d, eMSDP_INT_PERM, GET_REAL_INT(ch));
	  MSDPSetNumber( d, eMSDP_PCN_PERM, GET_REAL_PCN(ch));
	  MSDPSetNumber( d, eMSDP_CHA_PERM, GET_REAL_CHA(ch));
  ...
  if (ch->equipment[WEAR_WIELD]) {
    if (IS_GUN(ch->equipment[WEAR_WIELD])) {
      if (IS_LOADED(ch->equipment[WEAR_WIELD]))
          MSDPSetNumber( d, eMSDP_AMMO,  GET_LOADED_AMMO_COUNT(ch->equipment[WEAR_WIELD]) );
      else
          MSDPSetNumber( d, eMSDP_AMMO, 0 );
    }
  }
...
/* do_who in an HTML file, for the website, originally by Erik Niese-Petersen <Tenp@cris.com> 1996
   and editted by BuckFloyd@aol.com then implemented by Lump eventually in 2006 */
...
void make_who2html(void)
{
    ...
    if ((opf = fopen("/home/cyber/public_html/mud.tmp", "w")) == 0) {
        perror("SYSERR: Error opening mud.tmp file");
        return;
    }

    fprintf(opf, "<HTML><HEAD><TITLE>Who is on the Mud?</TITLE><META HTTP-EQUIV=Refresh CONTENT=30 URL=mudwho.html></HEAD>\n");
    fprintf(opf, "<style>\n");
    fprintf(opf, "<!--\n");
    fprintf(opf, "BODY{\n");
    fprintf(opf, "scrollbar-face-color:#330000;\n");
    fprintf(opf, "scrollbar-arrow-color:#330000;\n");
    fprintf(opf, "scrollbar-track-color:#330000;\n");
    fprintf(opf, "scrollbar-shadow-color:'';\n");
    fprintf(opf, "scrollbar-highlight-color:'';\n");
    fprintf(opf, "scrollbar-3dlight-color:'';\n");
    fprintf(opf, "scrollbar-darkshadow-Color:'';\n");
    fprintf(opf, "}\n");
    fprintf(opf, "-->\n");
    fprintf(opf, "</style>\n");

    fprintf(opf, "<body background=tile.jpg bgcolor=#000000 TEXT=#ffff80 LINK=#00ff40 VLINK=#80ff00 ALINK=#00ff40>\n");
    fprintf(opf, "Players Online Right Now<HR>\n");
    ...
    for (d = descriptor_list; d; d = d->next) {
        if (!d->connected) {
            ...
            if (GET_LEVEL(ch) < LVL_IMMORT || (GET_LEVEL(ch)>=LVL_IMMORT && !GET_INVIS_LEV(ch) && !PRF_FLAGGED(ch, PRF_NOWHO))) {
                sprintf(buf, "<font color=%s>[%2d %s] %s <BR>\n",
                    GET_LEVEL(ch) < LVL_IMMORT ? "white" : "yellow",
                    GET_LEVEL(ch), CLASS_ABBR(ch),
                    GET_NAME(ch));
                // , html_color(GET_TITLE(ch))); this was cool, but caused problems somewhere
                fprintf(opf, "%s", buf);
                num_players++;
            }
        }
    ...
    if (num_players == 0)
        fprintf(opf,"<font color=white>Apparently, nobody is logged on at the moment...<br>");
    else {
        sprintf(buf, "<font color=white>%d characters displayed.<br>\n", num_players);
        fprintf(opf, "%s", buf);
    }


    result = rename("/home/cyber/public_html/mud.tmp", "/home/cyber/public_html/mudwho.html");
    if (result != 0)
        mudlog(NRM, LVL_GOD, TRUE, "ERROR: Unable to rename mud.tmp to mudwho.html");
}

//execution for delayed psionics/hits.  Use exec_delay_hit to setup a delayed shot.
void exec_delayed(struct char_data *ch)
{

	struct delay_hit *d_hit = GET_DELAY_HIT(ch);
	struct char_data *victim;
	
	if (AFF_FLAGGED(ch, AFF_PRIMING))
		REMOVE_BIT_AR(AFF_FLAGS(ch), AFF_PRIMING);

	if (AFF_FLAGGED(ch, AFF_PARALYZE))
		affect_from_char(ch, PSIONIC_PARALYZE, TRUE);
	...
	if (!(victim = get_char_vis(ch, d_hit->victim, NULL, FIND_CHAR_ROOM))) 
	{
        if (FIGHTING(ch))
		{
			victim = ch->char_specials.fighting;
			send_to_char(ch, "You quickly retarget %s!\n\r", GET_NAME(victim));
		}
        else 
		{
            send_to_char(ch, "Your target is no longer visible!\n\r");
            return;
        }
    }
	
	if(d_hit->hit_msg.room_msg != NULL)
	{
		act(d_hit->hit_msg.room_msg, FALSE, ch, 0, victim, TO_NOTVICT);
	}
		
	if (d_hit->hit_msg.attacker_msg != NULL)
	{
		act(d_hit->hit_msg.attacker_msg, FALSE, ch, 0, victim, TO_CHAR);
	}
		
	if (d_hit->hit_msg.victim_msg != NULL)
	{
		act(d_hit->hit_msg.victim_msg, FALSE, ch, 0, victim, TO_VICT);
	}
	
	if(d_hit->echo_msg.room_msg != NULL)
	{
		act(d_hit->echo_msg.room_msg, FALSE, ch, 0, victim, TO_GNOTVICT);
	}
		
	if(d_hit->echo_msg.attacker_msg != NULL)
	{
		act(d_hit->echo_msg.attacker_msg, FALSE, ch, 0, victim, TO_CHAR);
	}
		
	if(d_hit->echo_msg.victim_msg != NULL)
	{
		act(d_hit->echo_msg.victim_msg, FALSE, ch, 0, victim, TO_VICT);
	}
	...
	if(d_hit->dam > -1)
	{
		if (victim->char_specials.saved.duplicates > 0)
		{
			...
			act("$n attacks thin air.", FALSE, ch, 0, victim, TO_NOTVICT);
			act("You destroy a hallucination of $N!", FALSE, ch, 0, victim, TO_CHAR);
			act("$n destroys a hallucination of you!", FALSE, ch, 0, victim, TO_VICT);
			
			if ((victim->char_specials.saved.duplicates == 0) && 
			(affected_by_psionic(victim, PSIONIC_DUPLICATES)))
			{
		...
		else
		{
			...
			damage(ch, victim, d_hit->dam, ((d_hit->dam_type >= TYPE_MIN && d_hit->dam_type <= TYPE_MAX) ? d_hit->dam_type : TYPE_MIN), DMG_NORMAL);
	...
	if ((victim) && GET_POS(victim) > POS_STUNNED)
	{
		if(d_hit->psi_num > 0)
		{
			call_psionic(ch, victim, NULL, d_hit->psi_num, d_hit->psi_level, 0, d_hit->psi_ack);
		}
		
		if(d_hit->af_msg.room_msg != NULL)
		{
			act(d_hit->af_msg.room_msg, FALSE, ch, 0, victim, TO_NOTVICT);
		}
		
		if(d_hit->af_msg.attacker_msg != NULL)
		{
			act(d_hit->af_msg.attacker_msg, FALSE, ch, 0, victim, TO_CHAR);
		}
		
		if(d_hit->af_msg.victim_msg != NULL)
		{
			act(d_hit->af_msg.victim_msg, FALSE, ch, 0, victim, TO_VICT);
		}
	}
