...
void restring_obj(struct char_data* ch, char* name, int field, char* string);
void restring_mob(struct char_data* ch, char* name, int field, char* string);
...
ACMD(do_skillset)
{
    ...
    char helpbuf[MAX_STRING_LENGTH];
    int skill;
    int value;
    int i;
    int qend;
    ...
    if (!*name) {
        ...
        for (qend = 0, i = 1; i < TOP_SKILL_DEFINE; i++) {
            if (*skills_info[i].name == '!')
                continue;

            send_to_char(ch, "%20s", skills_info[i].name);
    ...
    // Locate the last quote and lowercase the psionic words (if any)
    ...
    // find_skill_num() guarantees a valid psionics_info[] index, or -1, and we
    // checked for the -1 above so we are safe here
    SET_SKILL(vict, skill, value);
    mudlog(BRF, LVL_IMMORT, TRUE, "%s changed %s's %s to %d.", GET_NAME(ch), GET_NAME(vict), skills_info[skill].name, value);
    send_to_char(ch, "You change %s's %s to %d.\r\n", GET_NAME(vict), skills_info[skill].name, value);
}
ACMD(do_psionicset)
{
    struct char_data *vict;
    ...
    if (!*name) {
        ...
        send_to_char(ch, "Syntax: psionicset <name> '<psionic>' <value>\r\n"
        "Psionic being one of the following:\r\n");

        for (qend = 0, i = 1; i < TOP_PSIONIC_DEFINE; i++) {

            if (*psionics_info[i].name == '!')
                continue;

            send_to_char(ch, "%20s", psionics_info[i].name);
    ...
    // If there is no chars in argument
    if (!*argument) {
        send_to_char(ch, "Psionic name expected.\r\n");
        return;
    }

    if (*argument != '\'') {
        send_to_char(ch, "Psionic must be enclosed in: ''\r\n");
        return;
    }
    ...
    if ((psionic = find_psionic_num(help)) <= 0) {
        send_to_char(ch, "Unrecognized psionic.\r\n");
        return;
    }
    ...
    if (IS_NPC(vict)) {
        send_to_char(ch, "You can't set NPC psionics.\r\n");
        return;
    }

    // find_psionic_num() guarantees a valid psionics_info[] index, or -1, and we
    // checked for the -1 above so we are safe here
    SET_PSIONIC(vict, psionic, value);
    mudlog(BRF, LVL_IMMORT, TRUE, "%s changed %s's %s to %d.", GET_NAME(ch), GET_NAME(vict), psionics_info[psionic].name, value);
    send_to_char(ch, "You change %s's %s to %d.\r\n", GET_NAME(vict), psionics_info[psionic].name, value);
}
...
void saber_part_load(int announce)
{
    ...
    switch(i) {
        ...
        // Rest are default
        case 56: r_num = real_object(1589); break;
        case 55: r_num = real_object(1589); break; // coupler 2
        case 54: r_num = real_object(1295); break;
        case 53: r_num = real_object(1295); break;
        case 52: r_num = real_object(1295); break;
        case 51: r_num = real_object(1295); break;
        case 50: r_num = real_object(1295); break; // diss    5
        case 44: r_num = real_object(1294); break;
        case 43: r_num = real_object(1294); break;
        case 42: r_num = real_object(1294); break;
        case 41: r_num = real_object(1294); break;
        case 40: r_num = real_object(1294); break; // lens    5
        case 33: r_num = real_object(1293); break;
        case 32: r_num = real_object(1293); break; // gem     2
        case 23: r_num = real_object(1292); break;
        case 22: r_num = real_object(1292); break;
        case 21: r_num = real_object(1292); break;
        case 20: r_num = real_object(1292); break; // mech    4
        case 13: r_num = real_object(1291); break;
        case 12: r_num = real_object(1291); break;
        case 11: r_num = real_object(1291); break;
        case 10: r_num = real_object(1291); break; // cell    4
        default: r_num = real_object(1290); break; // hilt  100
    ...
    } while (ROOM_FLAGGED(to_room, ROOM_PRIVATE | ROOM_GODROOM | ROOM_DEATH | ROOM_HOUSE | ROOM_HOUSE_CRASH | ROOM_ATRIUM | ROOM_NO_TP) ||
            ZONE_FLAGGED(world[to_room].zone, ZONE_REMORT_ONLY | ZONE_NO_TP | ZONE_QUEST));

    obj_to_room(obj, to_room);

    if (announce)
        send_to_all("@WYou feel a great disturbance in the @DF@Worce.@n\r\n");

    mudlog(CMP, LVL_GOD, TRUE, "SABER: %s loaded in %s (%d)", obj->short_description, world[to_room].name, world[to_room].number);
}
...
void rainbow_token_load(int announce)
{
    ...
    v_num = rand_number(1,10);

    obj = read_object(v_num, VIRTUAL);

//    Don't load to room anymore, randomly load to mob and make token !LOCATE
//    do {
//        to_room = rand_number(0, top_of_world);
//    } while (ROOM_FLAGGED(to_room, ROOM_PRIVATE | ROOM_GODROOM | ROOM_DEATH |
//                                    ROOM_HOUSE | ROOM_HOUSE_CRASH |
//                                    ROOM_ATRIUM | ROOM_NO_TP) ||
//    ZONE_FLAGGED(world[to_room].zone, ZONE_REMORT_ONLY | ZONE_NO_TP | ZONE_QUEST));
//
//    obj_to_room(obj, to_room);
    ...
    // if we don't have any mobs, don't go any further
    if (j == 0) {
        log("Rainbow_token_load: No mobs found to load rainbow tokens.");
        return;
    }
    ...
    // don't give tokens to shop keepers
    if (mob_index[GET_MOB_RNUM(mob)].func == shop_keeper)
        continue;

    if (GET_LEVEL(mob) >= LVL_IMPL)
        continue;

    // Give tokens to mobs above level 8 only (for now?)
    if (GET_LEVEL(mob) <= 8)
        continue;

    // mob is in restricted room, try again
    if (ROOM_FLAGGED(IN_ROOM(mob), ROOM_PRIVATE | ROOM_GODROOM | ROOM_DEATH |
        ROOM_HOUSE | ROOM_HOUSE_CRASH | ROOM_ATRIUM | ROOM_NO_TP | ROOM_PEACEFUL))
        continue;

    // our random mob is in a NO_MARBLE zone! blah, let's try again
    // Also: REMORT_ONLY, QUEST */
    if (ZONE_FLAGGED(world[IN_ROOM(mob)].zone, ZONE_REMORT_ONLY | ZONE_NO_MARBLE | ZONE_QUEST))
        continue;

    // check inventory, if they already have a token then find a new mob
    inv = mob->carrying;
    found = FALSE;
    while (inv != NULL) {

        if (GET_OBJ_TYPE(inv) == ITEM_AUTOQUEST)
            found = TRUE;
    ...
    if (loop == 0) {
        // went through all possible mobs probably
        mudlog(CMP, LVL_GOD, TRUE, "WARNING: Too many rainbow tokens loaded, not enough mobs! modify.c (rainbow_token_load)");
        return;
    }
...
//    if (announce)
        send_to_all("@xA @xg@xr@xa@xn@xd @xr@xa@xi@xn@xb@xo@xw @xs@xl@xo@xw@xl@xy@x s@xp@xr@xe@xa@xd@xs@x a@xc@xr@xo@xs@xs@x t@xh@xe@x s@xk@xy@x.@n\r\n");

    mudlog(CMP, LVL_GOD, TRUE, "TOKEN: %s loaded on %s in %s (%d)", obj->short_description, GET_NAME(mob), world[to_room].name, world[to_room].number);
    rainbow_number++;
    rainbow_update();
}

// interpret an argument for do_restring
static void quad_arg(char *arg, int *type, char *name, int *field, char *string)
{
    ...
    //parse string, including multiple word string in "s
    ...
    if (*arg == '\"') {
        ...
        while (*arg && *arg != '\"' && i<(MAX_STRING_LENGTH-1)) {
            *(string++) = *arg;
            arg++;
            i++;
        }
        ...
        while (*arg && !isspace(*arg) && i<(MAX_STRING_LENGTH-1)) {
            *(string++) = *arg;
            arg++;
            i++;
        }
    ...
    *string = '\0';
    ...
    // get field number
    if (!(*field = search_block(buf, string_fields, 1)))
        return;
...
void restring_obj(struct char_data* ch, char* name, int field, char* string)
{
    ...
    if (!(obj = get_obj_in_list_vis(ch, name, NULL, ch->carrying))) {
        send_to_char(ch, "Can't find such a thing here..\r\n");
        return;
    }
    ...
    switch (field) {
        ...
        case 0: // name
            ...
            if (strlen(string)) {
                ...
                send_to_char(ch, "Changing name to: %s.\r\n", string);
                obj->name = strdup(string);
                //strncpy(obj->name, string, strlen(string));
            }
            else
                send_to_char(ch, "You must supply a new name.\r\n");
        ...
        case 1: // short description
            if (strlen(string)) {
                ...
                send_to_char(ch, "Changing short description to: %s.\r\n", string);
                obj->short_description = strdup(string);
                //strncpy(obj->short_description, string, strlen(string));
            }
            else
                send_to_char(ch, "You must supply a new short description.\r\n");
        ...
        case 2: // description
            ...
            if (strlen(string)) {
                ...
                send_to_char(ch, "Changing long description to: %s.\r\n", string);
                obj->description = strdup(string);
                //strncpy(obj->description, string, strlen(string));
            }
            else
                send_to_char(ch, "You must supply a new description.\r\n");

            break;

        default:
            send_to_char(ch, "That field is undefined for objects.\r\n");
            break;
    }
}

// todo: this procedure needs to be CHANGED */
void restring_mob(struct char_data* ch, char* name, int field, char* string)
{
    struct char_data *mob;

    if (!(mob = get_char_vis(ch, name, NULL, FIND_CHAR_WORLD))) {
        send_to_char(ch, "I don't know anyone by that name...\r\n");
        return;
    }

    if (!IS_NPC(mob)) {
        send_to_char(ch, "Sorry, string is not for characters.\r\n");
        return;
    }
    ...
    switch (field) {
        ...
        case 4:
            send_to_char(ch, "Monsters have no titles.\r\n");
            break;

        default:
            send_to_char(ch, "That field is undefined for monsters.\r\n");
            break;
    }
}
...
ACMD(do_restring)
{
    ...
    //   log(argument);
    if (!*argument) {
        send_to_char(ch, "Usage: restring ('obj' | 'mob') <name> <field> [\"<string>\"]\r\n");
        return;
    }

    quad_arg(argument, &type, name, &field, string);

    send_to_char(ch, "Type: %d, Name: %s, Field: %d, String: %s.\r\n", type, name, field, string);

    if (field == -1) {
        send_to_char(ch, "No field by that name. Try 'help string'.\r\n");
        return;
    }
    ...
    switch(type) {
        ...
        case TYPE_MOB:
            send_to_char(ch, "This is not implemented yet.\r\n");
            // restring_mob(ch, name, field, string);
            break;

        default:
            restring_obj(ch, name, field, string);
            break;
    }
}
