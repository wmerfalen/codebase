...
void parse_quest(FILE *quest_f, int nr)
{
  ...
  aquest_table[i].units_reward = t[0];
...
void generic_complete_quest(struct char_data *ch)
{
  ...
  int happy_qp, happy_units, happy_exp;
  ...
  if (--GET_QUEST_COUNTER(ch) <= 0) {
    ...
    if (QST_UNITS(rnum)) {
      if ((IS_HAPPYHOUR) && (IS_HAPPYUNITS)) {
        happy_units = (int)(QST_UNITS(rnum) * (((float)(100+HAPPY_UNITS))/(float)100));
        happy_units = MAX(happy_units, 0);
        increase_units(ch, happy_units);
        send_to_char(ch,
              "You have been awarded %d units for your service.\r\n",
              happy_units);
	  } else {
        increase_units(ch, QST_UNITS(rnum));
        send_to_char(ch,
              "You have been awarded %d units for your service.\r\n",
              QST_UNITS(rnum));
      }
    ...
    if (QST_EXP(rnum)) {
      gain_exp(ch, QST_EXP(rnum), FALSE);
