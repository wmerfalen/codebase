...
int get_obj_room(struct obj_data *obj);
/* Public functions made available from vehicles.c */
ACMD(do_drive);
ACMD(do_stop);
ACMD(do_refuel);
ACMD(do_vrestore);
...
#define BACKUP_FILE		 4
#define BIONICS_FILE     5  /* < Open up a player bionics save file                 */
#define STORAGE_FILE	 6
#define MAX_FILES        7 /**< Max number of files types vailable */
...
#define CHECK_PLAYER_SPECIAL(ch, var, spot) \
    (*(((ch)->player_specials == &dummy_mob) ? (log("SYSERR: Mob using '"#var"' at %s:%d for %d.", __FILE__, __LINE__, spot), &(var)) : &(var)))
#else
#define CHECK_PLAYER_SPECIAL(ch, var, spot)	(var)
#endif
...
#define PRF_FLAGS(ch) CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.pref), 1)
/** Affect flags on the NPC or PC. */
#define AFF_FLAGS(ch)	((ch)->char_specials.saved.affected_by)
/** New combat flags for new combat system... Seems legit - Gahan 2013 **/
#define CMB_FLAGS(ch)	((ch)->char_specials.saved.combat_flags)
/** Room flags.
...
/** 1 if flag is set in the combatflag bitarray, 0 if not. */
#define CMB_FLAGGED(ch, flag) (IS_SET_AR(CMB_FLAGS(ch), (flag)))
...
#define IS_TABLE(obj) (GET_OBJ_TYPE(obj) == ITEM_TABLE)
...
#define GET_STR_ADD(ch)     ((ch)->aff_abils.str_add)
/** Current dexterity of ch. */
#define GET_DEX(ch)     ((ch)->aff_abils.dex)
/** Current intelligence of ch. */
#define GET_INT(ch)     ((ch)->aff_abils.intel)
/** Current wisdom of ch. */
#define GET_PCN(ch)     ((ch)->aff_abils.pcn)
/** Current constitution of ch. */
#define GET_CON(ch)     ((ch)->aff_abils.con)
/** Current charisma of ch. */
#define BIONICS_CHA_LOSS(ch)   ((ch)->char_specials.bionic_cha_loss)
#define GET_CHA(ch)            (GET_CHA_ABIL(ch))
...
#define GET_PSI(ch)	  ((ch)->points.psi)
/** Maximum mana points (magic) of ch. */
#define GET_MAX_PSI_PTS(ch)   ((ch)->points.max_psi)
#define BIONICS_PSI_LOSS(ch)  ((ch)->char_specials.bionic_psi_loss)
#define GET_MAX_PSI(ch)       (MAX(0, GET_MAX_PSI_PTS(ch) - BIONICS_PSI_LOSS(ch)))
/** Current to-hit roll modifier for ch. */
#define GET_HITROLL(ch)	  ((ch)->points.hitroll)
/** Current damage roll modifier for ch. */
#define GET_DAMROLL(ch)   ((ch)->points.damroll)
/** Total amount of Rifts closed */
#define GET_RIFTS(ch)	  ((ch)->points.rifts)
/** Total amount of Event mobs killed */
#define GET_EVENTMOBS(ch) ((ch)->points.eventmobs)
/** Total verified Event shard counters */
#define GET_GREEN_SHARDS(ch)	((ch)->points.greenshards)
#define GET_BLUE_SHARDS(ch)		((ch)->points.blueshards)
#define GET_BLACK_SHARDS(ch)	((ch)->points.blackshards)
...
// Vehicle stuff
#define VEHICLE(ch)		  ((ch)->char_specials.vehicle.vehicle)
#define VEHICLE_NAME(ch)  ((vehicle)->short_description)
#define IN_VEHICLE(ch)  ((ch)->char_specials.invehicle == TRUE)
#define VEHICLE_VNUM(ch) ((ch)->char_specials.vehicle.vehicle_room_vnum)
#define WINDOW_VNUM(ch) ((ch)->char_specials.vehicle.vehicle_in_room)
// Combo Stuff
#define COMBOCOUNTER(ch, i)	((ch)->char_specials.combocounter[i])
#define GET_COMBO_LEARNED(ch, i) ((ch)->char_specials.saved.combos ? (((ch)->char_specials.saved.combos)[i].ident) : 0)
#define SET_COMBO_LEARNED(ch, i, id) { if ((ch)->char_specials.saved.combos) ((ch)->char_specials.saved.combos)[i].ident = id; }

/** Who or what the ch is hunting. */
#define HUNTING(ch)	  ((ch)->char_specials.hunting)
#define FUN_HUNTING(ch)			((ch)->char_specials.funhunting)
...
#define GET_COND(ch, i)		CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.conditions[(i)]), 2)
/** The room to load player ch into. */
#define GET_LOADROOM(ch)	CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.load_room), 3)
/** Number of skill practice sessions remaining for ch. */
#define GET_PRACTICES(ch)	CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.spells_to_learn), 4)
/** Current invisibility level of ch. */
#define GET_INVIS_LEV(ch)	CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.invis_level), 5)
/** Current wimpy level of ch. */
#define GET_WIMP_LEV(ch)	CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.wimp_level), 6)
/** Current freeze level (god command) inflicted upon ch. */
#define GET_FREEZE_LEV(ch)	CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.freeze_level), 7)
/** Current number of bad password attempts at logon. */
#define GET_BAD_PWS(ch)		CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.bad_pws), 8)
/** Not used?
...
#define POOFIN(ch)		CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->poofin),9)
/** The poofout string for the ch. */
#define POOFOUT(ch)		CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->poofout),10)
/** The OLC zoon permission for ch.
 * @deprecated Currently unused? */
#define GET_OLC_ZONE(ch)	CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.olc_zone),11)
/** Currently unused?
 * @deprecated Currently unused? */
#define GET_LAST_OLC_TARG(ch)	CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->last_olc_targ),12)
/** Currently unused?
 * @deprecated Currently unused? */
#define GET_LAST_OLC_MODE(ch)	CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->last_olc_mode),13)
/** Retrieve command aliases for ch. */
#define GET_ALIASES(ch)		CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->aliases),14)
/** Who ch last spoke to with the 'tell' command. */
#define GET_LAST_TELL(ch)	CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->last_tell),15)
/** Get unique session id for ch. */
#define GET_PREF(ch)      ((ch)->pref)
/** Get host name or ip of ch. */
#define GET_HOST(ch)		CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->host),16)
#define GET_LAST_MOTD(ch)       CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.lastmotd),26)
#define GET_LAST_NEWS(ch)       CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.lastnews),27)
#define GET_EXITS(ch)			CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->exits),28)
/** Get channel history i for ch. */
#define GET_HISTORY(ch, i)      CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.comm_hist[i]),17)
/** Return the page length (height) for ch. */
#define GET_PAGE_LENGTH(ch)     CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.page_length),18)
/** Return the page width for ch */
#define GET_SCREEN_WIDTH(ch)    CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.screen_width),19)
/* Autoquests data */
/** Return the number of questpoints ch has. */
#define GET_QUESTPOINTS(ch)     CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.questpoints),20)
/** Return the current quest that a player has assigned */
#define GET_QUEST(ch)           CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.current_quest), 21)
/** Number of goals completed for this quest. */
#define GET_QUEST_COUNTER(ch)   CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.quest_counter),22)
/** Time remaining to complete the quest ch is currently on. */
#define GET_QUEST_TIME(ch)      CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.quest_time),23)
/** The number of quests completed by ch. */
#define GET_NUM_QUESTS(ch)      CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.num_completed_quests),24)
...
#define GET_SKILL_LEVEL(ch, i)  ((ch)->char_specials.saved.skills ? (((ch)->char_specials.saved.skills)[i].level) : 0)
#define SET_SKILL(ch, i, lvl)   { if ((ch)->char_specials.saved.skills) ((ch)->char_specials.saved.skills)[i].level = lvl; }
#define GET_PSIONIC_LEVEL(ch, i)  ((ch)->char_specials.saved.psionics ? (((ch)->char_specials.saved.psionics)[i].level) : 0)
#define SET_PSIONIC(ch, i, lvl)   { if ((ch)->char_specials.saved.psionics) ((ch)->char_specials.saved.psionics)[i].level = lvl; }
...
#define STRENGTH_APPLY_INDEX(ch) \
        ( ((GET_STR_ADD(ch) ==0) || (GET_STR(ch) != 18)) ? GET_STR(ch) :\
          (GET_STR_ADD(ch) <= 50) ? 26 :( \
          (GET_STR_ADD(ch) <= 75) ? 27 :( \
          (GET_STR_ADD(ch) <= 90) ? 28 :( \
          (GET_STR_ADD(ch) <= 99) ? 29 :  30 ) ) )                   \
        )
...
#define IS_CORPSE(obj)		(GET_OBJ_TYPE(obj) == ITEM_CONTAINER && \
					GET_OBJ_VAL((obj), 3) == 1)
#define IS_BOUND(obj)       (obj->bound_id != 0 && obj->bound_name != NULL)
...
#define INVIS_OK(sub, obj) \
    ...
    (!AFF_FLAGGED((obj), AFF_HIDE) || AFF_FLAGGED(sub, AFF_INFRAVISION)))

/** Defines if sub character can see obj character, assuming mortal only
 * settings. */
#define MORT_CAN_SEE(sub, obj) (LIGHT_OK(sub) && (INVIS_OK(sub, obj) || MOB_FLAGGED(sub, MOB_QUESTMASTER)) && (!CHAR_IS_HIDDEN(obj) || MOB_FLAGGED(sub, MOB_QUESTMASTER)))
...
#define MORT_CAN_SEE_OBJ(sub, obj) \
  ((LIGHT_OK(sub) && INVIS_OK_OBJ(sub, obj) && CAN_SEE_OBJ_CARRIER(sub, obj) && !OBJ_IS_HIDDEN(obj)) || GET_OBJ_TYPE(obj) == ITEM_LIGHT)
...
#define VEHICLE_EXIT(ch, door)  (world[WINDOW_VNUM(ch)].dir_option[door])
...
#define VEHICLE_CAN_GO(ch, door) (VEHICLE_EXIT(ch, door) && \
			 (VEHICLE_EXIT(ch,door)->to_room != NOWHERE) && \
			 !IS_SET(VEHICLE_EXIT(ch, door)->exit_info, EX_CLOSED))
...
#define CLASS_ABBR(ch) (IS_NPC(ch) ? "--" : class_display[(int)GET_CLASS(ch)].class_name)
...
#define IS_HAPPYUNITS (happy_data.gold_rate > 0)

#define HAPPY_EXP    happy_data.exp_rate
#define HAPPY_UNITS   happy_data.gold_rate
#define HAPPY_QP     happy_data.qp_rate

#define HAPPY_TIME   happy_data.ticks_left

#define IS_HAPPYHOUR ((IS_HAPPYEXP || IS_HAPPYUNITS || IS_HAPPYQP) && (HAPPY_TIME > 0))
...
/* SPECIFIC CYBERASSAULT MACROS - Gahan */
/* Other Macros */
#define DEATH_TICKS(d)			((d)->dead)
#define GET_PSI_REGEN_RAW(ch)     ((ch)->player_specials->saved.char_psi_regen)
#define BIONIC_PSI_REGEN_LOSS(ch) ((ch)->char_specials.bionic_psi_regen_loss)
#define GET_PSI_REGEN(ch)         (CHECK_PLAYER_SPECIAL((ch), GET_PSI_REGEN_RAW(ch)) - BIONIC_PSI_REGEN_LOSS(ch))

#define GET_HIT_REGEN(ch)       ((ch)->player_specials->saved.char_hit_regen)
#define GET_MOVE_REGEN(ch)      ((ch)->player_specials->saved.char_move_regen)
#define GET_ALL_REGEN(ch)       ((ch)->player_specials->saved.char_all_regen)
#define GET_COPYOVER_ROOM(ch)   CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.copyover_room), 25)
#define WHO_CAN_SEE(sub, obj) ((!AFF_FLAGGED((obj),AFF_INVISIBLE) || AFF_FLAGGED(sub,AFF_DETECT_INVIS) || GET_LEVEL(sub) >= LVL_IMMORT )&& (GET_LEVEL(sub) >= GET_INVIS_LEV(obj)))
#define GET_MOB_HOME(mob)		((mob)->mob_specials.home)
#define GET_DAMTYPE(mob)		((mob)->mob_specials.damtype)
#define GET_PROMPT(ch)          CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->prompt_string), 25)
#define IS_SCRATCHED(obj)       (GET_OBJ_VAL((obj), 3))
#define GET_CHARGE(obj)         (GET_OBJ_VAL((obj), 0))
#define GET_OBJ_STAMP(obj)      ((obj)->obj_flags.stamp)
#define GET_DELAY_HIT(ch)	((ch)->delay_hit)
#define GET_DELAY_HIT_WAIT(ch)	((ch)->delay_hit_wait)
#define GET_MOB_ATTACKS(ch)     ((ch)->mob_specials.num_attacks)
#define GET_MOB_IMAGE(ch)       ((ch)->mob_specials.image)
#define GET_MOB_CLASS(mob)		((mob)->mob_specials.mobclass)
// Maximum experience an NPC will award
#define GET_MAX_EXP(ch)     ((ch)->points.max_exp)
/* Room Macros */
#define CAN_GO2(roomnum, door) (W_EXIT((roomnum), (door)) && \
    (W_EXIT((roomnum), (door))->to_room != NOWHERE) && \
    !IS_SET(W_EXIT((roomnum),(door))->exit_info, EX_CLOSED))
#define NEXT_ROOM(room, door)   (W_EXIT((room), (door))->to_room)
#define PK_OK_ROOM(ch)      (ROOM_FLAGGED((ch)->in_room,ROOM_PK_OK))
#define PK_NOT_OK_ROOM(ch)  (ROOM_FLAGGED((ch)->in_room, ROOM_HOLY_GROUND) || ROOM_FLAGGED((ch)->in_room, ROOM_PEACEFUL))
#define HAS_PSIONIC_ROUTINE(spl, flag) (IS_SET(PSI_ROUTINES(spl), (flag)))
#define GET_TOTHITS(ch)     ((ch)->char_specials.tothits)
#define GET_TOTDAM(ch)      ((ch)->char_specials.totdam)
// References the routine element for a psionic. Currently unused
#define PSI_ROUTINES(psi)    (psionics_info[psi].routines)
/* Crafting Macros - Gahan 2013 */
#define GET_PROFESSION(ch)		((ch)->player_specials->saved.crafting.cclass)
#define GET_SUB1(ch)			((ch)->player_specials->saved.crafting.subcclass1)
#define GET_SUB2(ch)			((ch)->player_specials->saved.crafting.subcclass2)
/* CH MACROS */
#define GET_AGE_EFFECTS(ch)     ((ch)->player_specials->saved.age_effects)
#define GET_ATTRIB_ADD(ch)      ((ch)->player_specials->saved.attrib_add)
#define GET_PSI_REGEN_RAW(ch)     ((ch)->player_specials->saved.char_psi_regen)
#define BIONIC_PSI_REGEN_LOSS(ch) ((ch)->char_specials.bionic_psi_regen_loss)
#define GET_PSI_REGEN(ch)         (CHECK_PLAYER_SPECIAL((ch), GET_PSI_REGEN_RAW(ch)) - BIONIC_PSI_REGEN_LOSS(ch))
#define GET_CHA_ABIL(ch)       ((ch)->aff_abils.cha)
#define GET_MAX_HIT_PTS(ch)    ((ch)->points.max_hit)
#define RANGED_FIGHTING(ch)		((ch)->char_specials.ranged_fighting)
#define IS_DEAD(ch)             (GET_POS(ch) == POS_DEAD)
#define CLASS_DUALABBR(ch) (IS_NPC(ch) ? "--" : class_display[(int)GET_CLASS(ch)].dualclass_name)
#define CLASS_DUALABBR2(ch) (IS_NPC(ch) ? "--" : class_display[(int)GET_CLASS2(ch)].dualclass_name)
#define GET_WIZTITLE(ch)		((ch)->player_specials->wiztitle)
#define TICK_TO_HIT(ch)			((ch)->char_specials.tick_to_hit)
#define COMBAT_TICKS(ch)		((ch)->char_specials.combat_tick)
#define RANGED_COMBAT_TICKS(ch)	((ch)->char_specials.ranged_combat_tick)
#define TRIGGER_TICKS(ch)		((ch)->char_specials.trigger_tick)
#define ATTACKS_PER_ROUND(ch)	((ch)->char_specials.attacks)
#define GET_CIRCLETICKS(ch)		((ch)->char_specials.circle_tick)
#define GET_CORETICKS(ch)		((ch)->char_specials.core_tick)
#define GET_PSIBULLETS(ch)		((ch)->char_specials.psibullets)
#define GET_CLAWS(ch)		((ch)->char_specials.callerclaws)
#define GET_STORAGESTATE(ch)	((ch)->char_specials.storage)
#define GET_STORAGESPACE(ch)	((ch)->player.storagecap)
#define IS_CARRYING_W(ch)       ((ch)->char_specials.carry_weight)
#define IS_WEARING_W(ch)		((ch)->char_specials.worn_weight)
#define IS_CARRYING_N(ch)       ((ch)->char_specials.carry_items)
#define IS_CARRYING_TOT(ch)		(IS_CARRYING_W(ch) + IS_WEARING_W(ch))
#define INSURANCE(ch)			((ch)->char_specials.insurance)
#define ENCUMBRANCE(ch)			((ch)->char_specials.encumbrance)
#define GET_FAME(ch)			((ch)->player_specials->saved.fame)
#define GET_PROGRESS(ch)		((ch)->player_specials->saved.progress)
#define GET_PROGRESSPCNT(ch)	(((((ch)->player_specials->saved.progress * 100) / 15) -6) + (GET_LEVEL(ch) / 6))
#define GET_TIER(ch)			((ch)->player_specials->saved.tier)
#define GET_TIER2(ch)			((ch)->player_specials->saved.tier2)
#define GET_CLASS2(ch)			((ch)->player_specials->saved.class2)
#define GET_BANK_UNITS(ch)		((ch)->points.bank_units)
#define GET_FREQ(ch)            ((ch)->player_specials->saved.frequency)
#define GET_UNITS(ch)			((ch)->points.units)
#define LOCKED(ch)				((ch)->char_specials.locked)
...
#define GET_REAL_PCN(ch)    ((ch)->real_abils.pcn)
#define GET_REAL_CON(ch)    ((ch)->real_abils.con)
#define GET_REAL_CHA(ch)    ((ch)->real_abils.cha)
#define PART_MISSING(ch, part)                    (IS_SET(BODYPART_CONDITION(ch, part), RIPPED) || IS_SET(BODYPART_CONDITION(ch, part), BIONIC_MISSING))
#define IS_MERCENARY(ch)	((!IS_NPC(ch)) && (find_class_bitvector(pc_class[GET_CLASS(ch)].class_name) & find_class_bitvector(pc_class[CLASS_MERCENARY].class_name)))
#define IS_CRAZY(ch)		((!IS_NPC(ch)) && (find_class_bitvector(pc_class[GET_CLASS(ch)].class_name) & find_class_bitvector(pc_class[CLASS_CRAZY].class_name)))
#define IS_STALKER(ch)		((!IS_NPC(ch)) && (find_class_bitvector(pc_class[GET_CLASS(ch)].class_name) & find_class_bitvector(pc_class[CLASS_STALKER].class_name)))
#define IS_CALLER(ch)		((!IS_NPC(ch)) && (find_class_bitvector(pc_class[GET_CLASS(ch)].class_name) & find_class_bitvector(pc_class[CLASS_CALLER].class_name)))
#define IS_BORG(ch)			((!IS_NPC(ch)) && (find_class_bitvector(pc_class[GET_CLASS(ch)].class_name) & find_class_bitvector(pc_class[CLASS_BORG].class_name)))
#define IS_HIGHLANDER(ch)	((find_class_bitvector(pc_class[GET_CLASS(ch)].class_name) & find_class_bitvector(pc_class[CLASS_HIGHLANDER].class_name)) || (IS_NPC(ch) && GET_MOB_CLASS(ch) == MOB_CLASS_HIGHLANDER))
//#define IS_HIGHLANDER(ch)	(find_class_bitvector(pc_class[GET_CLASS(ch)].class_name) & find_class_bitvector(pc_class[CLASS_HIGHLANDER].class_name))
#define IS_PREDATOR(ch)		((!IS_NPC(ch)) && (find_class_bitvector(pc_class[GET_CLASS(ch)].class_name) & find_class_bitvector(pc_class[CLASS_PREDATOR].class_name)))
#define GET_PSIMASTERY(ch)      ((ch)->player_specials->saved.psimastery)
#define GET_HIT_MOD(ch)         ((ch)->char_specials.mod_Hit)
#define GET_PSI_MOD(ch)         ((ch)->char_specials.mod_Psi)
#define GET_HND_MOD(ch)         ((ch)->char_specials.mod_HnD)
#define GET_AC_MOD(ch)          ((ch)->char_specials.mod_AC)
#define GET_REGEN_MOD(ch)       ((ch)->char_specials.mod_Regen)
#define GET_NUM_DEATHS(ch)      ((ch)->player.num_deaths)
#define GET_NUM_DTS(ch)         ((ch)->player.num_dts)
#define GET_AGE_MOD(ch)         ((ch)->player.age_modifier)
#define GET_PERCENT_MACHINE(ch) ((ch)->char_specials.saved.percent_machine)
#define GET_NUM_ESSENCES(ch)    ((ch)->player.essences)
#define GET_REMORTS(ch)     ((ch)->player.num_remorts)
#define AUTOPILOT_ROOM(ch)	((ch)->player.autopilot)
#define GET_NUM_DUPLICATES(ch)  ((ch)->char_specials.saved.duplicates)
#define IS_REMORT(ch)           (GET_REMORTS((ch)) > 0 ? TRUE : FALSE)
#define HAS_BIONIC(ch, i)       (BIONIC_LEVEL(ch, i) ? TRUE : FALSE)
#define BIONIC_LEVEL(ch, i)     ((ch)->char_specials.saved.bionics[(i)])
#define STANA(str)              (strchr("aeiouAEIOU", *str) ? "an" : "a")
// body part utils
#define BODYPART_CONDITION(ch, part)              ((ch)->char_specials.body_part[part].condition)
#define BIONIC_DEVICE(ch, part)	                  ((ch)->char_specials.body_part[part].bionic_device)
#define GET_NO_HITS(ch, part)                     ((ch)->char_specials.body_part[part].no_of_hits)
#define BIONIC_PERCENT(ch)                        ((ch)->char_specials.bionic_percent)
#define BODY_PART_HIT_LOSS(ch) ((ch)->char_specials.body_part_hit_loss)
#define BIONICS_PSI_LOSS(ch)  ((ch)->char_specials.bionic_psi_loss)
#define GET_TEAM(ch)            ((ch)->char_specials.team)
#define SET_BODYPART_CONDITION(ch, part, cond)    SET_BIT(BODYPART_CONDITION(ch, part), cond)
#define REMOVE_BODYPART_CONDITION(ch, part, cond) REMOVE_BIT(BODYPART_CONDITION(ch, part), cond)
#define GET_BODYPART_CONDITION(ch, part, cond)    IS_SET(BODYPART_CONDITION(ch, part), cond)
#define PART_MISSING(ch, part)                    (IS_SET(BODYPART_CONDITION(ch, part), RIPPED) || IS_SET(BODYPART_CONDITION(ch, part), BIONIC_MISSING))
#define HAS_ONE_HAND(ch)                          (!PART_MISSING(ch, BODY_PART_LEFT_HAND) || !PART_MISSING(ch, BODY_PART_RIGHT_HAND))
#define HAS_BOTH_HANDS(ch)                        (!PART_MISSING(ch, BODY_PART_LEFT_HAND) && !PART_MISSING(ch, BODY_PART_RIGHT_HAND))
#define BIONIC_LOCATION(device)                   (GET_OBJ_VAL(device, 0))
#define BIONIC_CHARISMA_LOSS(device)              (GET_OBJ_VAL(device, 1))
/* OBJ MACROS */
#define IS_OBJ_TIMER_ON(obj)    ((obj)->obj_flags.timer_on == TRUE)
#define OBJ_IS_HIDDEN(obj)	(OBJ_FLAGGED((obj), ITEM_HIDDEN))
#define CHAR_IS_HIDDEN(obj)	(AFF_FLAGGED(obj, AFF_HIDE))
#define GET_OBJ_WEAPONPSIONIC   ((obj)->obj_wpnpsi.which_psionic)
#define GET_OBJ_WEAPONPSIONICSKILL   ((obj)->obj_wpnpsi.skill_level)
#define GET_OBJ_APPLY_LOC(obj, i)   ((obj)->obj_flags.applies[(i)].location)
#define GET_OBJ_APPLY_MOD(obj, i)   ((obj)->obj_flags.applies[(i)].modifier)
#define GET_EXPLOSIVE_TYPE(obj)     ((obj)->obj_flags.value[0])
#define GET_EXPLOSIVE_SUB_TYPE(obj) ((obj)->obj_flags.value[1])
#define GET_CHARGE(obj)				(GET_OBJ_VAL((obj), 0))
#define IS_ATTACHMENT(obj)                (GET_OBJ_TYPE(obj) == ITEM_ATTACHMENT)
#define IS_AMMO(obj)                (GET_OBJ_TYPE(obj) == ITEM_AMMO)
#define IS_GUN(obj)                 (GET_OBJ_TYPE(obj) == ITEM_GUN)
#define GET_AMMO(obj)               ((obj)->obj_weapon.ammo_loaded)
#define GET_AMMO_LOADED(obj)        ((obj)->obj_weapon.ammo_loaded)
#define GET_AMMO_DAMTYPE(obj)		(GET_OBJ_VAL(GET_AMMO(obj), 2))
#define IS_LOADED(obj)              (GET_AMMO(obj) != NULL)
#define GET_AMMO_TYPE(obj)          ((obj)->obj_ammo.ammo_type)
#define GET_GUN_TYPE(obj)           ((obj)->obj_weapon.gun_type)
#define GET_AMMO_COUNT(obj)         ((obj)->obj_ammo.ammo_count)
#define GET_LOADED_AMMO_COUNT(obj)  ((obj)->obj_weapon.ammo_loaded->obj_ammo.ammo_count)
#define GET_SHOTS_PER_ROUND(obj)    ((obj)->obj_weapon.shots_per_round)
#define GET_RANGE(obj)			    ((obj)->obj_weapon.range)
#define GET_GUN_BASH_NUM(obj)       ((obj)->obj_weapon.bash[0])
#define GET_GUN_BASH_SIZE(obj)      ((obj)->obj_weapon.bash[1])
#define GET_OBJ_REMORT(obj)        ((obj)->obj_flags.remort)
#define GET_OBJ_COMPOSITION(obj)	((obj)->obj_flags.composition)
#define GET_ITEM_COMPOSITION(obj)	(category_composition_types[(obj)->obj_flags.composition])
#define GET_OBJ_SUBCOMPOSITION(obj)		((obj)->obj_flags.subcomposition)
#define GET_ITEM_SUBCOMPOSITION(obj)	\
	((GET_OBJ_COMPOSITION(obj) == 0) ? nothing_composition_types[(obj)->obj_flags.subcomposition] :( \
	(GET_OBJ_COMPOSITION(obj) == 1) ? trade_composition_types[(obj)->obj_flags.subcomposition] :( \
	(GET_OBJ_COMPOSITION(obj) == 2) ? metal_composition_types[(obj)->obj_flags.subcomposition] :( \
	(GET_OBJ_COMPOSITION(obj) == 3) ? pmetal_composition_types[(obj)->obj_flags.subcomposition] :( \
	(GET_OBJ_COMPOSITION(obj) == 4) ? gem_composition_types[(obj)->obj_flags.subcomposition] :( \
	(GET_OBJ_COMPOSITION(obj) == 5) ? rock_composition_types[(obj)->obj_flags.subcomposition] :( \
	(GET_OBJ_COMPOSITION(obj) == 6) ? organic_composition_types[(obj)->obj_flags.subcomposition] :( \
	(GET_OBJ_COMPOSITION(obj) == 7) ? other_composition_types[(obj)->obj_flags.subcomposition] : \
	nothing_composition_types[(obj)->obj_flags.subcomposition]))))))))
// Cyberassault Defines
extern const struct class_display_type class_display[NUM_CLASSES + 1];
#endif /* _UTILS_H_ */
