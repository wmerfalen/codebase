...
static void prefedit_disp_main_menu(struct descriptor_data *d)
{
  ...
  vict = PREFEDIT_GET_CHAR;
    ...
    sprintf(prompt_string, "%s%s%s", PREFEDIT_FLAGGED(PRF_DISPHP) ? "H" : "",   PREFEDIT_FLAGGED(PRF_DISPPSI) ? "P" : "",
...
static void prefedit_disp_toggles_menu(struct descriptor_data *d)
{
  ...
  /* The top section of the actual menu */
    send_to_char(d->character,
        ...
        "%s3%s) Autounits    %s[%s%3s%s]      %sC%s) Tell     %s[%s%3s%s]\r\n"
        // "%s4%s) Autosac      %s[%s%3s%s]\r\n"
        "%sD%s) Auction      %s[%s%3s%s]\r\n"
        ...
        // Line 3 - autounits and tell
        CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM), PREFEDIT_FLAGGED(PRF_AUTOUNITS) ? CBGRN(d->character, C_NRM) : CBRED(d->character, C_NRM),
        ONOFF(PREFEDIT_FLAGGED(PRF_AUTOUNITS)), CCCYN(d->character, C_NRM), CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM),
    ...
    send_to_char(d->character, "\r\n"
        ...
        CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM), CCYEL(d->character, C_NRM),
        ONOFF(!PREFEDIT_FLAGGED(PRF_NOSUMMON)), CCCYN(d->character, C_NRM), CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM),
  ...
  case PREFEDIT_MAIN_MENU:
    ...
    /* Below this point are Imm-only toggles */
        ...
        case PREFEDIT_PAGELENGTH:
            number = atoi(arg);
            OLC_PREFS(d)->page_length = MAX(10, MIN(number, 5000000));
            break;
        ...
        case PREFEDIT_TOGGLE_MENU:
            ...
            switch (*arg) {
                ...
                case '3':
                    TOGGLE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_AUTOUNITS);
                    break;
                ...
                case 'f':
                case 'F':
                    TOGGLE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_NOSUMMON);
                    break;
...
void prefedit_Restore_Defaults(struct descriptor_data *d)
{
  ...
  /* PRF_DISPMANA   - On */
  if (!PREFEDIT_FLAGGED(PRF_DISPPSI))
     SET_BIT_AR(PREFEDIT_GET_FLAGS, PRF_DISPPSI);
  ...
  /* PRF_NOSUMMON - Off */
  if (PREFEDIT_FLAGGED(PRF_NOSUMMON))
     REMOVE_BIT_AR(PREFEDIT_GET_FLAGS, PRF_NOSUMMON);
  ...
  /* PRF_AUTOUNITS   - On */
  if (!PREFEDIT_FLAGGED(PRF_AUTOUNITS))
     SET_BIT_AR(PREFEDIT_GET_FLAGS, PRF_AUTOUNITS);
