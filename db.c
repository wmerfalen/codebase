...
char *nmotd = NULL;                                 /* message for newbies           */
...
// Halloween event
//static void halloween_event(void);
...
ACMD(do_reboot)
{
  ...
  if (!str_cmp(arg, "all") || *arg == '*') {
    ...
    if (file_to_string_alloc(NMOTD_FILE, &nmotd) < 0)
      send_to_char(ch, "Cannot read nmotd\r\n");
...
void boot_world(void)
{
  ...
  log("Resetting World event info.");
  world_event_boot();
...
void boot_db(void)
{
  ...
  sort_commands();
  log("Sorting psionics list.");
  sort_psionics();
  log("Sorting skills list.");
  sort_skills();
...
#if 1
{
  ...
  if (!boot_time)
    boot_time = time(0);
  log("Booting rainbow token information.");
  rainbow_boot();

  // Halloween event - Gahan
  //log("Beginning halloween event cleanup.");
  //halloween_event();
...
static void halloween_event(void)
{
    ...
    for (room = 0; room <= top_of_world; room++) {
		if (room == NOWHERE || room > top_of_world) return;
...
void setup_dir(FILE *fl, int room, int dir)
{
  ...
  if (sscanf(line, " %d %d %d ", t, t + 1, t + 2) != 3) {
    ...
    if (t[0] == 1)
        ...
        world[room].dir_option[dir]->exit_info = EX_ISDOOR | EX_PICKPROOF | EX_BLOWPROOF;
    else if (t[0] == 3)
        world[room].dir_option[dir]->exit_info = EX_ISDOOR | EX_PICKPROOF | EX_HIDDEN;
    else if (t[0] == 4)
        world[room].dir_option[dir]->exit_info = EX_ISDOOR | EX_PICKPROOF;
    else if (t[0] == 5)
        world[room].dir_option[dir]->exit_info = EX_ISDOOR | EX_BLOWPROOF;
    else if (t[0] == 6)
        world[room].dir_option[dir]->exit_info = EX_ISDOOR | EX_BLOWPROOF | EX_HIDDEN;
    else if (t[0] == 7)
        world[room].dir_option[dir]->exit_info = EX_ISDOOR | EX_BLOWPROOF | EX_PICKPROOF | EX_HIDDEN;
    else if (t[0] == 8)
...
static void parse_simple_mob(FILE *mob_f, int i, int nr)
{
  ...
  GET_PSI(mob_proto + i) = t[4];
  GET_MOVE(mob_proto + i) = t[5];
  GET_MAX_PSI_PTS(mob_proto + i) = 1000;
  GET_MAX_MOVE(mob_proto + i) = 100;

  mob_proto[i].mob_specials.damnodice = t[6];
  if (mob_proto[i].mob_specials.damnodice < 0)
	  log("Warblade: Mob %s has negative number of damdice [%d]", GET_NAME(mob_proto + i), GET_MOB_VNUM(mob_proto + i));
  mob_proto[i].mob_specials.damsizedice = t[7];
  if (mob_proto[i].mob_specials.damsizedice < 0)
	  log("Warblade: Mob %s has negative size of damdice [%d]", GET_NAME(mob_proto + i), GET_MOB_VNUM(mob_proto + i));
  ...
  if (sscanf(line, " %d %d ", t, t + 1) != 2) {
    ...
    GET_UNITS(mob_proto + i) = t[0];
    GET_EXP(mob_proto + i) = t[1];
    GET_MAX_EXP(mob_proto + i) = t[1];
  ...
  if (!get_line(mob_f, line)) {
    ...
    if (sscanf(line, " %d %d %d %d %d %d", t, t + 1, t + 2, t + 3, t + 4, t + 5) < 6) {
        ...
        //    "...expecting line of form '# # # # #'", nr);
			t[5] = 0;
			add_to_save_list(zone_table[real_zone_by_thing(nr)].number, SL_MOB);
        //exit(1);
    }
    ...
    GET_MOB_CLASS(mob_proto + i) = t[3];

    // todo: need a util define here
    mob_proto[i].mob_specials.num_attacks = t[4];
	mob_proto[i].mob_specials.damtype = t[5];
	if (mob_proto[i].mob_specials.damtype < 0 || mob_proto[i].mob_specials.damtype >= MAX_DMG ) {
		mob_proto[i].mob_specials.damtype = 0;
		add_to_save_list(zone_table[real_zone_by_thing(nr)].number, SL_MOB);
	}
    GET_NUM_ESSENCES(mob_proto + i) = 0;
    GET_WEIGHT(mob_proto + i) = 200;
    GET_HEIGHT(mob_proto + i) = 198;
	mob_proto[i].char_specials.team = 0;
}
...
static void interpret_espec(const char *keyword, const char *value, int i, int nr)
{
  ...
  CASE("Pcn") {
    RANGE(3, 25);
    mob_proto[i].real_abils.pcn = num_arg;
  }
  ...
  CASE("Cha") {
    ...
    CASE("SavingNormal") {
        RANGE(-100, 100);
        mob_proto[i].char_specials.saved.apply_saving_throw[SAVING_NORMAL] = num_arg;
    }
    ...
    CASE("SavingDrug") {
        RANGE(-100, 100);
        mob_proto[i].char_specials.saved.apply_saving_throw[SAVING_DRUG] = num_arg;
    }

    CASE("SavingFire") {
        RANGE(-100, 100);
        mob_proto[i].char_specials.saved.apply_saving_throw[SAVING_FIRE] = num_arg;
    }

    CASE("SavingCold") {
        RANGE(-100, 100);
        mob_proto[i].char_specials.saved.apply_saving_throw[SAVING_COLD] = num_arg;
    }
    CASE("SavingElectricity") {
        RANGE(-100, 100);
        mob_proto[i].char_specials.saved.apply_saving_throw[SAVING_ELECTRICITY] = num_arg;
    }
    CASE("SavingExplosive") {
        RANGE(-100, 100);
        mob_proto[i].char_specials.saved.apply_saving_throw[SAVING_EXPLOSIVE] = num_arg;
    }
    CASE("SavingPsionic") {
        RANGE(-100, 100);
        mob_proto[i].char_specials.saved.apply_saving_throw[SAVING_PSIONIC] = num_arg;
    }
    CASE("SavingNonorm") {
        RANGE(-100, 100);
        mob_proto[i].char_specials.saved.apply_saving_throw[SAVING_NONORM] = num_arg;
    }
	CASE("Team") {
		RANGE(0, 8);
		mob_proto[i].char_specials.team = num_arg;
	}
	CASE("SavingPara") {
		RANGE(-100, 100);
		mob_proto[i].char_specials.saved.apply_saving_throw[SAVING_NONORM] = 0;
	}
	CASE("SavingPara") {
		RANGE(-100, 100);
		mob_proto[i].char_specials.saved.apply_saving_throw[SAVING_NONORM] = 0;
...
void parse_mobile(FILE *mob_f, int nr)
{
    ...
    if (((retval = sscanf(line, "%s %s %s %s %s %s %s %s %c", f1, f2, f3, f4, f5, f6, f7, f8, &letter)) != 9)) {
...
char *parse_object(FILE *obj_f, int nr)
{
    ...
    // todo: do we need to clear gun and ammo data?
    ...
    if ((retval = sscanf(line, "%d %d %d %d %d %d", t, t + 1, t + 2, t + 3, t + 4, t + 5)) != 6) {
        log("SYSERR: Format error in second numeric line (expecting 6 args, got %d), %s", retval, buf2);
        exit(1);
    }
    ...
    GET_GUN_TYPE(obj_proto + i) = 0;
    GET_AMMO_TYPE(obj_proto + i) = 0;

    // this is stupid and will remain so until I fix guns -- Tails
    if (GET_OBJ_TYPE(obj_proto + i) == ITEM_GUN) {
        GET_GUN_TYPE(obj_proto + i) = t[0];
        GET_SHOTS_PER_ROUND(obj_proto + i) = t[3];
        GET_RANGE(obj_proto + i) = t[4];
        GET_AMMO_LOADED(obj_proto + i) = NULL;
        GET_GUN_BASH_NUM(obj_proto + i) = 1;
        GET_GUN_BASH_SIZE(obj_proto + i) = t[5];
    }
    else if (GET_OBJ_TYPE(obj_proto + i) == ITEM_AMMO) {
        GET_AMMO_TYPE(obj_proto + i) = t[0];
        GET_AMMO_COUNT(obj_proto + i) = t[1];
    }
    else if (GET_OBJ_TYPE(obj_proto + i) == ITEM_KEY) {
        // keys need a timer, what else? -Lump
        GET_OBJ_TIMER(obj_proto + i) = t[4];
    }
	if (GET_OBJ_COMPOSITION(obj_proto + i) < 0 || GET_OBJ_COMPOSITION(obj_proto + i) > 7)
		GET_OBJ_COMPOSITION(obj_proto + i) = 0;
	if (GET_OBJ_SUBCOMPOSITION(obj_proto + i) < 0 || GET_OBJ_SUBCOMPOSITION(obj_proto + i) > 10)
		GET_OBJ_COMPOSITION(obj_proto + i) = 0;
	...
	GET_OBJ_COMPOSITION(obj_proto + i) = t[4];
	GET_OBJ_SUBCOMPOSITION(obj_proto + i) = t[5];
    ...
    // extra descriptions and apply fields
    for (j = 0; j < MAX_OBJ_APPLY; j++) {
        GET_OBJ_APPLY_LOC(obj_proto + i, j) = APPLY_NONE;
        GET_OBJ_APPLY_MOD(obj_proto + i, j) = 0;
//    obj_proto[i].affected[j].location = APPLY_NONE;
//    obj_proto[i].affected[j].modifier = 0;
    }

    strcat(buf2, ", after numeric constants\n" /* strcat: OK (for 'buf2 >= 87') */
        "...expecting 'E', 'A', 'T', 'R', 'S', '$', or next object number");

    j = 0;
    k = 0;

    obj_proto[i].obj_wpnpsi.which_psionic = 0;
    obj_proto[i].obj_wpnpsi.skill_level = 0;

    obj_proto[i].obj_flags.remort = 0;
    ...
    for (;;) {
        ...
        switch (*line) {
            ...
            case 'A':
                ...
                if (j >= MAX_OBJ_APPLY) {
                    log("SYSERR: Too many A fields (%d max), %s", MAX_OBJ_APPLY, buf2);
                    exit(1);
                }
                ...
                GET_OBJ_APPLY_LOC(obj_proto + i, j) = t[0];
                GET_OBJ_APPLY_MOD(obj_proto + i, j) = t[1];
                j++;
                break;
            ...
            case 'R':
                if (!get_line(obj_f, line)) {
                    log("SYSERR: Format error in 'R' field, %s\n"
                        "...expecting 1 numeric constant but file ended!", buf2);
                    exit(1);
                }

                if ((retval = sscanf(line, " %d", t)) != 1) {
                    log("SYSERR: Format error in 'R' field, %s\n"
                        "...expecting 1 numeric argument, got %d\n"
                        "...offending line: '%s'", buf2, retval, line);
                    exit(1);
                }

                obj_proto[i].obj_flags.remort = t[0];
                break;
            ...
            case 'S':
                ...
                obj_proto[i].obj_wpnpsi.which_psionic = t[0];
                obj_proto[i].obj_wpnpsi.skill_level = t[1];
                break;
...
static void load_zones(FILE *fl, char *zonename)
{
  ...
  while (get_line(fl, buf))
    if ((strchr("MOPGERDTLVB", buf[0]) && buf[1] == ' ') || (buf[0] == 'S' && buf[1] == '\0'))
      num_of_cmds++;
  ...
  line_num += get_line(fl, buf);
    // todo: convert zone flags to an array as with other flags
    if  (sscanf(buf, " %hd %hd %d %d %d %d", &Z.bot, &Z.top, &Z.lifespan, &Z.reset_mode, &Z.min_level, &Z.max_level) != 6) {
    ...
    if (!get_line(fl, buf)) {
        log("SYSERR: Expecting zoneflags of zone %s but file ended!", zname);
        exit(1);
    }

    else if (sscanf(buf, "%s %s %s %s", zbuf1, zbuf2, zbuf3, zbuf4) == 4) {
    ...
    sprintf(buf, "zone %s", zname);    /* sprintf: OK (until 399-bit integers) */

    for (taeller = 0; taeller < ZN_ARRAY_MAX; taeller++)
        check_bitvector_names(Z.zone_flags[taeller], zone_bits_count, buf, "zone");
...
//    } else if (ZCMD.command=='V') { /* a string-arg command */
	if (ZCMD.command=='V') {
		if (sscanf(ptr, " %d %d %d %d %d %79s %79[^\f\n\r\t\v]", &tmp, &ZCMD.arg1, &ZCMD.arg2, &ZCMD.arg3, &ZCMD.arg4, t1, t2) != 7)
			error = 1;
		else {
...
struct char_data *read_mobile(mob_vnum nr, int type) /* and mob_rnum */
{
  ...
  *mob = mob_proto[i];
  ...
  if (!mob->points.max_hit) {
    mob->points.max_hit = dice(mob->points.hit, mob->points.psi) +
      mob->points.move;
  } else
    mob->points.max_hit = rand_number(mob->points.hit, mob->points.psi);

  mob->points.hit = mob->points.max_hit;
  mob->points.psi = mob->points.max_psi;
...
void reset_zone(zone_rnum zone, bool full)
{
    int cmd_no;
    int last_cmd = 0;
    ...
    for (cmd_no = 0; ZCMD.command != 'S'; cmd_no++) {
        ...
        switch (ZCMD.command) {
            ...
            case 'O':            /* read an object */
                if (obj_index[ZCMD.arg1].number < ZCMD.arg2 && (full || ZCMD.arg4 > rand_number(0, 99))) {
                    if (ZCMD.arg3 != NOWHERE) {
            ...
            case 'G':            /* obj_to_char */
                ...
                if (obj_index[ZCMD.arg1].number < ZCMD.arg2 && (full || ZCMD.arg3 > rand_number(0, 99))) {
                    obj = read_object(ZCMD.arg1, REAL);
                    obj_to_char(obj, mob);
                    last_cmd = 1;
                    load_otrigger(obj);
...
/*            case 'L':
                if (!mob) {
                    ...
                    snprintf(error, sizeof(error), "trying to load gun of non-existant mob with obj #%d, command disabled", obj_index[ZCMD.arg1].vnum);
                    ZONE_ERROR(error);
                    ZCMD.command = '*';
                    break;
                }
                ...
                if (obj_index[ZCMD.arg1].number < ZCMD.arg2) {
                    ...
                    if (!mob->equipment[WEAR_WIELD]) {
                        snprintf(error, sizeof(error), "trying to load non-existant weapon of mob %s with obj #%d", GET_NAME(mob), obj_index[ZCMD.arg1].vnum);
                        ZONE_ERROR(error);
                        break;
                    }

                    if (!IS_GUN(mob->equipment[WEAR_WIELD])) {
                        snprintf(error, sizeof(error), "trying to load weapon %s of mob %s that is not a gun with obj #%d", mob->equipment[WEAR_WIELD]->short_description, GET_NAME(mob), obj_index[ZCMD.arg1].vnum);
                        ZONE_ERROR(error);
                        break;
                    }

                    if (!IS_AMMO(obj)) {
                        snprintf(error, sizeof(error), "trying to load gun of mob %s with obj #%d that is not ammo", GET_NAME(mob), obj_index[ZCMD.arg1].vnum);
                        ZONE_ERROR(error);
                        break;
                    }

                    if (IS_LOADED(mob->equipment[WEAR_WIELD])) {
                        last_cmd = 0;
                        break;
                    }

                    obj_to_obj(obj, mob->equipment[WEAR_WIELD]);
                    GET_AMMO_LOADED(mob->equipment[WEAR_WIELD]) = obj;
                    last_cmd = 1;
                }
                else
                    last_cmd = 0;
            ...
            case 'D':            /* set state of door */
                if (ZCMD.arg2 < 0 || ZCMD.arg2 >= NUM_OF_DIRS || (world[ZCMD.arg1].dir_option[ZCMD.arg2] == NULL)) {
                    ...
                    // doors could be reset by being blown up
                    SET_BIT(world[ZCMD.arg1].dir_option[ZCMD.arg2]->exit_info, EX_ISDOOR);
            ...
            case 'B':            /* install bionic device into mob */

                if (!mob) {
                    char error[MAX_STRING_LENGTH];
                    snprintf(error, sizeof(error), "trying to install bionic device into non-existant mob with obj #%d, command disabled", obj_index[ZCMD.arg1].vnum);
                    ZONE_ERROR(error);
                    ZCMD.command = '*';
                    break;
                }
                ...
                if (obj_index[ZCMD.arg1].number < ZCMD.arg2 && (full || ZCMD.arg4 > rand_number(0, 99))) {
                    ...
                    // is the body part location valid?
                    if (ZCMD.arg3 < 0 || ZCMD.arg3 >= NUM_BODY_PARTS) {
                        char error[MAX_STRING_LENGTH];
                        snprintf(error, sizeof(error), "invalid body part position (position #%d), command disabled", ZCMD.arg3);
                        ZONE_ERROR(error);
                        ZCMD.command = '*';

                    } else {
                        // is this a bionic device?
                        if (obj_proto[ZCMD.arg1].obj_flags.type_flag != ITEM_BIONIC_DEVICE) {
                            char error[MAX_STRING_LENGTH];
                            snprintf(error, sizeof(error), "trying to install an object not a bionic device (obj #%d), command disabled", ZCMD.arg1);
                            ZONE_ERROR(error);
                            ZCMD.command = '*';

                        } else if (obj_proto[ZCMD.arg1].obj_flags.value[0] != ZCMD.arg3) {
                            char error[MAX_STRING_LENGTH];
                            snprintf(error, sizeof(error), "trying to install bionic device (obj #%d) into incorrect body part (#%d, %s), command disabled", ZCMD.arg1, ZCMD.arg3, body_parts[ZCMD.arg3]);
                            ZONE_ERROR(error);
                            ZCMD.command = '*';
                        } else {
                            obj = read_object(ZCMD.arg1, REAL);
                            last_cmd = implant_bionic(mob, ZCMD.arg3, obj);
                            tobj = obj;
  ...
  if (ch->player_specials != NULL && ch->player_specials != &dummy_mob) {
    ...
    if (ch->player_specials->saved.completed_quests)
      free(ch->player_specials->saved.completed_quests);
        if (ch->player_specials->prompt_string)
            free(ch->player_specials->prompt_string);
    if (GET_HOST(ch))
      free(GET_HOST(ch));
    if (IS_NPC(ch))
...
void clear_char(struct char_data *ch)
{
  ...
  if (ch->points.max_psi < 100)
    ch->points.max_psi = 100;
}
...
void init_char(struct char_data *ch)
{
  ...
  for (i = 0; i < 3; i++)
    ...
    GET_COPYOVER_ROOM(ch) = NOWHERE;
    GET_SCREEN_WIDTH(ch) = PAGE_WIDTH;
    GET_NUM_DTS(ch) = 0;
    GET_NUM_DEATHS(ch) = 0;
}
void post_init_char(struct char_data *ch)
{
    ch->points.armor = 0;

    if (GET_CLASS(ch) == CLASS_BORG) {
        BIONIC_LEVEL(ch, BIO_INTERFACE) =  BIONIC_BASIC;
        BIONIC_LEVEL(ch, BIO_CHASSIS) = BIONIC_BASIC;
        BIONIC_LEVEL(ch, BIO_SPINE) = BIONIC_BASIC;
        BIONIC_LEVEL(ch, BIO_SHOULDER) = BIONIC_BASIC;
        BIONIC_LEVEL(ch, BIO_RIBCAGE) = BIONIC_BASIC;
        BIONIC_LEVEL(ch, BIO_HIP) = BIONIC_BASIC;
    }
    ...
    // Set new amounts to start at
    GET_MAX_PSI_PTS(ch) += (GET_INT(ch) * 10);
    GET_PSI(ch) = GET_MAX_PSI(ch);
    GET_MAX_HIT_PTS(ch) += (GET_CON(ch) * 10);
    GET_HIT(ch) = GET_MAX_HIT(ch);
    GET_MAX_MOVE(ch) += (GET_DEX(ch) * 20);
    GET_MOVE(ch) = GET_MAX_MOVE(ch);
}
...
static int check_object(struct obj_data *obj)
{
  ...
  case ITEM_POTION:
    ...
    error |= check_object_psionic_number(obj, 1);
    error |= check_object_psionic_number(obj, 2);
    error |= check_object_psionic_number(obj, 3);
    break;
  case ITEM_WAND:
  case ITEM_STAFF:
    error |= check_object_level(obj, 0);
...
static int check_object_psionic_number(struct obj_data *obj, int val)
{
  ...
  if (GET_OBJ_VAL(obj, val) < 0)
    error = TRUE;
    if (GET_OBJ_VAL(obj, val) > TOP_PSIONIC_DEFINE)
        error = TRUE;
    ...
    if (error)
        log("SYSERR: Object #%d (%s) has out of range psionic #%d.",
    ...
    // Now check for unnamed psionics
    psionicname = psionic_name(GET_OBJ_VAL(obj, val));

    if ((psionicname == unused_psiname || !str_cmp("UNDEFINED", psionicname)) && (error = TRUE))
        log("SYSERR: Object #%d (%s) uses '%s' psionic #%d.",
            GET_OBJ_VNUM(obj), obj->short_description, psionicname,
            GET_OBJ_VAL(obj, val));
...
void rainbow_boot(void)
{
    ...
    if (!(fl = fopen(RAINBOW_FILE, "r"))) {
        perror("Unable to open rainbow file");
        return;
    }

    if (!get_line(fl, line) || sscanf(line, "%d\n", &num) != 1) {
        fprintf(stderr, "Format error in rainbow file, setting tokens to 0!\n");
        num = 0;
    }
    ...
    // only load up to 50 tokens
    if (num > 50)
        num = 50;

    for (i = 0; i < num;i++)
        rainbow_token_load(0);
}


void rainbow_update(void)
{
    FILE *fl;

    if (!(fl = fopen(RAINBOW_FILE, "w"))) {
        mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Error opening rainbow token file");
        return;
    }
...
// This is the function to tally experience gains from players using skills and
// psionics.  This will be called by every skill/psionic in the game, and will
// determine if they should give the character experience, or not, based on how
// often the skill/psi is used.
void increment_skilluse(int skillid, int skilltype, int playerid, int exp, struct char_data *ch) {
    struct gain_data *glist = global_gain;
    while(glist) {
        if(glist->playerid == playerid) {
            update_skilluse(skillid, skilltype, glist, exp, ch);
            return;
        }
        glist = glist->next;
    } 
...
void update_skilluse(int skillid, int skilltype, struct gain_data *gdata, int exp, struct char_data *ch) {
    struct skill_data *slist = gdata->use;
    struct skill_data *curr = gdata->use;
    struct skill_data *tmpuse;
    bool found = FALSE;
   
    while(slist && !found){
        if(slist->skillid == skillid && slist->type == skilltype) {
			update_expgain(slist, ch, exp);
			slist->timesused++;
            found = TRUE;
        }
        curr = slist;
    ...
    if (found == FALSE) { 
		tmpuse = (struct skill_data*)malloc(sizeof(struct skill_data));
        tmpuse->skillid = skillid;
        tmpuse->type = skilltype;
        tmpuse->totalgained = 0;
		tmpuse->timesused = 1;
        tmpuse->next = NULL;

		update_expgain(tmpuse, ch, exp);
...
// This mini function just handles the exp it is received from the previous two
// functions and just makes sure that as you use your skill more and more, you
// gain less and less exp from it.  This is going to help against botting. - Gahan

int update_expgain(struct skill_data *slist, struct char_data *ch, int exp)
{
	int gain = 0;
	int maxxp;

	if (exp > 0 && !IS_NPC(ch)) {
		maxxp = exp_cap[GET_LEVEL(ch)].exp_cap / 10;
		if (exp > maxxp)
			exp = maxxp;

		if ((IS_HAPPYHOUR) && (IS_HAPPYEXP))
			exp += (int)((float)exp * ((float)HAPPY_EXP / (float)(100)));
    
		exp = exp * (1 - (slist->totalgained / (.020 * titles[GET_LEVEL(ch) + 1].exp) ));
	...
	if (exp > 0 && !IS_NPC(ch)) {
		slist->totalgained += exp;
	}
	exp = gain;
	return (gain);
}

// This function here is going to over time, increase the amount of exp you can gain from
// skills.  This is incase you cap your exp with one skill, it wont stay shitty exp for
// ever. - Gahan

void update_glist(void)
{
    struct gain_data *glist = global_gain;
    struct skill_data *slist = glist->use;

    while(glist) {
        slist = glist->use;
        while (slist) {

            if(slist->totalgained > 0) {
                slist->totalgained -= (titles[glist->playerlevel + 1].exp * 0.10);
				if (slist->totalgained < 0)
					slist->totalgained = 0;
            }
            slist = slist->next;
