...
static void qedit_setup_new(struct descriptor_data *d)
{
  ...
  quest->units_reward= 0;              /* Prize in resource units    */
...
static void qedit_disp_menu(struct descriptor_data *d)
{
  ...
  write_to_output(d,
    ...
    "\tg G\tn) Units          : [\tc%6d\tn] \tg T\tn) Exp Points  : [\tc%6d\tn] \tg O\tn) Object : [\tc%6d\tn]\r\n"
    ...
    quest->units_reward, quest->exp_reward, quest->obj_reward == NOTHING ? -1 : quest->obj_reward,
...
void qedit_parse(struct descriptor_data *d, char *arg)
{
 ...
 case 'g':
 case 'G':
   OLC_MODE(d) = QEDIT_UNITS;
   write_to_output(d, "Enter the number of units (0 for none) : ");
   break;
 case 't':
 case 'T':
 ...
 case 'P':
   ...
   break;
    ...
    case QEDIT_FLAGS:
      ...
      return;
    case QEDIT_QUANTITY:
      OLC_QUEST(d)->value[6] = LIMIT(number, 1, 1000);
      break;
    case QEDIT_POINTSCOMP:
      OLC_QUEST(d)->value[0] = LIMIT(number, 0, 5);
      break;
    case QEDIT_POINTSQUIT:
      OLC_QUEST(d)->value[1] = LIMIT(number, 0, 5);
      break;
    case QEDIT_PREREQ:
 ...
 write_to_output(d, "Enter maximum level to accept the quest : " );
    ...
    case QEDIT_TIMELIMIT:
      OLC_QUEST(d)->value[4] = LIMIT(number, -1, 500);
      break;
    ...
    case QEDIT_PREVQUEST:
      ...
      break;
    case QEDIT_UNITS:
      OLC_QUEST(d)->units_reward = LIMIT(number, 0, 2000000);
      break;
    case QEDIT_EXP:
      OLC_QUEST(d)->exp_reward = LIMIT(number, 0, 1000000);
      break;
    case QEDIT_OBJ:
      if ((number = atoi(arg)) != -1)
        if (real_object(number) == NOTHING) {
