...
#include "psionics.h" /* for damage types */
#include "custom.h"
#include "modify.h"
...
static OCMD(do_otransform)
{
  ...
  if (!*arg)
    ...
    if (obj->worn_by) {
      pos = obj->worn_on;
      wearer = obj->worn_by;
      unequip_char(obj->worn_by, pos, TRUE);
    }
    ...
    if (wearer) {
      equip_char(wearer, obj, pos, TRUE);
    }
    ...
    if (is_abbrev(arg1, "mob")) {
      ...
      if (tch) {
        if (arg2 != NULL && *arg2 && (pos = find_eq_pos_script(arg2)) >= 0 &&
          ...
          equip_char(tch, object, pos, TRUE);
          load_otrigger(object);
          return;
        }
      ...
      if (cnt && (!IS_CONTAINER(cnt) && !IS_TABLE(cnt))) {
...
static OCMD(do_odamage) {
  ...
  if (obj->worn_by)
    ...
    wearer = obj->carried_by;

  if (wearer) {
    // if we have an actor (worn or carried_ then bypass the script damage
    // this will give experience to the "actor" as requested by Warblade, implemented by Lump
    damage(wearer, ch, dam, TYPE_UNDEFINED, DMG_NORMAL);

    if (!IS_NPC(ch) && GET_POS(ch) == POS_DEAD) {
        mudlog( BRF, 0, TRUE, "%s killed by script at %s from object worn by %s",
          GET_NAME(ch), world[ch->in_room].name, GET_NAME(wearer));
    }
  } else
...
static OCMD(do_osetval)
{
  ...
  if (position>=0 && position<NUM_OBJ_VAL_POSITIONS +1) {
    worn_by = obj->worn_by;
    worn_on = obj->worn_on;
    ...
    if (worn_by != NULL) {
      equip_char(worn_by, obj, worn_on, TRUE);
    }
  } else
...
static OCMD(do_osetapply)
{
  ...
  int position, new_value, new_values, worn_on;
  struct char_data *worn_by = NULL;

 one_argument(two_arguments(argument, arg1, arg2), arg3);
  if (arg1 == NULL || !*arg1 || arg2 == NULL || !*arg2 || !is_number(arg1) || !is_number(arg2)) {
    obj_log(obj, "osetapply: bad syntax");
    return;
  }

  position = atoi(arg1);
  new_value = atoi(arg2);
  new_values = atoi(arg3);


  if (position>=-1 && position < MAX_OBJ_APPLY + 1 ) {
    worn_by = obj->worn_by;
    worn_on = obj->worn_on;
    ...
    GET_OBJ_APPLY_LOC(obj, position) = new_value;
    GET_OBJ_APPLY_MOD(obj, position) = new_values;
