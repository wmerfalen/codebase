...
void stop_follower(struct char_data *ch)
{
  ...
  if (AFF_FLAGGED(ch, AFF_CHARM)) {
    ...
    if (affected_by_psionic(ch, PSIONIC_CHARM))
      affect_from_char(ch, PSIONIC_CHARM, FALSE);
  } else {
...
{
    int room = NOWHERE;
    struct obj_data *attached = NULL;

    if (IN_ROOM(obj) != NOWHERE) room = IN_ROOM(obj);
    else if (obj->carried_by) room = IN_ROOM(obj->carried_by);
    else if (obj->worn_by) room = IN_ROOM(obj->worn_by);
    else if (obj->in_obj) {
        // the object is in another object
        room = get_obj_room(obj->in_obj);
    }
    else if (obj->implanted) room = IN_ROOM(obj->implanted);
    else if (GET_OBJ_TYPE(obj) == ITEM_EXPLOSIVE && GET_EXPLOSIVE_TYPE(obj) == EXPLOSIVE_PLASTIQUE) {

        // todo: we assume that plastique has no subtype so it is safe to use the second object value
        if (GET_OBJ_VAL(obj, 1) == PLASTIQUE_OBJ) {

            attached = obj->attached;

            if (!attached) {
                mudlog(NRM, LVL_IMMORT, TRUE, "ERROR: Plastique (%s) attached to object which is not in game.", obj->short_description);
                room = NOWHERE;
            }
            else
                room = get_obj_room(attached);

        }
        else if (GET_OBJ_VAL(obj, 1) >= PLASTIQUE_ROOM_NORTH && GET_OBJ_VAL(obj, 1) <= PLASTIQUE_ROOM_DOWN)
            room = GET_OBJ_VAL(obj, 5);
        else
            room = NOWHERE;
    }
    else {
        // we have a problem?
        mudlog(NRM, LVL_IMMORT, TRUE, "SYSERR: obj (%s) is in an unknown location!", obj->short_description);
        room = NOWHERE;
    }
...
int get_class_by_name(char *classname)
{
    ...
    for (i=0; i<NUM_CLASSES; i++)
      if (is_abbrev(classname, pc_class[i].class_name)) return(i);
