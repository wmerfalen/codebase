...
static int house_handle_obj(struct obj_data *temp, int locate, struct obj_data **cont_row, int vnum);
...
static int House_load(room_vnum vnum)
{
  ...
  for (current = loaded; current != NULL; current=current->next)
    num_objs += house_handle_obj(current->obj, current->locate, cont_row, vnum);
...
// Command to get back to your house (load room) when at recall
ACMD(do_home)
{
    int load_room;
    if (GET_LOADROOM(ch) != NOWHERE) {
      if (IN_ROOM(ch) == r_mortal_start_room) {
...
static int house_handle_obj(struct obj_data *temp, int locate, struct obj_data **cont_row, int vnum)
{
  ...
  if (!temp)  /* this should never happen, but.... */
    return FALSE;
  if (OBJ_FLAGGED(temp, ITEM_NORENT)) {
	extract_obj(temp);
	return FALSE;
  }
  obj_to_room(temp, real_room(vnum));

  if (locate > 0) { /* item equipped?? This should never happen, so lets just dump it on the floor */
	obj_to_room(temp, real_room(vnum));
	return (TRUE);
  } else { /* locate <= 0 */
    for (j = MAX_BAG_ROWS-1;j > -locate;j--)
      if (cont_row[j]) { /* no container -> back to house room */
        for (;cont_row[j];cont_row[j] = obj1) {
          obj1 = cont_row[j]->next_content;
          obj_to_room(cont_row[j], real_room(vnum));
        }
        cont_row[j] = NULL;
      }
    ...
    if (j == -locate && cont_row[j]) { /* content list existing */
      if (GET_OBJ_TYPE(temp) == ITEM_CONTAINER) {
        ...
        obj_to_room(temp, real_room(vnum)); /* add to room first ... */
