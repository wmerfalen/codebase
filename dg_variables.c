...
char *skill_percent(struct char_data *ch, char *skill)
{
  ...
  snprintf(retval, sizeof(retval), "%d", GET_SKILL_LEVEL(ch, skillnum));
  return retval;
}
...
int char_has_item_type(char *item, struct char_data *ch)
{
	struct obj_data *obj;
    ...
    for (i = 1; i < NUM_ITEM_FLAGS + 1; i++) {
        if (is_abbrev(item, item_types[i])) {
            type = TRUE;
			itemtype = i;
        }
    }
	if (type == TRUE && itemtype != -1) {
		for (i = 0; i < NUM_WEARS; i++)
			if (GET_EQ(ch, i))
				if (GET_OBJ_TYPE(GET_EQ(ch, i)) == itemtype)
					return 1;
		for (obj = ch->carrying; obj; obj = next_obj) {
			next_obj = obj->next_content;
			if (GET_OBJ_TYPE(obj) == itemtype)
				return 1;
			if (GET_OBJ_TYPE(obj) == ITEM_CONTAINER) {
//      if (IS_CONTAINER(obj) && IS_TABLE(obj))  {

				for (cont = obj->contains; cont; cont = next_item) {
					next_item = cont->next_content;
					if (GET_OBJ_TYPE(cont) == itemtype)
						return 1;
				}
			}
		}
...
static int handle_oset(struct obj_data * obj, char * argument) {
  ...
  struct oset_handler {
    ...
    { "actiondesc", oset_action_description},
    { "alias", oset_alias},
    { "names", oset_alias},
    { "apply", oset_apply},
    { "itemlevel", oset_itemlevel},
    { "remortlevel", oset_remortlevel},
    { "level", oset_itemlevel},
    { "remort", oset_remortlevel},
    { "wpnpsi", oset_weaponpsionic},
    { "weaponpsi", oset_weaponpsionic},
    { "weaponpsionic", oset_weaponpsionic},
    ...
    if (vd) {
      ...
      if (!str_cmp(var, "self")) {
        ...
        *str = '\0';
        ...
        else if (!str_cmp(field, "units") && IS_HAPPYHOUR)
          snprintf(str, slen, "%d", HAPPY_UNITS);
      ...
      switch (LOWER(*field)) {
        case 'a':
          if (!str_cmp(field, "affect")) {
            if (subfield && *subfield) {
              int psionic = find_skill_num(subfield);
              if (affected_by_psionic(c, psionic))
                strcpy(str, "1");
              else
                strcpy(str, "0");
            } else
        ...
        case 'c':
          if (!str_cmp(field, "canbeseen")) {
            ...
            if (subfield && *subfield) {
              ...
              GET_CHA_ABIL(c) += addition;
              if (GET_CHA_ABIL(c) > max) GET_CHA_ABIL(c) = max;
              if (GET_CHA_ABIL(c) < 3) GET_CHA_ABIL(c) = 3;
            }
            ...
            if (subfield && *subfield) {
              ...
              snprintf(str, slen, "%s", pc_class[GET_CLASS(c)].class_name);
          }
        ...
        case 'e':
          if (!str_cmp(field, "eq")) {
            ...
            if (subfield && *subfield) {
              int addition = MIN(atoi(subfield), 1000000);

              gain_exp(c, addition, FALSE);
            }
        ...
        case 'f':
		   if (!str_cmp(field, "fame")) {
			 ...
			 snprintf(str, slen, "%d", GET_FAME(c));
		   }
            ...
            if (subfield && *subfield) {
              int player = get_flag_by_name(player_bits, subfield);
              if (!IS_NPC(c) && player != NOFLAG && PLR_FLAGGED(c, player))
                 strcpy(str, "1");
              else
                 strcpy(str, "0");
            }
        ...
        case 'g':
          if (!str_cmp(field, "gold")) {
            if (subfield && *subfield) {
              int addition = atoi(subfield);
              GET_UNITS(c) += addition;
            }
            snprintf(str, slen, "%d", GET_UNITS(c));
          }
          break;
        case 'h':
          ...
          else if (!str_cmp(field, "has_item_type")) {
            if (!(subfield && *subfield))
              *str = '\0';
            else
              snprintf(str, slen, "%d", char_has_item_type(subfield, c));
          }
        ...
        case 'm':
          ...
          else if (!str_cmp(field, "maxpsi")) {
            if (subfield && *subfield) {
              int addition = atoi(subfield);
              GET_MAX_PSI_PTS(c) = MAX(GET_MAX_PSI_PTS(c) + addition, 1);
            }
            snprintf(str, slen, "%d", GET_MAX_PSI(c));
          }
        ...
        case 'p':
          ...
          else if (!str_cmp(field, "psi")) {
            if (subfield && *subfield) {
              int addition = atoi(subfield);
                GET_PSI(c) += addition;
            }
            snprintf(str, slen, "%d", GET_PSI(c));
		  }
		  else if (!str_cmp(field, "pcn")) {
            if (subfield && *subfield) {
              int addition = atoi(subfield);
                int max = (IS_NPC(c) || GET_LEVEL(c) >= LVL_GRGOD) ? 25 : 25;
                  GET_PCN(c) += addition;
                  if (GET_PCN(c) > max) GET_PCN(c) = max;
                    if (GET_PCN(c) < 3) GET_PCN(c) = 3;
             }
             snprintf(str, slen, "%d", GET_PCN(c));
           }
           break;
        case 'q':
          ...
          break;
                case 'r':
                    if (!str_cmp(field, "room")) {  /* in NOWHERE, return the void */
                        ...
                        snprintf(str, slen, "%c%ld",UID_CHAR, (IN_ROOM(c)!= NOWHERE) ? (long) world[IN_ROOM(c)].number + ROOM_ID_BASE : ROOM_ID_BASE);
#else
                        snprintf(str, slen, "%d", (IN_ROOM(c)!= NOWHERE) ? world[IN_ROOM(c)].number : 0);
#endif
                    }
                    else if (!str_cmp(field, "remorts")) {
                        snprintf(str, slen, "%d", GET_REMORTS(c));
}
                    break;
        case 's':
/*          if (!str_cmp(field, "saving_breath")) {
          ...
          if (!str_cmp(field, "sex"))
		  ...
		  else if (!str_cmp(field, "shardgreen")) {
			if (subfield && *subfield) {
			  int addition = atoi(subfield);
			  GET_GREEN_SHARDS(c) = MAX(1, GET_GREEN_SHARDS(c) + addition);
			}
			snprintf(str, slen, "%d", GET_GREEN_SHARDS(c));
		  }
		  else if (!str_cmp(field, "shardblue")) {
			if (subfield && *subfield) {
			  int addition = atoi(subfield);
			  GET_BLUE_SHARDS(c) = MAX(1, GET_BLUE_SHARDS(c) + addition);
			}
			snprintf(str, slen, "%d", GET_BLUE_SHARDS(c));
		  }
		  else if (!str_cmp(field, "shardblack")) {
			if (subfield && *subfield) {
			  int addition = atoi(subfield);
			  GET_BLACK_SHARDS(c) = MAX(1, GET_BLACK_SHARDS(c) + addition);
			}
			snprintf(str, slen, "%d", GET_BLACK_SHARDS(c));
		  }
            ...
            *str = '\0'; /* so the parser know we recognize 'skillset' as a field */
            ...
            if (GET_STR(c) >= 18) {
              if (subfield && *subfield) {
                int addition = atoi(subfield);
                GET_STR_ADD(c) += addition;
                if (GET_STR_ADD(c) > 100) GET_STR_ADD(c) = 100;
                if (GET_STR_ADD(c) < 0) GET_STR_ADD(c) = 0;
              }
              snprintf(str, slen, "%d", GET_STR_ADD(c));
            }
          }
          break;
        case 't':
      ...
      *str = '\x1';
      switch (LOWER(*field)) {
        case 'a':
          ...
          if (!str_cmp(field, "actiondesc"))
            snprintf(str, slen, "%s",  o->action_description);
          else if (!str_cmp(field, "affects")) {
            if (subfield && *subfield) {
	...
	case 'c':
        ...
        else if (!str_cmp(field, "comp")) {
            snprintf(str, slen, "%s", GET_ITEM_COMPOSITION(o));
}
...
//      if (IS_CONTAINER(o) && IS_TABLE(o)) 
              snprintf(str, slen, "%d", item_in_list(subfield,o->contains));
            else
            	strcpy(str, "0");
          }
          else if (!str_cmp(field, "counttable")) {
            if (GET_OBJ_TYPE(o) == ITEM_TABLE)
//      if (IS_CONTAINER(o) && IS_TABLE(o)) 
              snprintf(str, slen, "%d", item_in_list(subfield,o->contains));
            else
            	strcpy(str, "0");
	...
	case 'h':
        ...
        case 'i':
          ...
          else if (!str_cmp(field, "isin")) {
            if (o->in_obj)
            snprintf(str, slen, "%ld", GET_ID(o->in_obj));
            else
              *str = '\0';
          }
          else if (!str_cmp(field, "is_pc")) {
	  ...
	  break;
        case 'l':
          ...
          if (!str_cmp(field, "longdesc")) {
            snprintf(str, slen, "%s",  o->description);
        ...
        case 'r':
          if (!str_cmp(field, "room")) {
            ...
            snprintf(str, slen, "%d", GET_OBJ_REMORT(o));
          }
          break;
        case 's':
          if (!str_cmp(field, "shortdesc"))
            snprintf(str, slen, "%s",  o->short_description);
        else if (!str_cmp(field, "subcomp")) {
            snprintf(str, slen, "%s", GET_ITEM_SUBCOMPOSITION(o));
          }
          break;
        case 't':
