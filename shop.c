...
static void shopping_scan(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr);
...
static char *list_object(struct obj_data *obj, int cnt, int oindex, int shop_nr, struct char_data *keeper, struct char_data *ch);
...
static int add_to_list_shop(struct shop_buy_data *list, int type, int *len, int *val);
...
static int is_ok_char(struct char_data *keeper, struct char_data *ch, int shop_nr)
{
    ...
    if ((IS_MERCENARY(ch) && NOTRADE_MERCENARY(shop_nr)) ||
        (IS_CRAZY(ch) && NOTRADE_CRAZY(shop_nr)) ||
        (IS_STALKER(ch) && NOTRADE_STALKER(shop_nr)) ||
        (IS_CALLER(ch) && NOTRADE_CALLER(shop_nr)) ||
        (IS_BORG(ch) && NOTRADE_BORG(shop_nr)) ||
        (IS_HIGHLANDER(ch) && NOTRADE_HIGHLANDER(shop_nr)) ||
...
static int same_obj(struct obj_data *obj1, struct obj_data *obj2)
{
    ...
    for (index = 0; index < EF_ARRAY_MAX; index++)
        if (GET_OBJ_EXTRA_AR(obj1, index) != GET_OBJ_EXTRA_AR(obj2, index))
            return (FALSE);

    for (index = 0; index < MAX_OBJ_APPLY; index++)
        if ((GET_OBJ_APPLY_LOC(obj1, index) != GET_OBJ_APPLY_LOC(obj2, index)) ||
            (GET_OBJ_APPLY_MOD(obj1, index) != GET_OBJ_APPLY_MOD(obj2, index)))
            return (FALSE);
...
// todo: update this function to new code base
static void shopping_scan(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
...
static void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    if ((buynum = transaction_amt(arg)) < 0) {
        char buf[MAX_INPUT_LENGTH];
        snprintf(buf, sizeof(buf), "%s A negative amount?  Try selling me something.", GET_NAME(ch));
        do_tell(keeper, buf, cmd_tell, 0);
        return;
    }
    ...
    if (obj && OBJ_FLAGGED(obj, ITEM_QUEST_ITEM)) {
        if (GET_OBJ_COST(obj) > GET_QUESTPOINTS(ch) && !IS_GOD(ch)) {
            char actbuf[MAX_INPUT_LENGTH];
            snprintf(actbuf, sizeof(actbuf), "%s You haven't earned enough quest points for such an item.", GET_NAME(ch));
            do_tell(keeper, actbuf, cmd_tell, 0);
            return;
        }
    }
    else { // has the player got enough units?

        if (buy_price(obj, shop_nr, keeper, ch) > GET_UNITS(ch) && !IS_GOD(ch)) {
            char actbuf[MAX_INPUT_LENGTH];
    ...
    if (IS_CARRYING_TOT(ch) + GET_OBJ_WEIGHT(obj) > CAN_CARRY_W(ch)) {
    ...
    if (OBJ_FLAGGED(obj, ITEM_QUEST_ITEM)) {
        while (obj && (GET_QUESTPOINTS(ch) >= GET_OBJ_COST(obj) || IS_GOD(ch)) && IS_CARRYING_N(ch) < CAN_CARRY_N(ch) &&
            bought < buynum && IS_CARRYING_TOT(ch) + GET_OBJ_WEIGHT(obj) <= CAN_CARRY_W(ch)) {
        ...
        while (obj && (GET_UNITS(ch) >= buy_price(obj, shop_nr, keeper, ch) || IS_GOD(ch)) &&
            IS_CARRYING_N(ch) < CAN_CARRY_N(ch) && bought < buynum && IS_CARRYING_TOT(ch) + GET_OBJ_WEIGHT(obj) <= CAN_CARRY_W(ch)) {
                ...
                if (shop_producing(obj, shop_nr)) {
                    obj = read_object(GET_OBJ_RNUM(obj), REAL);
                //    log("shop produces the item");
                }
                else {
                    obj_from_char(obj);
                    SHOP_SORT(shop_nr)--;
                //    log("shop is not producing item");
                }
                ...
                if (!IS_GOD(ch))
                    GET_UNITS(ch) -= charged;

                last_obj = obj;
                //log("calling get_purchase_obj with arg: %s", arg);
                obj = get_purchase_obj(ch, arg, keeper, shop_nr, FALSE);
                //log("received obj: %s", obj ? obj->short_description : "NULL");
    ...
    // NB: at this point obj could be null if we wanted to buy more of an item than shop had
    // or, even worse, the player wanted 1 item and the second check made obj null
    // but if last_obj is not null, then it holds an item like what we bought
    ...
    if (bought < buynum) {
        ...
        else if (!OBJ_FLAGGED(obj, ITEM_QUEST_ITEM) && GET_UNITS(ch) < buy_price(obj, shop_nr, keeper, ch))
            snprintf(buf, sizeof(buf), "%s You can only afford %d.", GET_NAME(ch), bought);
        else if (OBJ_FLAGGED(obj, ITEM_QUEST_ITEM) && GET_QUESTPOINTS(ch) < GET_OBJ_COST(obj))
            snprintf(buf, sizeof(buf), "%s You only had sufficient quest points for %d.", GET_NAME(ch), bought);
        else if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
            snprintf(buf, sizeof(buf), "%s You can only hold %d.", GET_NAME(ch), bought);
        else if (IS_CARRYING_TOT(ch) + GET_OBJ_WEIGHT(obj) > CAN_CARRY_W(ch))
    ...
    // need to use last_obj in case obj is null
    if (!IS_GOD(ch) && obj && !OBJ_FLAGGED(last_obj, ITEM_QUEST_ITEM)) {

        GET_UNITS(keeper) += unitamt;

        if (SHOP_USES_BANK(shop_nr))
            if (GET_UNITS(keeper) > MAX_OUTSIDE_BANK) {
                SHOP_BANK(shop_nr) += (GET_UNITS(keeper) - MAX_OUTSIDE_BANK);
                GET_UNITS(keeper) = MAX_OUTSIDE_BANK;
            }
    }
    ...
    if (obj && OBJ_FLAGGED(last_obj, ITEM_QUEST_ITEM))
        snprintf(tempbuf, sizeof(tempbuf), "%s That has cost you %d quest points.", GET_NAME(ch), unitamt);
    else
        snprintf(tempbuf, sizeof(tempbuf), shop_index[shop_nr].message_buy, GET_NAME(ch), unitamt);
...
static struct obj_data *slide_obj(struct obj_data *obj, struct char_data *keeper, int shop_nr)
{
    ...
    // removed because this is what item stacking already does
...
static void shopping_sell(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
	...
	if (IS_BOUND(obj)) {
		send_to_char(ch, "You cannot sell bound equipment.\r\n");
		return;
	}
    if (!IS_SET(SHOP_BITVECTOR(shop_nr), HAS_UNLIMITED_CASH) && GET_UNITS(keeper) + SHOP_BANK(shop_nr) < sell_price(obj, shop_nr, keeper, ch)) {
        char buf[MAX_INPUT_LENGTH];
    ...
    while (obj && (IS_SET(SHOP_BITVECTOR(shop_nr), HAS_UNLIMITED_CASH) || GET_UNITS(keeper) + SHOP_BANK(shop_nr) >= sell_price(obj, shop_nr, keeper, ch)) && sold < sellnum) {

        int charged = sell_price(obj, shop_nr, keeper, ch);
        unitamt += charged;

        if (!IS_SET(SHOP_BITVECTOR(shop_nr), HAS_UNLIMITED_CASH))
            GET_UNITS(keeper) -= charged;
    ...
    if (sold < sellnum) {
        ...
        else if (GET_UNITS(keeper) + SHOP_BANK(shop_nr) < sell_price(obj, shop_nr, keeper, ch))
    ...
    GET_UNITS(ch) += unitamt;

    strlcpy(tempstr, times_message(0, name, sold), sizeof(tempstr));
    snprintf(tempbuf, sizeof(tempbuf), "$n sells %s.", tempstr);
    act(tempbuf, FALSE, ch, obj, 0, TO_ROOM);

    snprintf(tempbuf, sizeof(tempbuf), shop_index[shop_nr].message_sell, GET_NAME(ch), unitamt);
    do_tell(keeper, tempbuf, cmd_tell, 0);

    send_to_char(ch, "The shopkeeper now has %s.\r\n", tempstr);

    if (GET_UNITS(keeper) < MIN_OUTSIDE_BANK) {
        unitamt = MIN(MAX_OUTSIDE_BANK - GET_UNITS(keeper), SHOP_BANK(shop_nr));
        SHOP_BANK(shop_nr) -= unitamt;
        GET_UNITS(keeper) += unitamt;
    }
}
...
static void shopping_value(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    snprintf(buf, sizeof(buf), "%s I'll give you %d resource units for that!", GET_NAME(ch), sell_price(obj, shop_nr, keeper, ch));
    do_tell(keeper, buf, cmd_tell, 0);
}
...
static char *list_object(struct obj_data *obj, int cnt, int aindex, int shop_nr, struct char_data *keeper, struct char_data *ch)
{
    ...
    char specinfo[64];
    char itemname[128];
    char quantity[16];    /* "Unlimited" or "%d" */

    if (shop_producing(obj, shop_nr))
        strcpy(quantity, "Unlimited   ");    /* strcpy: OK (for 'quantity >= 10') */
    else
        sprintf(quantity, "%5d       ", cnt);    /* sprintf: OK (for 'quantity >= 11', 32-bit int) */
    ...
    strcpy(specinfo, "");
    if (OBJ_FLAGGED(obj, ITEM_MINLEV15))
        strcpy(specinfo, "@RMinlev: 15@n");

    if (OBJ_FLAGGED(obj, ITEM_MINLEV25))
        strcpy(specinfo, "@RMinlev: 25@n");

    if (OBJ_FLAGGED(obj, ITEM_REMORT_ONLY))
        sprintf(specinfo, "@RRemort: %d@n", obj->obj_flags.remort);

    snprintf(result, sizeof(result), " @C%2d) @G %9s@n   %-*s %-10s  @G%6d%s\r\n",
    aindex, quantity, count_color_chars(itemname)+48, itemname, specinfo,
    buy_price(obj, shop_nr, keeper, ch), OBJ_FLAGGED(obj, ITEM_QUEST_ITEM) ? " @Yqp@n" : " @Gru's@n");
...
static void shopping_list(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    one_argument(arg, name);
    send_to_char(ch, "@Y----------------------------------------------------------------------------------------------@n\r\n");
    len = strlcpy(buf,   "@C ##   Available      Item                                                           Cost@n\r\n"
        "@Y----------------------------------------------------------------------------------------------@n\r\n", sizeof(buf));
        ...
        for (obj = keeper->carrying; obj; obj = obj->next_content)
            if (CAN_SEE_OBJ(ch, obj) && GET_OBJ_COST(obj) > 0) {
                ...
                if (!*name || isname(name, last_obj->name)) {
                    ...
                    if (OBJ_FLAGGED(last_obj, ITEM_QUEST_ITEM))
                        has_quest = TRUE;
                }
    ...
    if (!last_obj)    /* we actually have nothing in our list for sale, period */
        ...
        page_string(ch->desc, buf, TRUE);
            send_to_char(ch, "@YTo see the stats of an item use \"shopscan #\". @YYou @Rmust@Y have the @Rgadgetry@Y skill.@n\r\n");
        if (has_quest)
            send_to_char(ch, "@YItems flagged \"qp\" require quest points to purchase.@n\r\n");
    }
}
...
SPECIAL(shop_keeper)
{
    ...
    else if (CMD_IS("shopscan") || CMD_IS("appraise")) {
        shopping_scan(argument, ch, keeper, shop_nr);
        return (TRUE);
    }
...
static int read_list(FILE *shop_f, struct shop_buy_data *list, int new_format, int max, int type)
{
    ...
    if (new_format) {
        for (;;) {
            ...
            error += add_to_list_shop(list, type, &len, &temp);
        }
...
static int read_type_list(FILE *shop_f, struct shop_buy_data *list, int new_format, int max)
{
    ...
    do {
        ...
        error += add_to_list_shop(list, LIST_TRADE, &len, &num);
...
static void list_all_shops(struct char_data *ch)
{
    ...
    int shop_nr;
    int headerlen = strlen(list_all_shops_header);
    size_t len = 0;
...
static void list_detailed_shop(struct char_data *ch, int shop_nr)
{
    ...
    if (SHOP_KEEPER(shop_nr) != NOBODY) {
        send_to_char(ch, "%s (#%d), Special Function: %s\r\n", GET_NAME(&mob_proto[SHOP_KEEPER(shop_nr)]), mob_index[SHOP_KEEPER(shop_nr)].vnum, YESNO(SHOP_FUNC(shop_nr)));

        if ((k = get_char_num(SHOP_KEEPER(shop_nr))))
            send_to_char(ch, "Coins:      [%9d], Bank: [%9d] (Total: %d)\r\n",
        GET_UNITS(k), SHOP_BANK(shop_nr), GET_UNITS(k) + SHOP_BANK(shop_nr));
    }
    else
    ...
    // todo: this is stupid.  Need a local buffer. */
    {
        char buf1[128];
