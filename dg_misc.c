...
#define PINFO psionics_info[psionicnum]
...
void do_dg_cast(void *go, struct script_data *sc, trig_data *trig, int type, char *cmd)
{
  ...
  s = strtok(cmd, "'");
  if (s == NULL) {
    script_log("Trigger: %s, VNum %d. dg_cast needs psionic name.",
      GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig));
    return;
  }
  s = strtok(NULL, "'");
  if (s == NULL) {
    script_log("Trigger: %s, VNum %d. dg_cast needs psionic name in `'s.",
      GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig));
    return;
  }
  t = strtok(NULL, "\0");
  ...
  psionicnum = find_psionic_num(s);
  if ((psionicnum < 1) || (psionicnum > TOP_PSIONIC_DEFINE)) {
    script_log("Trigger: %s, VNum %d. dg_cast: invalid psionic name (%s)",
  ...
  if (IS_SET(PINFO.targets, TAR_IGNORE)) {
    ...
    if (!target &&
          (IS_SET(PINFO.targets, TAR_CHAR_ROOM) ||
           IS_SET(PINFO.targets, TAR_CHAR_WORLD))) {
      if ((tch = get_char(t)) != NULL)
        target = TRUE;
    }

    if (!target &&
          (IS_SET(PINFO.targets, TAR_OBJ_INV) ||
           IS_SET(PINFO.targets, TAR_OBJ_EQUIP) ||
           IS_SET(PINFO.targets, TAR_OBJ_ROOM) ||
           IS_SET(PINFO.targets, TAR_OBJ_WORLD))) {
  ...
  if (!caster) {
    ...
    call_psionic(caster, tch, tobj, psionicnum, DG_PSIONIC_LEVEL, CAST_PSIONIC, TRUE);
    extract_char(caster);
  } else
    call_psionic(caster, tch, tobj, psionicnum, GET_LEVEL(caster), CAST_PSIONIC, TRUE);
}
		  ...
		  int script_type, char *cmd)
{
  ...
  if (!str_cmp(value_p, "off")) {
    affect_from_char(ch, PSIONIC_DG_AFFECT, TRUE);
    return;
  }

  /* add the affect */
  new_affect(&af);
  af.type = PSIONIC_DG_AFFECT;
