...
void zedit_disp_body_part_menu(struct descriptor_data *d);
  ...
  for (subcmd = 0; MYCMD.command != 'S'; subcmd++) {
    ...
    switch (MYCMD.command) {
      ...
      case 'E':
            case 'B':
                if (!mobloaded) {
                    write_to_output(d, "Equip/Give/Bionic command not saved since no mob was created first.\r\n");
                    continue;
                }
                break;
            case 'P':
                if (!objloaded) {
                    write_to_output(d, "Put command not saved since another object was not created first.\r\n");
                    continue;
                }
                break;
            case 'L':
                if (!objloaded) {
                    write_to_output(d, "Load command not saved since another object was not created first.\r\n");
                    continue;
                }
                break;
      /* Pass cases. */
...
static void zedit_disp_menu(struct descriptor_data *d)
{
  ...
  while (MYCMD.command != 'S') {
    ...
    switch (MYCMD.command) {
            case 'M':
                write_to_output(d, "%sLoad %s [%s%d%s], Max : %d, Freq. : %d",
                    MYCMD.if_flag ? " then " : "",
            ...
            case 'G':
                write_to_output(d, "%sGive it %s [%s%d%s], Max : %d, Freq. : %d",
                    MYCMD.if_flag ? " then " : "",
            ...
            case 'E':
                write_to_output(d, "%sEquip with %s [%s%d%s], %s, Max : %d, Freq. : %d",
                    MYCMD.if_flag ? " then " : "",
                    obj_proto[MYCMD.arg1].short_description,
                    cyn, obj_index[MYCMD.arg1].vnum, yel,
            ...
            case 'P':
                write_to_output(d, "%sPut %s [%s%d%s] in %s [%s%d%s], Max : %d, Freq. : %d",
                    MYCMD.if_flag ? " then " : "",
                    obj_proto[MYCMD.arg1].short_description,
                    cyn, obj_index[MYCMD.arg1].vnum, yel,
            ...
            case 'L':
                write_to_output(d, "%sLoad gun with %s [%s%d%s].",
                    MYCMD.if_flag ? " then " : "",
                    obj_proto[MYCMD.arg1].short_description,
                    cyn, obj_index[MYCMD.arg1].vnum, yel
                );
            ...
            case 'D':
                write_to_output(d, "%sSet door %s as %s.",
                    MYCMD.if_flag ? " then " : "",
                    dirs[MYCMD.arg2],
                    MYCMD.arg3 ? ((MYCMD.arg3 == 1) ? "closed" : ((MYCMD.arg3 == 2) ? "locked" : ((MYCMD.arg3 == 3) ? "closed (hidden)" : ((MYCMD.arg3 == 4) ? "locked (hidden)" : ((MYCMD.arg3 == 5) ? "open (hidden)" : "open"))))) : "open"
                );
                break;
            ...
            case 'B':
                write_to_output(d, "%sInstall Bionic %s [%s%d%s] in %s, Max : %d, Chance : %d",
                    MYCMD.if_flag ? " then " : "",
                    obj_proto[MYCMD.arg1].short_description,
                    cyn, obj_index[MYCMD.arg1].vnum, yel,
                    body_parts[MYCMD.arg3],
                    MYCMD.arg2, MYCMD.arg4
                );
                break;
...
static void zedit_disp_comtype(struct descriptor_data *d)
{
  ...
  clear_screen(d);
    write_to_output(d,
        ...
        "%sR%s) Remove an object from the room  %sL%s) Load a mob's gun\r\n"
        "%sT%s) Assign a trigger                %sV%s) Set a global variable\r\n"
        "%sB%s) Implant a bionic\r\n"
        "\r\n"
    ...
    switch (OLC_CMD(d).command) {
        ...
        case 'D':
            write_to_output(d,     "0)  Door open\r\n"
                ...
                "4)  Door locked (hidden)\r\n"
				"5)  Door open (hidden)\r\n"
				"Enter state of the door : ");
            break;
        case 'G':
            write_to_output(d, "Frequency to happen (100 = always) : ");
            break;
        case 'B':
            zedit_disp_body_part_menu(d);
            break;
        case 'V':
        case 'T':
        case 'M':
...
static void zedit_disp_arg4(struct descriptor_data *d)
{
    ...
    switch (OLC_CMD(d).command) {
        ...
        case 'P':
        case 'B':
            write_to_output(d, "Frequency to happen (0 - only on reboot, 100 - always) : ");
            break;
        case 'G':
        case 'D':
        case 'R':
	...
	// Parse the input for arg1, and goto next quiz
            ...
            switch (OLC_CMD(d).command) {
                ...
                case 'B':
                    ...
                    if ((pos = real_object(atoi(arg))) == NOTHING)
                        ...
                        if (obj_proto[pos].obj_flags.type_flag == ITEM_BIONIC_DEVICE) {
                            OLC_CMD(d).arg1 = pos;
                            zedit_disp_arg2(d);
                        }
                        else
                            write_to_output(d, "That object is not a bionic device, try again (0 to quit): ");
                    }
                    break;
        ...
        case ZEDIT_ARG3:
            ...
            switch (OLC_CMD(d).command) {
                ...
                case 'E':
                    pos = atoi(arg);
                    // Count number of wear positions.  We could use NUM_WEARS, this is more reliable
                    while (*equipment_types[i] != '\n')
                        i++;
                ...
                case 'G':
                    freq = atoi(arg);
                    if ((freq < 0) || (freq > 100))
                        write_to_output(d, "Try again (from 0 to 100) : ");
                    else {
                        OLC_CMD(d).arg3 = freq;
                        zedit_disp_menu(d);
                    }
                ...
                case 'B':
                    ...
                    // check for valid body part
                    if ((pos <= 0) || (pos >= NUM_BODY_PARTS))
                        write_to_output(d, "Invalid Body part, Try again : ");
                    else {
                        OLC_CMD(d).arg3 = pos;
                        zedit_disp_arg4(d);
                    }
        ...
        case ZEDIT_TIME_OFFSET:
            pos = atoi(arg);
            if ((pos < -100) || (pos > 100))
                write_to_output(d, "Try again (-100 to +100) : ");
            else {
                OLC_ZONE(d)->time_offset = pos;
                OLC_ZONE(d)->number = 1;
                zedit_disp_menu(d);
            }
            break;
