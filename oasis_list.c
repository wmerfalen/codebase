...
void perform_obj_aff_list(struct char_data * ch, char *arg)
{
  ...
  if (!(apply>0 && apply<NUM_APPLIES) ){
  ...
  else if ((apply == APPLY_SKILL) ||  /* olist affect 7 is Weapon Damage      */
           (apply == APPLY_PSI_MASTERY) ) { /* olist affect 8 is AC-Apply for Armor */
    for (num=0;num<=top_of_objt;num++) {
      if ((apply == APPLY_SKILL && obj_proto[num].obj_flags.type_flag == ITEM_WEAPON) ||
          (apply == APPLY_PSI_MASTERY && obj_proto[num].obj_flags.type_flag == ITEM_ARMOR) ) {
        ov = obj_index[num].vnum;
    ...
    else if (apply == APPLY_PSI_MASTERY)
      ...
      if (is_abbrev(arg, "help")) {
        ...
        if (is_abbrev(arg, "type")) {
          ...
          if (!*arg2) {
            ...
            {
              if (i == APPLY_SKILL)       /* Special Case 1 - Weapon Dam */
                send_to_char(ch, "%s%2d-%s%-14s%s", QNRM, i, QYEL, "Weapon Dam", QNRM);
              else if (i == APPLY_PSI_MASTERY)  /* Special Case 2 - Armor AC Apply */
