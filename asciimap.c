...
{
  { SECT_INSIDE,       "@c[@G.@c]@n" }, /* 0 */
  { SECT_CITY,         "@c[@WC@c]@n" },
  { SECT_FIELD,        "@c[@G,@c]@n" },
  { SECT_FOREST,       "@c[@GY@c]@n" },
  { SECT_HILLS,        "@c[@Mm@c]@n" },
  { SECT_MOUNTAIN,     "@c[@yM@c]@n" }, /* 5 */
  { SECT_WATER_SWIM,   "@c[@B~@c]@n" },
  { SECT_WATER_NOSWIM, "@c[@b=@c]@n" },
  { SECT_UNDERWATER,   "@c[@BU@c]@n" },
  { SECT_FLYING,       "@c[@C^@c]@n" },
  { SECT_PAUSE,        "@c[@C^@c]@n" }, /* 10 */
  { SECT_DESERT,       "@c[@C^@c]@n" },
  { SECT_DEATH,        "@c[@RX@c]@n" },
  { SECT_QUEST,        "@c[@Y!@c]@n" },
  { SECT_SHOP,         "@c[@G$@c]@n" },
  { SECT_UNDERGROUND,  "@c[@DU@c]@n" }, /* 15 */
  { -1,                ""        },
  { -1,                ""        },
  { -1,                ""        },
  { -1,                ""        },
...
static void perform_map( struct char_data *ch, char *argument, bool worldmap )
{
  ...
  if (IN_VEHICLE(ch))
	MapArea(WINDOW_VNUM(ch), ch, centre, centre, min, max, ns_size/2, ew_size/2, worldmap);
  ...
  send_to_char(ch, " @Y-@yCyberASSAULT Map System@Y-@n\r\n"
                   "@D  .-.__--.,--.__.-.@n\r\n" );
    ...
    count += sprintf(buf + count, "@n%s Pause\\\\", map_info[SECT_PAUSE].disp);
