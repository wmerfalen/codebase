...
static int update_all_objects(struct obj_data *refobj)
{
  ...
  for (obj = object_list; obj; obj = obj->next) {
    ...
    *obj = *refobj;
        ...
        obj->attached = swap.attached;
        obj->plastique = swap.plastique;
        obj->user = swap.user;
        obj->bionics = swap.bionics;
        obj->implanted = swap.implanted;
        // todo: do we need to do this here? is this a game-time dependent variable?
        obj->obj_flags.stamp = swap.obj_flags.stamp;  /* make sure object keeps it's stamp */
        obj->obj_flags.remort = swap.obj_flags.remort; 
        obj->obj_flags.level = swap.obj_flags.level; 
     obj->obj_flags.applies[0] = swap.obj_flags.applies[0];
     obj->obj_flags.applies[1] = swap.obj_flags.applies[1];
     obj->obj_flags.applies[2] = swap.obj_flags.applies[2];
     obj->obj_flags.applies[3] = swap.obj_flags.applies[3];
     obj->obj_flags.applies[4] = swap.obj_flags.applies[4];
//        obj->affected.location = swap.affected.location;
//        obj->affected.modifier = swap.affected.modifier;
        // with ammo in the game, we must maintain ammo counts that can change during the course of the game
        if (swap.obj_flags.type_flag == ITEM_AMMO) {
            //obj->obj_ammo.ammo_type = swap.obj_flags.value[0];
            obj->obj_ammo.ammo_count = swap.obj_flags.value[1];
        }
        else if (swap.obj_flags.type_flag == ITEM_GUN)
            obj->obj_weapon.ammo_loaded = swap.obj_weapon.ammo_loaded;

        // todo:  there are far more game-dependent things than this
    }
...
int save_objects(zone_rnum zone_num)
{
  ...
  for (counter = genolc_zone_bottom(zone_num); counter <= zone_table[zone_num].top; counter++) {
	...
	*buf = '\0';
            // I hope this fixes guns and ammo refreshing their current value when being edit -Lump
            if (GET_OBJ_TYPE(obj) == ITEM_GUN) {
                obj->obj_weapon.gun_type = obj->obj_flags.value[0];
                GET_SHOTS_PER_ROUND(obj) = obj->obj_flags.value[3];
                GET_RANGE(obj) = obj->obj_flags.value[4];
                obj->obj_weapon.ammo_loaded = NULL;
                GET_GUN_BASH_NUM(obj) = 1;
                GET_GUN_BASH_SIZE(obj) = obj->obj_flags.value[5];
            }
            else if (obj->obj_flags.type_flag == ITEM_AMMO) {
                obj->obj_ammo.ammo_type = obj->obj_flags.value[0];
                obj->obj_ammo.ammo_count = obj->obj_flags.value[1];
            }
            else if (obj->obj_flags.type_flag == ITEM_KEY) {
                // hope this fixes timer too!
                obj->obj_flags.timer = obj->obj_flags.value[4];
            }
			if (GET_OBJ_COMPOSITION(obj) < 0 || GET_OBJ_COMPOSITION(obj) > 7)
				GET_OBJ_COMPOSITION(obj) = 0;
			if (GET_OBJ_SUBCOMPOSITION(obj) < 0 || GET_OBJ_SUBCOMPOSITION(obj) > 10)
				GET_OBJ_COMPOSITION(obj) = 0;
            fprintf(fp,
                "#%d\n"
                "%s~\n"
            ...
            fprintf(fp, "%d %s %s %s %s %s %s %s %s %s %s %s %s\n"
                ...
                GET_OBJ_VAL(obj, 4), GET_OBJ_VAL(obj, 5),
                GET_OBJ_WEIGHT(obj), GET_OBJ_COST(obj),
                GET_OBJ_RENT(obj), GET_OBJ_LEVEL(obj),
				GET_OBJ_COMPOSITION(obj), GET_OBJ_SUBCOMPOSITION(obj)
                );
      ...
      script_save_to_disk(fp, obj, OBJ_TRIGGER);
            ...
            for (counter2 = 0; counter2 < MAX_OBJ_APPLY; counter2++)
                if (GET_OBJ_APPLY_MOD(obj, counter2))
                    fprintf(fp, "A\n%d %d\n", GET_OBJ_APPLY_LOC(obj, counter2), GET_OBJ_APPLY_MOD(obj, counter2));

            // Do we have a weapon psionic?
            if (obj->obj_wpnpsi.which_psionic > 0 && obj->obj_wpnpsi.skill_level > 0)
                fprintf(fp, "S\n%d %d\n", obj->obj_wpnpsi.which_psionic, obj->obj_wpnpsi.skill_level);

            // Do we have a remort level restriction?
            if (obj->obj_flags.remort > 0)
                fprintf(fp, "R\n%d\n", obj->obj_flags.remort);

            // Gun and Ammo saved as values
        }
    }
