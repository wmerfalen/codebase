...
/* local functions */
static int VALID_EDGE(room_rnum x, int y, struct char_data *ch, bool vehicle);
...
static int VALID_EDGE(room_rnum x, int y, struct char_data *ch, bool vehicle)
{
  ...
  if (vehicle && vhcle) {
    if (!vehicle_terrain_check(vhcle, TOROOM(x, y)) || IS_MARKED(TOROOM(x, y)))
	return 0;
  }
...
int find_first_step(room_rnum src, room_rnum target, struct char_data *ch, bool vehicle)
{
  int curr_dir;
  room_rnum curr_room;
  ...
  for (curr_dir = 0; curr_dir < DIR_COUNT; curr_dir++)
    if (VALID_EDGE(src, curr_dir, ch, vehicle)) {
  ...
  while (queue_head) {
    if (queue_head->room == target) {
      ...
      for (curr_dir = 0; curr_dir < DIR_COUNT; curr_dir++)
	if (VALID_EDGE(queue_head->room, curr_dir, ch, vehicle)) {
...
ACMD(do_track)
{
    ...
    if (IS_NPC(ch) || !GET_SKILL_LEVEL(ch, SKILL_TRACK)) {
    ...
    dir = find_first_step(IN_ROOM(ch), IN_ROOM(vict), ch, FALSE);
    ...
    switch (dir) {
        ...
        default:
            ...
            percent_miss = rand_number(0, 101);  /* 101 is a complete failure, no matter what the proficiency. */
            if (percent_miss > 100) {
                ...
                // we either found a random direction or we didnt
                if (tries)
...
void fun_hunt(struct char_data *ch)
{
	int dir;

	if (!ch || !FUN_HUNTING(ch) || FIGHTING(ch))
		return;
 
	if ((dir = find_first_step(IN_ROOM(ch), IN_ROOM(FUN_HUNTING(ch)), ch, FALSE)) < 0) {
		FUN_HUNTING(ch) = NULL;
		return;
	} 
	else {
...
void hunt_victim(struct char_data *ch)
{
  ...
  if (!found) {
    char actbuf[MAX_INPUT_LENGTH] = "Damn!  I lost them!!";
  ...
  if ((world[IN_ROOM(ch)].zone != world[IN_ROOM(HUNTING(ch))].zone) && ch->char_specials.targeted == NULL) {
	char buf[MAX_INPUT_LENGTH];
	snprintf(buf, sizeof(buf), "Get back here %s, you coward!!", GET_NAME(HUNTING(ch)));
	do_say(ch, buf, 0, 0);
	HUNTING(ch) = NULL;
	return;
  }
  
  if ((dir = find_first_step(IN_ROOM(ch), IN_ROOM(HUNTING(ch)), ch, FALSE)) < 0) {
    char buf[MAX_INPUT_LENGTH];
    ...
    if (IN_ROOM(ch) == IN_ROOM(HUNTING(ch))) {
		...
		hit(ch, HUNTING(ch), TYPE_UNDEFINED, thaco, 0);
	}
  }
}
