...
static void look_on_obj(struct char_data *ch, char *arg);
...
void display_missing_body_parts(struct char_data *i, struct char_data *ch)
{
    const char *hssh = HSSH(i);
    const char *hshr = HSHR(i);

    // display the missing body parts (arms, hands, legs, feet, ears, eyes)
    if (PART_MISSING(i, BODY_PART_LEFT_ARM) && PART_MISSING(i, BODY_PART_RIGHT_ARM))
        send_to_char(ch, "%c%s is missing both arms.\r\n", UPPER(*hssh), hssh + 1);
    else if (PART_MISSING(i, BODY_PART_LEFT_ARM))
        send_to_char(ch, "%c%s is missing %s left arm.\r\n", UPPER(*hssh), hssh + 1, hshr);
    else if (PART_MISSING(i, BODY_PART_RIGHT_ARM))
        send_to_char(ch, "%c%s is missing %s right arm.\r\n", UPPER(*hssh), hssh + 1, hshr);
    else if (PART_MISSING(i, BODY_PART_LEFT_HAND) && PART_MISSING(i, BODY_PART_RIGHT_HAND))
        send_to_char(ch, "%c%s is missing both hands.\r\n", UPPER(*hssh), hssh + 1);
    else if (PART_MISSING(i, BODY_PART_LEFT_HAND))
        send_to_char(ch, "%c%s is missing %s left hand.\r\n", UPPER(*hssh), hssh + 1, hshr);
    else if (PART_MISSING(i, BODY_PART_RIGHT_HAND))
        send_to_char(ch, "%c%s is missing %s right hand.\r\n", UPPER(*hssh), hssh + 1, hshr);

    if (PART_MISSING(i, BODY_PART_LEFT_LEG) && PART_MISSING(i, BODY_PART_RIGHT_LEG))
        send_to_char(ch, "%c%s is missing both legs.\r\n", UPPER(*hssh), hssh + 1);
    else if (PART_MISSING(i, BODY_PART_LEFT_LEG))
        send_to_char(ch, "%c%s is missing %s left leg.\r\n", UPPER(*hssh), hssh + 1, hshr);
    else if (PART_MISSING(i, BODY_PART_RIGHT_LEG))
        send_to_char(ch, "%c%s is missing %s right leg.\r\n", UPPER(*hssh), hssh + 1, hshr);
    else if (PART_MISSING(i, BODY_PART_LEFT_FOOT) && PART_MISSING(i, BODY_PART_RIGHT_FOOT))
        send_to_char(ch, "%c%s is missing both feet.\r\n", UPPER(*hssh), hssh + 1);
    else if (PART_MISSING(i, BODY_PART_LEFT_FOOT))
        send_to_char(ch, "%c%s is missing %s left foot.\r\n", UPPER(*hssh), hssh + 1, hshr);
    else if (PART_MISSING(i, BODY_PART_RIGHT_FOOT))
        send_to_char(ch, "%c%s is missing %s right foot.\r\n", UPPER(*hssh), hssh + 1, hshr);

    if (PART_MISSING(i, BODY_PART_LEFT_WRIST) && PART_MISSING(i, BODY_PART_RIGHT_WRIST))
        send_to_char(ch, "%c%s is missing both wrists.\r\n", UPPER(*hssh), hssh + 1);
    else if (PART_MISSING(i, BODY_PART_LEFT_WRIST))
        send_to_char(ch, "%c%s is missing %s left wrist.\r\n", UPPER(*hssh), hssh + 1, hshr);
    else if (PART_MISSING(i, BODY_PART_RIGHT_WRIST))
        send_to_char(ch, "%c%s is missing %s right wrist.\r\n", UPPER(*hssh), hssh + 1, hshr);

    if (PART_MISSING(i, BODY_PART_LEFT_EYE) && PART_MISSING(i, BODY_PART_RIGHT_EYE))
        send_to_char(ch, "%c%s is missing both eyes.\r\n", UPPER(*hssh), hssh + 1);
    else if (PART_MISSING(i, BODY_PART_LEFT_EYE))
        send_to_char(ch, "%c%s is missing %s left eye.\r\n", UPPER(*hssh), hssh + 1, hshr);
    else if (PART_MISSING(i, BODY_PART_RIGHT_EYE))
        send_to_char(ch, "%c%s is missing %s right eye.\r\n", UPPER(*hssh), hssh + 1, hshr);
...
static void show_obj_to_char(struct obj_data *obj, struct char_data *ch, int mode)
{
  ...
  case SHOW_OBJ_LONG:
	...
	if (obj->bound_id == GET_IDNUM(ch))
		send_to_char(ch, "%s @Y(Bound to you)@n", obj->description);
	else if (OBJ_FLAGGED(obj, ITEM_BOP))
		send_to_char(ch, "%s @Y(Binding)@n", obj->description);
	else
  ...
  case SHOW_OBJ_SHORT:
	...
	if (obj->bound_id == GET_IDNUM(ch))
		send_to_char(ch, "%s @Y(Bound to you)@n", obj->short_description);
	else if (OBJ_FLAGGED(obj, ITEM_BOP))
		send_to_char(ch, "%s @Y(Binding)@n", obj->short_description);
    else 
...
static void show_obj_modifiers(struct obj_data *obj, struct char_data *ch)
{
    ...
    if (OBJ_FLAGGED(obj, ITEM_PSIONIC) && AFF_FLAGGED(ch, AFF_DETECT_PSIONIC))
        send_to_char(ch, " (psionic aura)");
...
void list_obj_to_char(struct obj_data *list, struct char_data *ch, int mode, int show)
{
    ...
    if (num > 0 && (mode != SHOW_OBJ_LONG || *display->description != '.' ||
      ...
      if (mode == SHOW_OBJ_LONG)	  
        send_to_char(ch, "%s", CCGRN(ch, C_NRM));
      if (num != 1)
        send_to_char(ch, "(%3i) ", num);
	  if (OBJ_IS_HIDDEN(display))
...
void diag_char_to_char(struct char_data *i, struct char_data *ch)
{
  struct {
    ...
    {  -1, "is bleeding awfully from their wounds."	},
  };
  ...
  send_to_char(ch, "%c%s %s", UPPER(*pers), pers + 1, diagnosis[ar_index].text);
}
...
void look_at_char(struct char_data *i, struct char_data *ch)
{
    ...
    display_missing_body_parts(i, ch);
    ...
    if (found) {
        ...
        for (j = 0; j < NUM_WEARS; j++)
            if (GET_EQ(i, wear_order_index[j]) && CAN_SEE_OBJ(ch, GET_EQ(i, wear_order_index[j]))) {
                send_to_char(ch, "%s", wear_where[wear_order_index[j]]);
                show_obj_to_char(GET_EQ(i, wear_order_index[j]), ch, SHOW_OBJ_SHORT);
            }
    }
	if (!IS_NPC(i) && (PRF_FLAGGED(i, PRF_AFK) || PRF_FLAGGED(i, PRF_FROZEN) || !i->desc)) {

		send_to_char(ch, "\r\n");

		if (!IS_NPC(i) && PRF_FLAGGED(i, PRF_AFK))
			send_to_char(ch, "@B%s is away from their keyboard.@n\r\n", GET_NAME(i));

		if (!IS_NPC(i) && PRF_FLAGGED(i, PRF_FROZEN))
			send_to_char(ch, "@B%s is frozen solid, and cannot move.@n\r\n", GET_NAME(i));

		if (!IS_NPC(i) && !i->desc)
			send_to_char(ch, "@B%s is link-dead.@n\r\n", GET_NAME(i));
	
	}

	if (IS_NPC(i) && MOB_FLAGGED(i, MOB_THIEF)) {
		send_to_char(ch, "\r\n");
		send_to_char(ch, "@R%s is a thief!@n\r\n", GET_NAME(i));
	}

    // display bionics
    found = FALSE;
    for (j = 1; !found && j < NUM_BODY_PARTS; j++)
        if (BIONIC_DEVICE(i, j) && CAN_SEE_OBJ(ch, BIONIC_DEVICE(i, j)))
            found = TRUE;

    if (found) {

        send_to_char(ch, "\r\n");    /* act() does capitalization. */
        act("$n has implanted:", FALSE, i, 0, ch, TO_VICT);

        for (j = 1; j < NUM_BODY_PARTS; j++)
            if (BIONIC_DEVICE(i, j) && CAN_SEE_OBJ(ch, BIONIC_DEVICE(i, j))) {
                send_to_char(ch, "%s", where_body_parts[j]);
                show_obj_to_char(BIONIC_DEVICE(i, j), ch, SHOW_OBJ_SHORT);
            }
    }
	// Gahantodo: Reimplement this after conversion
    // todo: something is very odd in the following code, check on conversion -- Tails
    if (ch != i && ((GET_CLASS(ch) == CLASS_PREDATOR && GET_REMORTS(ch) > 5)
        || GET_CLASS(ch) == CLASS_STALKER || GET_LEVEL(ch) >= LVL_IMMORT)) {
...
static void list_one_char(struct char_data *i, struct char_data *ch)
{
	...
	if (LOCKED(ch)) {
		if (i == LOCKED(ch))
			send_to_char(ch, "@R(TARGET) ");
	}
	if (AFF_FLAGGED(i, AFF_HIDE) && IS_NPC(i) && GET_LEVEL(ch) >= LVL_IMMORT)
		send_to_char(ch, "(hidden) ");	
	if (!IS_NPC(i) && PLR_FLAGGED(i, PLR_WRITING))
		send_to_char(ch, "@B%s is writing, and is currently busy.@n\r\n", GET_NAME(i));
    
	if (!IS_NPC(i) && PRF_FLAGGED(i, PRF_FROZEN))
        send_to_char(ch, " @1(FROZEN)@n");
    
	if (IS_NPC(i) && GET_MOB_SPEC(i) == questmaster)
        send_to_char(ch, "@y(@YQM@y)@Y ");

    if (!IS_NPC(i) && affected_by_psionic(i, PSIONIC_SANCTUARY))
		send_to_char(ch, "@W");

	else if (!IS_NPC(i) && affected_by_psionic(i, PSIONIC_SUPER_SANCT))
        send_to_char(ch, "@D");
    
	else if (!IS_NPC(i) && affected_by_psionic(i, PSIONIC_MEGA_SANCT))
        send_to_char(ch, "@Y");
	...
	if (IS_NPC(i) && i->player.long_descr && GET_POS(i) == GET_DEFAULT_POS(i))
		send_to_char(ch, "%c%s", UPPER(*i->player.long_descr), i->player.long_descr + 1);
		...
		if (IS_NPC(i)) {
			...
			if (GET_POS(i) != POS_FIGHTING) {
				if (!SITTING(i))
					...
					furniture = SITTING(i);
					send_to_char(ch, " is %s upon %s", (GET_POS(i) == POS_SLEEPING ? "sleeping" : (GET_POS(i) == POS_RESTING ? "resting" : "sitting")), OBJS(furniture, ch));
				}
			}
			else {
				if (FIGHTING(i)) {
			...
			if (affected_by_psionic(i, PSIONIC_SANCTUARY))
				send_to_char(ch, ", surrounded by a white aura");
		    else if (affected_by_psionic(i, PSIONIC_SUPER_SANCT))
			    send_to_char(ch, ", surrounded by a silver aura");
		    else if (affected_by_psionic(i, PSIONIC_MEGA_SANCT))
			    send_to_char(ch, ", surrounded by a golden aura");
            send_to_char(ch, ".@n\r\n");
            if (AFF_FLAGGED(i, AFF_PSI_MIRROR)) {
                send_to_char(ch, "@g%s is here shrouded in mirrors.@n\r\n", i->player.name);
            }
		}
	}
}
...
void list_char_to_char(struct char_data *list, struct char_data *ch)
{
	...
	for (i = list; i; i = i->next_in_room)
		if (ch != i) {
			...
			if (!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_HOLYLIGHT) && IS_NPC(i) && i->player.long_descr && *i->player.long_descr == '.')
				continue;
            ...
            if (CAN_SEE(ch, i)) {
                ...
                list_one_char(i, ch);
                send_to_char(ch, "%s@n", CCNRM(ch, C_NRM));
            }
			else if (AFF_FLAGGED(ch, AFF_SENSE_LIFE) && AFF_FLAGGED(i, AFF_HIDE))
				send_to_char(ch, "@RYou sense a life form present in the room.@n\r\n");
			else if (IS_DARK(IN_ROOM(ch)) && !CAN_SEE_IN_DARK(ch) && AFF_FLAGGED(i, AFF_INFRAVISION))
				send_to_char(ch, "@RYou see a pair of glowing red eyes looking your way.@n\r\n");
    }
}
...
void do_auto_exits(struct char_data *ch)
{
  ...
  send_to_char(ch, "@C[ @cExits: ");
	if (IN_VEHICLE(ch)) {
		for (door = 0; door < NUM_OF_DIRS; door++) {
			if (!VEHICLE_EXIT(ch, door) || VEHICLE_EXIT(ch, door)->to_room == NOWHERE)
				continue;
			if (EXIT_FLAGGED(VEHICLE_EXIT(ch, door), EX_CLOSED) && !CONFIG_DISP_CLOSED_DOORS)
				continue;
			// only imms can see hidden doors
			if (EXIT_FLAGGED(VEHICLE_EXIT(ch, door), EX_HIDDEN) && GET_LEVEL(ch) < LVL_IMMORT)
				continue;
			if (EXIT_FLAGGED(VEHICLE_EXIT(ch, door), EX_CLOSED))
				if (EXIT_FLAGGED(VEHICLE_EXIT(ch, door), EX_HIDDEN))
					send_to_char(ch, "%s(^\t(%c%s\t)) @c", CCRED(ch, C_NRM), UPPER(*dirs[door]), dirs[door] + 1);
				else
					send_to_char(ch, "%s(\t(%c%s\t)) @c", CCRED(ch, C_NRM), UPPER(*dirs[door]), dirs[door] + 1);
			else 
				send_to_char(ch, "@c\t(%c%s\t) ", UPPER(*dirs[door]), dirs[door] + 1);
			slen++;
		}
		send_to_char(ch, "%s@G(Leave)@n @C]%s\r\n", slen ? "" : "None! ", CCNRM(ch, C_NRM));
	}
	else {
		for (door = 0; door < NUM_OF_DIRS; door++) {
			...
			if (EXIT_FLAGGED(EXIT(ch, door), EX_CLOSED))
				if (EXIT_FLAGGED(EXIT(ch, door), EX_HIDDEN))
					...
					send_to_char(ch, "%s(\t(%c%s\t)) @c", CCRED(ch, C_NRM), UPPER(*dirs[door]), dirs[door] + 1);
					sprintf(temp,"(%c%s) ", UPPER(*dirs[door]), dirs[door] + 1);
					strcat(exits,temp);
				}
			else {
				send_to_char(ch, "@c\t(%c%s\t) ", UPPER(*dirs[door]), dirs[door] + 1);
		...
		send_to_char(ch, "%s@C]%s\r\n", slen ? "" : "None!", CCNRM(ch, C_NRM));
//here
		GET_EXITS(ch) = strdup(exits);
	}
}
...
ACMD(do_exits)
{
  ...
  if (IN_VEHICLE(ch)) {
  	for (door = 0; door < DIR_COUNT; door++) {
		...
		if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_SHOWVNUMS) && !EXIT_FLAGGED(VEHICLE_EXIT(ch, door), EX_CLOSED))
			send_to_char(ch, "%-5s - [%5d]%s %s\r\n", dirs[door], GET_ROOM_VNUM(VEHICLE_EXIT(ch, door)->to_room),
			EXIT_FLAGGED(VEHICLE_EXIT(ch, door), EX_HIDDEN) ? " [HIDDEN]" : "", world[VEHICLE_EXIT(ch, door)->to_room].name);
		else if (CONFIG_DISP_CLOSED_DOORS && EXIT_FLAGGED(VEHICLE_EXIT(ch, door), EX_CLOSED)) {
			/* But we tell them the door is closed */
			send_to_char(ch, "%-5s - The %s is closed%s\r\n", dirs[door],
			(VEHICLE_EXIT(ch, door)->keyword)? fname(VEHICLE_EXIT(ch, door)->keyword) : "opening",
			EXIT_FLAGGED(VEHICLE_EXIT(ch, door), EX_HIDDEN) ? " and hidden." : ".");
		}
		else
			send_to_char(ch, "%-5s - %s\r\n", dirs[door], IS_DARK(VEHICLE_EXIT(ch, door)->to_room) &&
			!CAN_SEE_IN_DARK(ch) ? "Too dark to tell." : world[VEHICLE_EXIT(ch, door)->to_room].name);
	}
...
void look_at_room(struct char_data *ch, int ignore_brief)
{
	...
	else if (GET_LEVEL(ch) < LVL_IMMORT && room_affected_by_psionic(IN_ROOM(ch), PSIONIC_SMOKE)) {
		send_to_char(ch, "The smoke is too thick to see through!\r\n\r\n");
		do_auto_exits(ch);
		return;
	}
	...
	if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_SHOWVNUMS)) {
		...
		send_to_char(ch, "@C[%5d] @n", GET_ROOM_VNUM(IN_ROOM(ch)));
		...
		send_to_char(ch, "@C%s@n", world[IN_ROOM(ch)].name);
  
        if (PK_OK_ROOM(ch))
            send_to_char(ch, " @R[PK]@n");
	}
	...
	list_char_to_char(world[IN_ROOM(ch)].people, ch);
    // room effects
    if (room_affected_by_psionic(IN_ROOM(ch), PSIONIC_FIRE))
        send_to_char(ch, "%sThe room is on fire!!!%s\r\n", CCRED(ch, C_NRM), CCNRM(ch, C_NRM));
    if (room_affected_by_psionic(IN_ROOM(ch), PSIONIC_TURRET))
        send_to_char(ch, "%s@DA large turret s@Oc@Da@On@Ds the room for enemies.%s\r\n", CCRED(ch, C_NRM), CCNRM(ch, C_NRM));

    if (room_affected_by_psionic(IN_ROOM(ch), PSIONIC_SMOKE)) /* they must be 31+ */
        send_to_char(ch, "@DThe room is filled with smoke.@n\r\n");
	/* autoexits */
...
static void look_in_obj(struct char_data *ch, char *arg)
{
	...
	else if (!(bits = generic_find(arg, FIND_OBJ_ROOM | FIND_OBJ_INV | FIND_OBJ_EQUIP, ch, &dummy, &obj)))
		send_to_char(ch, "There doesn't seem to be %s %s here.\r\n", AN(arg), arg);
	else if ((GET_OBJ_TYPE(obj) != ITEM_DRINKCON) && (GET_OBJ_TYPE(obj) != ITEM_FOUNTAIN) && (GET_OBJ_TYPE(obj) != ITEM_CONTAINER) && (GET_OBJ_TYPE(obj) != ITEM_TABLE))
		send_to_char(ch, "There's nothing inside or on that!\r\n");
	else {
...
static void look_on_obj(struct char_data *ch, char *arg)
{
	...
	if (!*arg)
		send_to_char(ch, "Look on what?\r\n");
	else if (!(bits = generic_find(arg, FIND_OBJ_ROOM | FIND_OBJ_INV | FIND_OBJ_EQUIP, ch, &dummy, &obj)))
		send_to_char(ch, "There doesn't seem to be %s %s here.\r\n", AN(arg), arg);
	else if ((GET_OBJ_TYPE(obj) != ITEM_DRINKCON) && (GET_OBJ_TYPE(obj) != ITEM_FOUNTAIN) && (GET_OBJ_TYPE(obj) != ITEM_TABLE))
		send_to_char(ch, "There's nothing on that!\r\n");
	else {
		if (GET_OBJ_TYPE(obj) == ITEM_TABLE) {
			if (OBJVAL_FLAGGED(obj, CONT_CLOSED) && (GET_LEVEL(ch) < LVL_IMMORT || !PRF_FLAGGED(ch, PRF_NOHASSLE)))
				send_to_char(ch, "It is closed.\r\n");
			...
			if ((GET_OBJ_VAL(obj, 1) == 0) && (GET_OBJ_VAL(obj, 0) != -1))
				send_to_char(ch, "It has nothing on it.\r\n");
			else {
				if (GET_OBJ_VAL(obj, 0) < 0) {
					char buf2[MAX_STRING_LENGTH];
					sprinttype(GET_OBJ_VAL(obj, 2), color_liquid, buf2, sizeof(buf2));
					send_to_char(ch, "It's drenched in %s liquid.\r\n", buf2);
				}
				else if (GET_OBJ_VAL(obj,1)>GET_OBJ_VAL(obj,0))
					send_to_char(ch, "Its covered in murky liquid.\r\n"); /* BUG */
				else {
					char buf2[MAX_STRING_LENGTH];
					amt = (GET_OBJ_VAL(obj, 1) * 3) / GET_OBJ_VAL(obj, 0);
					sprinttype(GET_OBJ_VAL(obj, 2), color_liquid, buf2, sizeof(buf2));
					send_to_char(ch, "It's %sdrenched in a %s liquid.\r\n", fullness[amt], buf2);
				}
			}
		}
	}
...
static void look_at_target(struct char_data *ch, char *arg)
{
  ...
  bits = generic_find(arg, FIND_OBJ_INV | FIND_OBJ_EQUIP | FIND_OBJ_ROOM | FIND_CHAR_ROOM, ch, &found_char, &found_obj);
  ...
  if (bits) {
    ...
    if (GET_OBJ_TYPE(found_obj) == ITEM_TABLE)
    look_on_obj(ch, arg);
    else {
...
ACMD(do_units)
{
  ...
  else if (GET_UNITS(ch) == 1)
    send_to_char(ch, "You have one miserable little resource unit.\r\n");
  else
    send_to_char(ch, "You have %d resource units.\r\n", GET_UNITS(ch));
}
...
ACMD(do_oldscore)
{
    ...
    if (age(ch)->month == 0 && age(ch)->day == 0)
        send_to_char(ch, "  It's your birthday today! Go get drunk!\r\n");
    else
        send_to_char(ch, "\r\n");

    send_to_char(ch, "Your stats are: Str: [@c%d/%d@n] Int: [@c%d@n] Pcn: [@c%d@n] Dex: [@c%d@n] Con: [@c%d@n] Cha: [@c%d@n]\n\r",
        GET_STR(ch), GET_STR_ADD(ch), GET_INT(ch), GET_PCN(ch), GET_DEX(ch), GET_CON(ch), GET_CHA(ch));
    send_to_char(ch, "Base stats are: Str: [@y%d/%d@n] Int: [@y%d@n] Pcn: [@y%d@n] Dex: [@y%d@n] Con: [@y%d@n] Cha: [@c%d@n]\n\r",
        GET_REAL_STR(ch), GET_REAL_ADD(ch), GET_REAL_INT(ch), GET_REAL_PCN(ch), GET_REAL_DEX(ch), GET_REAL_CON(ch), GET_CHA(ch));

    send_to_char(ch, "You have @g%d@n(%d) hit, @g%d@n(%d) psi, and @g%d@n(%d) movement points.\r\n",
        GET_HIT(ch), GET_MAX_HIT(ch), GET_PSI(ch), GET_MAX_PSI(ch), GET_MOVE(ch), GET_MAX_MOVE(ch));

    send_to_char(ch, "Your regen rates are H:%d (%d)  P:%d (%d)  V:%d (%d)\n\r",
        hit_gain(ch)/10, hit_gain(ch), psi_gain(ch)/10, psi_gain(ch), move_gain(ch)/10, move_gain(ch));

    send_to_char(ch, "Your armor will absorb @B%d@n.\r\n", GET_AC(ch));

    send_to_char(ch, "Your hitroll is @B%d@n, and your damroll is @B%d@n.\n\r",
        ch->points.hitroll, ch->points.damroll);

    send_to_char(ch, "You have scored @c%d@n exp, and have @c%d@n resource units.\r\n",
        GET_EXP(ch), GET_UNITS(ch));

    if (!IS_NPC(ch)) {

        if (GET_LEVEL(ch) < LVL_IMMORT)
            send_to_char(ch, "You need @c%d@n exp to reach your next level.\r\n", (exptolevel(ch, GET_LEVEL(ch) +1) - GET_EXP(ch)));

        send_to_char(ch, "You have @c%d@n quest point(s).\n\r", (int)(ch->player_specials->saved.questpoints));

        if (IS_HIGHLANDER(ch))
            send_to_char(ch, "You have @C%d@n essences.\n\r", ch->player.essences);
        ...
        if (IS_REMORT(ch))
            send_to_char(ch, "This ranks you as %s %s (level %d x%d remorts).\r\n", GET_NAME(ch), GET_TITLE(ch), GET_LEVEL(ch), GET_REMORTS(ch));
        else
            send_to_char(ch, "This ranks you as %s %s (level %d).\r\n", GET_NAME(ch), GET_TITLE(ch), GET_LEVEL(ch));

        send_to_char(ch, "You have hit %d DTs and have died %d times!\r\n", GET_NUM_DTS(ch), GET_NUM_DEATHS(ch));
    ...
    switch (GET_POS(ch)) {
        ...
        case POS_MEDITATING:
            send_to_char(ch, "You are meditating.\r\n");
            break;

        case POS_FOCUSING:
            send_to_char(ch, "You are focusing.\r\n");
            break;
    ...
    if (HAS_BIONIC(ch, BIO_INTERFACE)) {
        send_to_char(ch, "You have had cybernetic augmentation.\n\r");
        send_to_char(ch, "You are @r[@g%d@r]@n percent bionic.\n\r", BIONIC_PERCENT(ch));
    }

    if (PRF_FLAGGED(ch, PRF_NOSUMMON))
...
//    char pbuf[MAX_INPUT_LENGTH];
    ...
    REMOVE_BIT_AR(PRF_FLAGS(ch), PRF_OLDSCORE);

    playing_time = *real_time_passed((time(0) - ch->player.time.logon) + ch->player.time.played, 0);
    send_to_char(ch, "@D-=-=-=-=-=-=-=+-+-++-++-++-++-++-++-++-++-++-++-++-+-+=-=-=-=-=-=-=-@n\r\n");

    if (IS_REMORT(ch))
        send_to_char(ch, "@DYou Are: @W%s %s    @D(@n@wLevel @W%d @Dx@W %d @n@wRemorts@D)@n\r\n", GET_NAME(ch), GET_TITLE(ch), GET_LEVEL(ch), GET_REMORTS(ch));
    else
        send_to_char(ch, "@DYou Are: @W%s %s    @D(@n@wLevel @W%d @Dx@W 0 @n@wRemorts@D)@n\r\n", GET_NAME(ch), GET_TITLE(ch), GET_LEVEL(ch));
//    send_to_char(ch, "Online Time: [ @y%dd %dh@n ]\r\n", playing_time.day, playing_time.hours);
    send_to_char(ch, "@DAge:    [ @W%d@D ]     Height: [ @W%d ft@D ]     Weight: [ @W%d @Dlbs ]@n\r\n", GET_AGE(ch), GET_HEIGHT(ch) / 30, GET_WEIGHT(ch));
    send_to_char(ch, "@D-=-=-=-=-=-=-=+-+-++-++-++-++-++-++-++-++-++-++-++-+-+=-=-=-=-=-=-=-@n\r\n");
    send_to_char(ch, "  @DHP: [ @G%4d / %4d@D ]       HP Regen: [ @G%3d@D ]      Armor: [ @W%3d@D ]@n  \r\n",
        GET_HIT(ch), GET_MAX_HIT(ch), hit_gain(ch)/10, GET_AC(ch));

    send_to_char(ch, " @DPsi: [ @M%4d / %4d@D ]      Psi Regen: [ @M%3d@D ]    Hitroll: [ @W%3d@n@D ]  @n\r\n",
        GET_PSI(ch), GET_MAX_PSI(ch), psi_gain(ch)/10, GET_HITROLL(ch));

    send_to_char(ch, "@DMove: [ @C%4d / %4d@D ]     Move Regen: [ @C%3d@D ]    Damroll: [ @W%3d@D ]@n  \r\n",
        GET_MOVE(ch), GET_MAX_MOVE(ch), move_gain(ch)/10, GET_DAMROLL(ch));

    send_to_char(ch, "@D-=-=-=-=-=-=-=+-+-++-++-++-++-++-++-++-++-++-++-++-+-+=-=-=-=-=-=-=-@n\r\n");

    send_to_char(ch, "@DStr: [ @W%2d@D ]         Base: [ @W%2d@D ]      Experience: [ @W%10d@D ]@n\r\n",
        GET_STR(ch), GET_REAL_STR(ch), GET_EXP(ch));

    send_to_char(ch, "@DInt: [ @W%2d@D ]               [ @W%2d@D ]        To Level: [ @W%10d@D ]@n\r\n",
        GET_INT(ch), GET_REAL_INT(ch), (exptolevel(ch, GET_LEVEL(ch) +1) - GET_EXP(ch)));

    send_to_char(ch, "@DPcn: [ @W%2d@D ]               [ @W%2d@D ]      Res. Units: [ @W%10d@D ]@n\r\n",
        GET_PCN(ch), GET_REAL_PCN(ch), GET_UNITS(ch));

    send_to_char(ch, "@DDex: [ @W%2d@D ]               [ @W%2d@D ]            Bank: [ @W%10d@n @D]@n\r\n",
        GET_DEX(ch), GET_REAL_DEX(ch), GET_BANK_UNITS(ch));

    send_to_char(ch, "@DCon: [ @W%2d@D ]               [ @W%2d@D ]         %%Bionic: [        @W%2d%%@D ]@n\r\n",
        GET_CON(ch), GET_REAL_CON(ch), BIONIC_PERCENT(ch));

	send_to_char(ch, "@DCha: [ @W%2d@D ]               [ @W%2d@D ]          Deaths: @D[      @R%5d@n @D]@n\r\n", GET_CHA(ch), GET_REAL_CHA(ch), GET_NUM_DEATHS(ch));

    send_to_char(ch, "@DDTs: [ @R%2d@n@D ]                               \r\n",
        GET_NUM_DTS(ch));
	send_to_char(ch, "@D-----------------------[ @WDamage Resistances@D ]-----------------------@n\r\n");
	send_to_char(ch, "@D Normal    [ @G%3d@n@D ]   @DPoison   [ @G%3d@n@D ]   @DDrugs       [ @G%3d@n@D ]\r\n",
		GET_SAVE(ch, 0), GET_SAVE(ch, 1), GET_SAVE(ch, 2));
	send_to_char(ch, "@D Heat      [ @G%3d@n@D ]   @DCold     [ @G%3d@n@D ]   @DElectricity [ @G%3d@n@D ]\r\n",
		GET_SAVE(ch, 3), GET_SAVE(ch, 4), GET_SAVE(ch, 5));
	send_to_char(ch, "@D Explosive [ @G%3d@n@D ]   @DPsionic  [ @G%3d@n@D ]   @DEthereal    [ @G%3d@n@D ]\r\n\r\n",
		GET_SAVE(ch, 6), GET_SAVE(ch, 7), GET_SAVE(ch, 8));
	send_to_char(ch, "@D----------------------[ @WCharacter Progression@D ]---------------------@n\r\n");
	send_to_char(ch, "@DProgression:  [ @G%2d@n@D ]      Tier: [ @G%5d @D]     Progression%%: @D[ @G%4d@n@D%%]@n\r\n",
		GET_PROGRESS(ch), GET_TIER(ch), GET_PROGRESSPCNT(ch));
	send_to_char(ch, "@DQPs:  [ @G%10d@n@D ]      Fame: [ @G%5d@D ]      \r\n",
		GET_QUESTPOINTS(ch), GET_FAME(ch));

	send_to_char(ch, "@D--------------------------------------------------------------------@n\r\n");
			send_to_char(ch, "@wMaximum weight allowance:     [ @R%4d@w ]@n\r\n", CAN_CARRY_W(ch));
			send_to_char(ch, "@wYou are carrying:             [ @R%4d@w ]@n\r\n", IS_CARRYING_W(ch));
			send_to_char(ch, "@wYou are wearing:              [ @R%4d@w ]@n\r\n", IS_WEARING_W(ch));
			send_to_char(ch, "@wEncumbrance:                  [ @R%3d@W%%@w ]@n\r\n", ENCUMBRANCE(ch));
    send_to_char(ch, "@D-=-=-=-=-=-=-=+-+-++-++-++-++-++-++-++-++-++-++-++-+-+=-=-=-=-=-=-=-@n\r\n");
//    send_to_char(ch, "%d attacks per round.\r\n", ATTACKS_PER_ROUND(ch));
    if (IS_HIGHLANDER(ch))
        send_to_char(ch, "@YYou have @C%d@n@Y essences.@n\r\n", ch->player.essences);
        send_to_char(ch, "@D");
    ...
    switch (GET_POS(ch)) {
        ...
        case POS_FIGHTING:
            if (FIGHTING(ch))
                ...
                send_to_char(ch, "You are fighting thin air.\r\n");
            break;
    ...
    if (AFF_FLAGGED(ch, AFF_MANIAC))
        send_to_char(ch, "@RYou are a maniac!@n\r\n");
...
/* Old score, or new score?  New by default, with PRF_OLDSCORE flag for old style */
ACMD(do_score)
{
    if (PRF_FLAGGED(ch, PRF_OLDSCORE))
        do_oldscore(ch, "", 0, 0);
    else
        do_newscore(ch, "", 0, 0);
...
ACMD(do_equipment)
{
	...
	if (!IS_MOB(ch)) {
		if (!PRF_FLAGGED(ch, PRF_BLIND)) {
		    send_to_char(ch, "@B_ _ _ _ _ _ _ _ _ _ _ _ _ _\r\n");
			send_to_char(ch, "|    @WEquipment Status@B     |\r\n");
			send_to_char(ch, "_ _ _ _ _ __ _ _ _ _ _ _ _@n\r\n");
		}
	}
...
ACMD(do_time)
{
  ...
  send_to_char(ch, "The %d%s Day of the %s, Year %d.\r\n",
	  day, suf, month_name[time_info.month], (time_info.year + 2300));
}
...
ACMD(do_newwho)
{
    ...
    REMOVE_BIT_AR(PRF_FLAGS(ch), PRF_OLDWHO);
        send_to_char(ch, "\r\n");
	send_to_char(ch, "@D-=-=-=-=-=-=+-++-++-++-++-++-++-++-++-++-++-++-+=-=-=-=-=-=-@n\r\n");
	send_to_char(ch, "@D            |@WC@D||@Wy@D||@Wb@D||@We@D||@Wr@D||@WA@D||@WS@D||@WS@D||@WA@D||@WU@D||@WL@D||@WT@D|@n@n\r\n");
	send_to_char(ch, "@D-=-=-=-=-=-=+-++-++-++-++-++-++-++-++-++-++-++-+=-=-=-=-=-=-@n\r\n\r\n");
    ...
    for (d = descriptor_list; d && !short_list; d = d->next) {
        ...
        if (IS_PLAYING(d) && WHO_CAN_SEE(ch, tch)) {
    ...
    for (i = 0; *rank[i].disp != '\n'; i++) {
        ...
        for (d = descriptor_list; d; d = d->next) {
            ...
            if (short_list) {
                send_to_char(ch, "%s@D[@W%2d %s@D ] @n@W %-12.12s@n%s%s",
				...
				if (GET_LEVEL(tch) >= LVL_IMMORT) {
					if (GET_LEVEL(tch) == LVL_IMMORT)
						GET_WIZTITLE(tch) = "@C    God    ";
					if (GET_LEVEL(tch) == LVL_GOD)
						GET_WIZTITLE(tch) = "@Y  Builder  ";
					if (GET_LEVEL(tch) == LVL_GRGOD)
						GET_WIZTITLE(tch) = "@R  Builder  ";
					if (GET_LEVEL(tch) == LVL_CIMP)
						GET_WIZTITLE(tch) = "@BGame Design";
					if (GET_LEVEL(tch) == LVL_IMPL)
						GET_WIZTITLE(tch) = "@GImplementor";
					if (!strcmp(GET_NAME(tch), "Gahan"))
						GET_WIZTITLE(tch) = "@G Developer ";
					if (!strcmp(GET_NAME(tch), "Warblade"))
						GET_WIZTITLE(tch) = "@GHeadBuilder";
					if (!strcmp(GET_NAME(tch), "Bosstone"))
						GET_WIZTITLE(tch) = "@GImplementor";

					send_to_char(ch, "@D[ @W%-11s@D ] @W%s@n%s%s%s",
						GET_WIZTITLE(tch), GET_NAME(tch), (*GET_TITLE(tch) ? " " : ""), GET_TITLE(tch), CCNRM(ch, C_SPR));
				}
				else if (GET_CLASS2(tch) == 0) {
					send_to_char(ch, "%s@D[@W%2d %s|--- @C%2d@D] @W%s@n%s%s%s",
						(GET_LEVEL(tch) >= LVL_IMMORT ?CCYEL(ch, C_SPR) : ""),
						GET_LEVEL(tch), CLASS_DUALABBR(tch), GET_PROGRESS(tch),
						GET_NAME(tch), (*GET_TITLE(tch) ? " " : ""), GET_TITLE(tch),
						CCNRM(ch, C_SPR));
				}
				else {
					send_to_char(ch, "%s@D[@W%2d %s|%s @C%2d@D] @W%s@n%s%s%s",
						(GET_LEVEL(tch) >= LVL_IMMORT ?CCYEL(ch, C_SPR) : ""),
						GET_LEVEL(tch), CLASS_DUALABBR(tch), CLASS_DUALABBR2(tch), GET_PROGRESS(tch),
						GET_NAME(tch), (*GET_TITLE(tch) ? " " : ""), GET_TITLE(tch),
						CCNRM(ch, C_SPR));
				}
                ...
                if (PLR_FLAGGED(tch, PLR_PK))
                    send_to_char(ch, " (PK)");

                if (PRF_FLAGGED(tch, PRF_NOWHO))
                    send_to_char(ch, " @c(nowho)@n");

                if (PLR_FLAGGED(tch, PLR_REP))
                    send_to_char(ch, " @B(@WREP@B)@n");
    ...
    if (!num_can_see)
        ...
        send_to_char(ch, "@G%d@W visible player online out of @G%d @Wconnected.@n\r\n", num_can_see, connected);
    else
        send_to_char(ch, "@G%d@W visible characters displayed out of @G%d @Wconnected.@n\r\n", num_can_see, connected);

	if (IS_HAPPYHOUR > 0)
		send_to_char(ch, "@WIt's a @YHappy Hour!@W Type @Yhappyhour show@W for more information.@n\r\n");
	if (IS_HAPPYHOUR > 0 || EVENT_LIVE)
		send_to_char(ch, "@D-=-=-=-=-=-=+-++-++-++-++-++-++-++-++-++-++-++-+=-=-=-=-=-=-@n\r\n");
	if (EVENT_LIVE)
		send_to_char(ch, "\r\n@GThere is currently a world event in progress.  Type '@Yeventinfo@G' for details.@n\r\n");
}
...
ACMD(do_oldwho)
{
    ...
    for (i = 0; *rank[i].disp != '\n'; i++) {
        ...
        for (d = descriptor_list; d; d = d->next) {
            ...
            if (short_list) {
                ...
                num_can_see++;
                send_to_char(ch, "%s@D[@W%2d %s@D]@n @W%s@n%s%s%s",
                    (GET_LEVEL(tch) >= LVL_IMMORT ? CCYEL(ch, C_SPR) : ""),
    ...
    if (!num_can_see)
        ...
        send_to_char(ch, "@G%d@W visible character displayed out of @G%d @Wconnected.@n\r\n", num_can_see, connected);
    else
        send_to_char(ch, "@G%d@W visible characters displayed out of @G%d @Wconnected.@n\r\n", num_can_see, connected);
...
static void print_object_location(int num, struct obj_data *obj, struct char_data *ch, int recur)
{
  ...
  else if (obj->implanted)
    send_to_char(ch, "implanted in %s%s\r\n", PERS(obj->implanted, ch), QNRM);
  else if (obj->in_obj) {
    send_to_char(ch, "inside %s%s%s\r\n", obj->in_obj->short_description, QNRM, (recur ? ", which is" : " "));
    if (recur)
      print_object_location(0, obj->in_obj, ch, recur);
  }
    else if (GET_OBJ_TYPE(obj) == ITEM_EXPLOSIVE && GET_EXPLOSIVE_TYPE(obj) == EXPLOSIVE_PLASTIQUE) {

        // todo: assuming that plastique is storing its location type in val 2
        if (GET_OBJ_VAL(obj, 1) == PLASTIQUE_VEHICLE) {
            // todo: add in vehicles
            send_to_char(ch, "attached to an unknown vehicle\r\n");
        }
        else if (GET_OBJ_VAL(obj, 1) == PLASTIQUE_OBJ) {

            if (obj->attached) {
                send_to_char(ch, "attached to %s%s\r\n", obj->attached->short_description, (recur ? ", which is" : " "));
                if (recur)
                    print_object_location(0, obj->attached, ch, recur);
            }
            else
                send_to_char(ch, "attached to an unknown object\r\n");

        }
        else if (GET_OBJ_VAL(obj, 1) >= PLASTIQUE_ROOM_NORTH && GET_OBJ_VAL(obj, 1) <= PLASTIQUE_ROOM_DOWN)
              send_to_char(ch, "attached to %s [%s].\r\n", world[GET_OBJ_VAL(obj, 5)].name, dirs[GET_OBJ_VAL(obj, 1)]);
        else
...
ACMD(do_levels)
{
  ...
  if (arg != NULL && *arg) {
    ...
    for (i = min_lev; i <= max_lev; i++) {
		if (i == max_lev) {
			nlen = snprintf(buf + len, sizeof(buf) - len, "--------------\r\nProg %9d\r\n", (10000000 * GET_PROGRESS(ch)));
			if (len + nlen >= sizeof(buf))
				break;
			len += nlen;
			break;
		}
        nlen = snprintf(buf + len, sizeof(buf) - len, "[%2d] %9d\r\n", i, exptolevel(ch, i));
        if (len + nlen >= sizeof(buf))
            break;
        len += nlen;
    }
...
ACMD(do_consider)
{
  ...
  if (victim == ch) {
    send_to_char(ch, "If you truly want to kill yourself, type 'suicide'.\r\n");
    return;
  }
  if (!IS_NPC(victim)) {
    ...
    if (victim == ch) {
        send_to_char(ch, "@GIf you truly want to kill yourself, type 'suicide'.@n\r\n");
        return;
	}
    
    if (GET_MAX_HIT(victim) < 501)
        send_to_char(ch, "@G%s looks about as tough as a puppy.@n\r\n", GET_NAME(victim));
    else if (GET_MAX_HIT(victim) > 500 && (GET_MAX_HIT(victim) < 1001))
        send_to_char(ch, "@G%s looks about as tough as a newbie.@n\r\n", GET_NAME(victim));
    else if (GET_MAX_HIT(victim) > 1001 && (GET_MAX_HIT(victim) < 5001))
        send_to_char(ch, "@R%s looks quite tough by normal standards.@n\r\n", GET_NAME(victim));
    else if (GET_MAX_HIT(victim) > 5001 && (GET_MAX_HIT(victim) < 10001))
        send_to_char(ch, "@R%slooks well trained and dangerous.@n\r\n", GET_NAME(victim));
    else if (GET_MAX_HIT(victim) > 10001 && (GET_MAX_HIT(victim) < 20001))
        send_to_char(ch, "@R%s looks abnormally tough.@n\r\n", GET_NAME(victim));
    else if (GET_MAX_HIT(victim) > 20001 && (GET_MAX_HIT(victim) < 30001))
        send_to_char(ch, "@R%s looks extremely difficult to defeat in battle.@n\r\n", GET_NAME(victim));
    else if (GET_MAX_HIT(victim) > 30001 && (GET_MAX_HIT(victim) < 50001))
        send_to_char(ch, "@R%s looks superhuman.@n\r\n", GET_NAME(victim));
    else if (GET_MAX_HIT(victim) > 50001 && (GET_MAX_HIT(victim) < 100001))
        send_to_char(ch, "@R...you would probably need a missle to harm %s.@n\r\n", GET_NAME(victim));
    else if (GET_MAX_HIT(victim) > 100001 && (GET_MAX_HIT(victim) < 500000))
        send_to_char(ch, "@R%s looks tougher than god himself.@n\r\n", GET_NAME(victim));
    else
        send_to_char(ch, "@RChills run down your spine as you consider this fight.@n\r\n");

    if (GET_DAMROLL(ch) >= GET_DAMROLL(victim)) {
      damr = (GET_DAMROLL(ch) - GET_DAMROLL(victim));
      if (damr < 51) 
        send_to_char(ch, "@GThe fight seems fair damage wise.@n\r\n");
      else if (damr < 151)
        send_to_char(ch, "@GYou could hurt them worse than they could hurt you, given the chance.@n\r\n");
      else if (damr < 299 && damr < 500)
        send_to_char(ch, "@GYou have the ability to a lot more damage then them!@n\r\n");
      else 
        send_to_char(ch, "@GYour ability to inflict damage far exceeds theirs!!@n\r\n");
    }
    if (GET_DAMROLL(victim) > GET_DAMROLL(ch)) {
      damr = (GET_DAMROLL(victim) - GET_DAMROLL(ch));
      if (damr < 51) 
        send_to_char(ch, "@GThe fight seems fair damage wise.@n\r\n");
      else if (damr < 151)
        send_to_char(ch, "@RThey could hurt you worse than you could hurt them, given the chance.@n\r\n");
      else if (damr < 299 && damr < 500)
        send_to_char(ch, "@RThey have the ability to do a lot more damage than you!@n\r\n");
      else 
        send_to_char(ch, "@RTheir ability to inflict damage far exceeds yours!!@n\r\n");
    }
    if (GET_AC(ch) == GET_HITROLL(victim)) {
        send_to_char(ch, "@GYou will get hurt... but you should survive.@n\r\n");
    }
    if (GET_AC(ch) > GET_HITROLL(victim)) {
      damr = (GET_AC(ch) - GET_HITROLL(victim));
      if (damr < 51) 
        send_to_char(ch, "@GYou will get hurt... but you should survive..@n\r\n");
      else if (damr > 51 && (damr < 100)) 
        send_to_char(ch, "@G%s would need some lucky hits to hurt you.@n\r\n", GET_NAME(victim));
      else if (damr > 99) 
        send_to_char(ch, "@GYou should be able to shrug off anything %s can throw in your direction.@n\r\n", GET_NAME(victim));
    }

    if (GET_HITROLL(victim) > GET_AC(ch)) {
      damr = (GET_HITROLL(victim) - GET_AC(ch));
      if (damr < 51) 
        send_to_char(ch, "@RYour armor provides very little defense against %s.@n\r\n", GET_NAME(victim));
      else
        send_to_char(ch, "@RYour armor will provide nearly no defense against %s's attacks.@n\r\n", GET_NAME(victim));
    }
    if (GET_AC(victim) == GET_HITROLL(ch)) {
        send_to_char(ch, "@G%s's armor shouldn't be much of a problem for you.@n\r\n", GET_NAME(victim));
    }
    if (GET_HITROLL(ch) > GET_AC(victim)) {
      damr = (GET_HITROLL(ch) - GET_AC(victim));
      if (damr < 51) 
        send_to_char(ch, "@G%s's armor can barely stand your assault!@n\r\n", GET_NAME(victim));
      else if (damr > 51)
        send_to_char(ch, "@G%s might as well be naked when it comes to the fury of your attacks.@n\r\n", GET_NAME(victim));
    }
    if (GET_AC(victim) > GET_HITROLL(ch)) {
      damr = (GET_AC(victim) - GET_HITROLL(ch));
      if (damr < 51) 
        send_to_char(ch, "@G%s's armor shouldn't be much of a problem.@n\r\n", GET_NAME(victim));
      else if (damr > 50 && (damr < 101))
        send_to_char(ch, "@R%s is somewhat armored from your attacks.@n\r\n", GET_NAME(victim));
      else if (damr > 100)
	    send_to_char(ch, "@RYou would need to be really lucky to even scratch %s!@n\r\n", GET_NAME(victim));
	}
    if (GET_LEVEL(victim) > 30)
        send_to_char(ch, "@Y%s is protected from special attacks.@n\r\n", GET_NAME(victim));
 
    if (SCRIPT(victim) && TRIGGERS(SCRIPT(victim)))
        send_to_char(ch, "@Y%s may have some tricks up their sleeves.@n\r\n", GET_NAME(victim));
	 
	if (GET_SKILL_LEVEL(ch, SKILL_PSIONICS_LORE == 0)) {
		send_to_char(ch, "@WYou can disern no information about any psionics surrounding %s.\r\n", GET_NAME(victim));
		return;
	}
		...
		if (affected_by_psionic(victim, PSIONIC_SANCTUARY))
			send_to_char(ch, "@W%s is protected by Sanctuary.@n\r\n", GET_NAME(victim));
	
		else if (affected_by_psionic(victim, PSIONIC_SUPER_SANCT))
		    send_to_char(ch, "@D%s is protected by Super Sanctuary.@n\r\n", GET_NAME(victim));
	    
		else if (affected_by_psionic(victim, PSIONIC_MEGA_SANCT))
		    send_to_char(ch, "@Y%s surrounded by Mega Sanctuary.@n\r\n", GET_NAME(victim));	
		
		if (AFF_FLAGGED(victim, AFF_CHARM))
			send_to_char(ch, "@W%s is charmed.@n\r\n", GET_NAME(victim));

	    if (victim->char_specials.saved.duplicates > 0)
		    act("@G$n has several duplicate images surrounding $m.@n\r\n", FALSE, victim, 0, ch, TO_VICT);

	    if (AFF_FLAGGED(victim, AFF_STUN))
		    act("@Y$n appears to be stunned!@n\r\n", FALSE, victim, 0, ch, TO_VICT);
	}
    if (AFF_FLAGGED(victim, AFF_PSI_MIRROR)) {
        send_to_char(ch, "@B%s's image is being shrouded by dozens of floating mirrors nearby!@n\r\n", GET_NAME(victim));
    }

	for (i=0;i < MAX_SAVING_THROW;i++) {
		if (GET_SAVE(victim, i) != 0) {
			if (GET_SAVE(victim, i) <= -90)
				send_to_char(ch, "@G%s is extremely vulnerable to %s damage.@n\r\n", GET_NAME(victim), damagetype_bits[i]);
			else if (GET_SAVE(victim, i) <= -70)
				send_to_char(ch, "@G%s is very vulnerable to %s damage.@n\r\n", GET_NAME(victim), damagetype_bits[i]);
			else if (GET_SAVE(victim, i) <= -40)
				send_to_char(ch, "@G%s is moderately vulnerable to %s damage.@n\r\n", GET_NAME(victim), damagetype_bits[i]);
			else if (GET_SAVE(victim, i) <= -20)
				send_to_char(ch, "@G%s is fairly vulnerable to %s damage.@n\r\n", GET_NAME(victim), damagetype_bits[i]);
			else if (GET_SAVE(victim, i) <= -5)
				send_to_char(ch, "@G%s is slightly vulnerable to %s damage.@n\r\n", GET_NAME(victim), damagetype_bits[i]);
			else if (GET_SAVE(victim, i) >= 70)
				send_to_char(ch, "@G%s is extremely resistant to %s damage.@n\r\n", GET_NAME(victim), damagetype_bits[i]);
			else if (GET_SAVE(victim, i) >= 50)
				send_to_char(ch, "@G%s is very resistant to %s damage.@n\r\n", GET_NAME(victim), damagetype_bits[i]);
			else if (GET_SAVE(victim, i) >= 30)
				send_to_char(ch, "@G%s is moderately resistant to %s damage.@n\r\n", GET_NAME(victim), damagetype_bits[i]);
			else if (GET_SAVE(victim, i) >= 20)
				send_to_char(ch, "@G%s is fairly resistant to %s damage.@n\r\n", GET_NAME(victim), damagetype_bits[i]);
			else if (GET_SAVE(victim, i) >= 5)
				send_to_char(ch, "@G%s is slightly resistant to %s damage.@n\r\n", GET_NAME(victim), damagetype_bits[i]);
		}
	}	
}
...
ACMD(do_toggle)
{
  ...
  any_one_arg(argument, arg2); 

  if (!*arg) {
    if (!GET_WIMP_LEV(ch))
      strcpy(buf2, "OFF");        
    else
      sprintf(buf2, "%-3.3d", GET_WIMP_LEV(ch));  
...
ACMD(do_whois)
{
  ...
  send_to_char(ch, "Class: %s\r\n", pc_class[GET_CLASS(victim)].class_name);
...
/* Gahantodo: This will be the recon skill... scan will scan an object with identifier
ACMD(do_scan)
{
  int door;
  bool found=FALSE;
...
ACMD(do_search)
{
    ...
    if (AFF_FLAGGED(ch, AFF_BLIND)) {
        send_to_char(ch, "You're blind, you can't see a damned thing!\r\n");
        return;
    }

	if (GET_SKILL_LEVEL(ch, SKILL_SEARCH) == 0) 
		chance = 2;
	else 
		chance = 5;
	if (GET_SKILL_LEVEL(ch, SKILL_SEARCH) == 0) 
        send_to_char(ch, "You stumble around as you try to search the room.\r\n");
	else 
        send_to_char(ch, "You skillfully begin to search the room.\r\n");




	for (obj = world[IN_ROOM(ch)].contents; obj && !foundobj; obj = obj->next_content) {
		if (OBJ_IS_HIDDEN(obj)) {
	        ...
	        if (rand_number(1,10) <= chance) {
				foundobj = TRUE;
				REMOVE_BIT_AR(GET_OBJ_EXTRA(obj), ITEM_HIDDEN);
				send_to_char(ch, "@YYou notice %s on the ground.@n\r\n", obj->short_description);
				act("@Y$n reveals a hidden object!@n", TRUE, ch, 0, 0, TO_NOTVICT);
				int exp = (GET_LEVEL(ch) * (GET_LEVEL(ch) / 2)) * 15;
				increment_skilluse(SKILL_SEARCH, 1, GET_IDNUM(ch), exp, ch);
				break;
			}
		}
	}
	
	for (hidden = world[IN_ROOM(ch)].people; hidden && !foundhid; hidden = hidden->next_in_room) {
		if (AFF_FLAGGED(hidden, AFF_HIDE)) {
	        ...
	        if (rand_number(1,15) <= chance) {
				foundhid = TRUE;
				REMOVE_BIT_AR(AFF_FLAGS(hidden), AFF_HIDE);
				send_to_char(ch, "@YYou expose the hiding spot of %s.@n\r\n", GET_NAME(hidden));
				act("@Y$n reveals the hiding spot of $N.@n", TRUE, ch, 0, hidden, TO_NOTVICT);
				act("@Y$n reveals your hiding spot!@n", TRUE, ch, 0, hidden, TO_VICT);
				int exp = (GET_LEVEL(ch) * (GET_LEVEL(ch) / 2)) * 15;
				increment_skilluse(SKILL_SEARCH, 1, GET_IDNUM(ch), exp, ch);
				break;
			}
    ...
    for (door = 0; door < NUM_OF_DIRS; door++) {
        if (EXIT(ch, door) && EXIT(ch, door)->to_room != NOWHERE) {
			...
			chance += 2;
                ...
                if (rand_number(1,15) <= chance) {
                    send_to_char(ch, "@YYou have revealed a hidden exit to the %s.@n\r\n", dirs[door]);
					act("@Y$n has revealed a hidden exit!@n", TRUE, ch, 0, 0, TO_NOTVICT);
                    ...
                    send_to_char(ch, "You finish your search.\r\n");
...
{
    int i;
    int limit = GET_LEVEL(ch) <= LVL_GRGOD ? TOP_MORT_PSIONICS : TOP_IMMORT_PSIONICS;

    send_to_char(ch, "You can cast the following psionics:\n\r");

    for (i = 1; i < limit; i++)
        if (GET_PSIONIC_LEVEL(ch, i) > 0 && strcmp("!UNUSED!", psionics_info[i].name))
            send_to_char(ch, "%-20s %s\r\n", psionics_info[i].name, how_good(GET_PSIONIC_LEVEL(ch, i)));
}

ACMD(do_skills)
{
    int i;
    int limit = GET_LEVEL(ch) <= LVL_GRGOD ? TOP_MORT_SKILLS : TOP_IMMORT_SKILLS;

    send_to_char(ch, "You know of the following skills:\n\r");

    for (i = 1; i < limit; i++)
        if (GET_SKILL_LEVEL(ch, i) > 0 && strcmp("!UNUSED!", skills_info[i].name))
            send_to_char(ch, "%-20s %s\n\r", skills_info[i].name, how_good(GET_SKILL_LEVEL(ch, i)));
            send_to_char(ch, "\r\nAlso see: SKILLTREE\r\n");
...
ACMD(do_bionics)
{
    ...
    for (i = 0; i < MAX_BIONIC; i++) {
        ...
        if (HAS_BIONIC(ch, i)) {
            ...
            if (found != TRUE) {
                send_to_char(ch, "You have the following bionic enhancements:\r\n");
                send_to_char(ch, "\r\n\r\n   ENHANCEMENT          LEVEL\r\n");
                send_to_char(ch, "\r\n ------------------------------------\r\n");
                found = TRUE;
            }
            ...
            j = BIONIC_LEVEL(ch, i);
            if (j < 0 || j > 3)
                send_to_char(ch, "   %-20s UNKNOWN\r\n", bionic_names[i][0]);
            else
                send_to_char(ch, "   %-20s %-20s\r\n", bionic_names[i][0], bionic_level[j]);

        }
    }

    if (!found)
        send_to_char(ch, "You currently have no bionics.\r\n");
...
ACMD(do_anatomy)
{
    ...
    send_to_char(ch, "@W----------------------------------------------------------------------------------------------------------@n\r\n");
    send_to_char(ch, "@DBody Part            Condition  Bionic Device                      Type      Requires\r\n");
    send_to_char(ch, "@W----------------------------------------------------------------------------------------------------------@n\r\n");

    for (i = 1; i < NUM_BODY_PARTS; i++) {

        sprintbit(BODYPART_CONDITION(ch, i), part_condition, buf, sizeof(buf));
        send_to_char(ch, "@Y%-20s %-10s %-35s", body_parts[i], buf, BIONIC_DEVICE(ch, i) ? BIONIC_DEVICE(ch, i)->short_description :"");
        send_to_char(ch, "@Y%-10s ", BIONIC_DEVICE(ch, i) ?  biotypes[GET_OBJ_VAL(BIONIC_DEVICE(ch, i), 2)] : "");
        send_to_char(ch, "@Y%-10s ", BIONIC_DEVICE(ch, i) ?  biotypes[GET_OBJ_VAL(BIONIC_DEVICE(ch, i), 3)] : "");
        send_to_char(ch, "@Y%-10s ", BIONIC_DEVICE(ch, i) ?  biotypes[GET_OBJ_VAL(BIONIC_DEVICE(ch, i), 4)] : "");
        send_to_char(ch, "@Y%-10s \r\n", BIONIC_DEVICE(ch, i) ?  biotypes[GET_OBJ_VAL(BIONIC_DEVICE(ch, i), 5)] : "");
...
ACMD(do_hitroll)
{
    send_to_char(ch, "Your hitroll is %d and your damroll is %d.\r\n", GET_HITROLL(ch), GET_DAMROLL(ch));
}
ACMD(do_ammo)
{
if (ch->equipment[WEAR_WIELD]) {
   ...
   send_to_char(ch, "You have %d bullets in your gun.\r\n", GET_LOADED_AMMO_COUNT(ch->equipment[WEAR_WIELD]));
   else
    send_to_char(ch, "That gun is empty.\r\n");
        return;
  }
    send_to_char(ch, "You have no gun!\r\n");
        return;
 }
    send_to_char(ch, "Hmm!\r\n");
...
ACMD(do_armor)
{
    send_to_char(ch, "Your armor will absorb %d.\r\n", GET_AC(ch));
}

ACMD(do_exp)
{
    send_to_char(ch, "You have gained %d experience points total and need %d to level.\r\n", GET_EXP(ch), (exptolevel(ch, GET_LEVEL(ch) +1) - GET_EXP(ch)));
}
...
ACMD(do_affections)
{
  ...
  struct mud_event_data *pMudEvent = NULL;
    ...
    else if (affected_by_psionic(ch, PSIONIC_SUPER_SANCT))
        send_to_char(ch, "You are protected by Super Sanctuary.\n\r");
    else if (affected_by_psionic(ch, PSIONIC_MEGA_SANCT))
        send_to_char(ch, "You are protected by Mega Sanctuary.(tm)\n\r");

    if (AFF_FLAGGED(ch, AFF_POISON))
        send_to_char(ch, "You are poisoned!\n\r");

    if (AFF_FLAGGED(ch, AFF_DODGE))
        send_to_char(ch, "You are dodging attacks like a badass.\n\r");
    ...
    if (affected_by_psionic(ch, PSIONIC_INDESTRUCTABLE_AURA))
        send_to_char(ch, "You are surrounded by an indestructible aura!\r\n");

    if (affected_by_psionic(ch, PSIONIC_PSI_SHIELD))
        send_to_char(ch, "You are enveloped by a psionic shield.\r\n");

    if (AFF_FLAGGED(ch, AFF_COOL_AURA))
        send_to_char(ch, "You are insulated by a cooling aura.\n\r");

    if (AFF_FLAGGED(ch, AFF_BIO_TRANCE))
        send_to_char(ch, "You are in a state of bio trance.\n\r");

    if (affected_by_psionic(ch, PSIONIC_COURAGE))
        send_to_char(ch, "You are courageous!\n\r");

    if (affected_by_psionic(ch, PSIONIC_FANATICAL_FURY))
        send_to_char(ch, "Your fury is fanatical!\n\r");

    if (affected_by_psionic(ch, PSIONIC_INSPIRE))
        send_to_char(ch, "You are inspired.\n\r");

    if (affected_by_psionic(ch, PSIONIC_INVIGORATE))
        send_to_char(ch, "You have been invigorated.\n\r");

    if (affected_by_psionic(ch, PSIONIC_LETHARGY))
        send_to_char(ch, "You feel lethargic.\n\r");

    if (AFF_FLAGGED(ch, AFF_LEVITATE))
        send_to_char(ch, "You are levitating off the ground.\n\r");

    if (AFF_FLAGGED(ch, AFF_PETRIFY))
        send_to_char(ch, "You have been petrified!\n\r");

    if (AFF_FLAGGED(ch, AFF_SECOND_SIGHT))
        send_to_char(ch, "You seem to be more perceptive in combat.\n\r");

    if (AFF_FLAGGED(ch, AFF_SENSE_LIFE))
        send_to_char(ch, "You are sensitive to the presence of hidden lifeforms.\n\r");

    if (AFF_FLAGGED(ch, AFF_THERMAL_AURA))
        send_to_char(ch, "You are protected by a warming aura.\n\r");

    if (affected_by_psionic(ch, PSIONIC_WEAKEN))
        send_to_char(ch, "You feel very weak!\n\r");

    if (affected_by_psionic(ch, PSIONIC_HASTE))
        send_to_char(ch, "You are moving very quickly!\n\r");

    if (affected_by_psionic(ch, PSIONIC_VORPAL_SPEED))
        send_to_char(ch, "You are moving at the speed of light!\r\n");

    if (affected_by_psionic(ch, PSIONIC_DUPLICATES) || ch->char_specials.saved.duplicates > 0)
        send_to_char(ch, "You are surrounded by images.\n\r");

    if (AFF_FLAGGED(ch, AFF_NOTRACK))
        send_to_char(ch, "You cannot be tracked!\n\r");

    if (AFF_FLAGGED(ch, AFF_SNEAK))
        send_to_char(ch, "You're sneaking around like a thug!\n\r");

    if (AFF_FLAGGED(ch, AFF_BLIND_IMMUNE))
        send_to_char(ch, "Your eyes have a rigid, steeled look to them.\n\r");

    if (AFF_FLAGGED(ch, AFF_LETH_IMMUNE))
        send_to_char(ch, "Your skin crawls with electricity.\n\r");

    if (affected_by_psionic(ch, PSIONIC_DEGRADE))
        send_to_char(ch, "You are more vulnerable to attack.\n\r");

    if (affected_by_psionic(ch, PSIONIC_MEGA_DEGRADE))
        send_to_char(ch, "You are MUCH more vulnerable to attack.\n\r");

    if (affected_by_psionic(ch, PSIONIC_COMBAT_MIND))
        send_to_char(ch, "Your mind is prepared for combat.\n\r");
    if (affected_by_psionic(ch, PSIONIC_BEAR_RAGE))
        send_to_char(ch, "You feel the rage of the bear.\n\r");
    if (affected_by_psionic(ch, PSIONIC_PUMA_SPEED))
        send_to_char(ch, "You feel as fast as a puma.\n\r");
    if (affected_by_psionic(ch, PSIONIC_WOLF_SENSE))
        send_to_char(ch, "You feel as agile as the wolf.\n\r");
	
	if (affected_by_psionic(ch, PSIONIC_SLOW_POISON))
		send_to_char(ch, "You are slightly protected against poison damage.\r\n");
	if (affected_by_psionic(ch, PSIONIC_NEUTRALIZE_DRUG))
		send_to_char(ch, "You are slightly protected against drug damage.\r\n");
	if (affected_by_psionic(ch, PSIONIC_HEAT_SHIELD))
		send_to_char(ch, "You are slightly protected against heat damage.\r\n");
	if (affected_by_psionic(ch, PSIONIC_COLD_SHIELD))
		send_to_char(ch, "You are slightly protected against cold damage.\r\n");
	if (affected_by_psionic(ch, PSIONIC_DISSIPATE))
		send_to_char(ch, "You are slightly protected against electrical damage.\r\n");
	if (affected_by_psionic(ch, PSIONIC_MAGNETIC_FIELD))
		send_to_char(ch, "You are slightly protected against explosive damage.\r\n");
	if (affected_by_psionic(ch, PSIONIC_PSIONIC_SHIELD))
		send_to_char(ch, "You are slightly protected against psionic damage.\r\n");
	if (affected_by_psionic(ch, PSIONIC_SPIRIT_WARD))
		send_to_char(ch, "You are slightly protected against ethereal damage.\r\n");		
	if (affected_by_psionic(ch, PSIONIC_RESURRECTION_FATIGUE))
		send_to_char(ch, "@DYou are suffering the effects of death...@n\r\n");


//  if ((pMudEvent = char_has_mud_event(ch, eD_ROLL))) 
  if (((pMudEvent = char_has_mud_event(ch, eD_ROLL))) && (GET_SKILL_LEVEL(ch, SKILL_DEFENSE_ROLL) != 0) && char_has_mud_event(ch, eD_ROLL)) 
    send_to_char(ch, "Defensive Roll - Duration: %ld seconds\r\n", (event_time(pMudEvent->pEvent) / 10));

  if ((GET_SKILL_LEVEL(ch, SKILL_DEFENSE_ROLL) != 0) && !char_has_mud_event(ch, eD_ROLL)) 
    send_to_char(ch, "Defensive Roll - Available\r\n");

  if (((pMudEvent = char_has_mud_event(ch, eD_REBOOT))) && (GET_SKILL_LEVEL(ch, SKILL_REBOOT) != 0) && char_has_mud_event(ch, eD_REBOOT)) 
    send_to_char(ch, "Reboot - Duration: %ld seconds\r\n", (event_time(pMudEvent->pEvent) / 10));

  if ((GET_SKILL_LEVEL(ch, SKILL_REBOOT) != 0) && !char_has_mud_event(ch, eD_REBOOT)) 
    send_to_char(ch, "Reboot - Avaliable\r\n");

    // Routine to show what drugs a char is affected by
    if (IS_ON_DRUGS(ch)) {

        bool firstItem = TRUE;
        send_to_char(ch, "You are on the following drugs: ");

        for (aff = ch->drugs_used; aff; aff = aff->next) {

            if (aff->next && aff->type == aff->next->type)
                continue;

            if (firstItem) {
                send_to_char(ch, "%s", drug_names[aff->type]);
                firstItem = FALSE;
            }
            else
                send_to_char(ch, ", %s", drug_names[aff->type]);
