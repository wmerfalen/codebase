...
ACMD(do_mail)
{
  ...
  if (!ch->desc || IS_NPC(ch))
    return;			/* so mobs don't get caught here */
  two_arguments(argument, arg, arg2);
  if (!*arg) {
	  send_to_char(ch, "Try typing 'help mail' to find out more information about the CA mail system.\r\n");
	  return;
  }
  else {
	  if (!strcmp(arg, "write")) {
		  if (!*arg2) {
			  send_to_char(ch, "Who would you like to send mail to?\r\n");
			  return;
		  }
		  if (*arg2) {
		  	postmaster_send_mail(ch, ch, cmd, arg2);
			return;
		  }
	  }
	  else if (!strcmp(arg, "check")) {
		  postmaster_check_mail(ch, ch, cmd, argument);
		  return;
	  }
	  else if (!strcmp(arg, "receive")) {
		postmaster_receive_mail(ch, ch, cmd, argument);
	    return;
	  }
	  else
		  return;
			  ...
			  int cmd, char *arg)
{
  ...
  if (GET_LEVEL(ch) < MIN_MAIL_LEVEL) {
    snprintf(buf, sizeof(buf), "The mail-bot tells you, 'Sorry, you have to be level %d to send mail!'", MIN_MAIL_LEVEL);
  ...
  if (!*buf) {			/* you'll get no argument from me! */
    act("The mail-bot tells you, 'You need to specify an addressee!'",
	FALSE, mailman, 0, ch, TO_VICT);
    return;
  }
  if (GET_UNITS(ch) < STAMP_PRICE && GET_LEVEL(ch) < LVL_IMMORT) {
    snprintf(buf, sizeof(buf), "The mail-bot tells you, 'A stamp costs %d units%s.'\r\n"
	    "The mail-bot tells you, '...which I see you can't afford.'", STAMP_PRICE,
            STAMP_PRICE == 1 ? "" : "s");
    act(buf, FALSE, mailman, 0, ch, TO_VICT);
    return;
  }
  if ((recipient = get_id_by_name(buf)) < 0 || !mail_recip_ok(buf)) {
    act("The mail-bot tells you, 'No one by that name is registered here!'",
	FALSE, mailman, 0, ch, TO_VICT);
    return;
  }
  act("$n starts to write some mail.", TRUE, ch, 0, 0, TO_ROOM);
  snprintf(buf, sizeof(buf), "The mail-bot tells you, 'I'll take %d units for the stamp.'\r\n"
       "The mail-bot tells you, 'Write your message. (/s saves /h for help).'",
	  STAMP_PRICE);

  act(buf, FALSE, mailman, 0, ch, TO_VICT);

  if (GET_LEVEL(ch) < LVL_IMMORT)
    GET_UNITS(ch) -= STAMP_PRICE;
			  ...
			  int cmd, char *arg)
{
  if (has_mail(GET_IDNUM(ch)))
    act("The mail-bot tells you, 'You have mail waiting.'", FALSE, mailman, 0, ch, TO_VICT);
  else
    act("The mail-bot tells you, 'Sorry, you don't have any mail waiting.'", FALSE, mailman, 0, ch, TO_VICT);
}
			  ...
			  int cmd, char *arg)
{
  ...
  if (!has_mail(GET_IDNUM(ch))) {
    snprintf(buf, sizeof(buf), "The mail-bot tells you, 'Sorry, you don't have any mail waiting.'");
    act(buf, FALSE, mailman, 0, ch, TO_VICT);
    return;
  }
  while (has_mail(GET_IDNUM(ch))) {
    ...
    act("The mail-bot gives you a piece of mail.", FALSE, mailman, 0, ch, TO_VICT);
    act("The mail-bot gives $n a piece of mail.", FALSE, ch, 0, mailman, TO_ROOM);
  }
}
