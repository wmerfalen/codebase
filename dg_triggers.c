...
#include "oasis.h"
#include "constants.h"
#include "psionics.h"  /* for skill_name() */
...
int quest_mtrigger(struct char_data *actor, int dir)
{
    struct trig_data *t;
    struct char_data *ch;
    char buf[MAX_INPUT_LENGTH];
    ...
    for (ch = world[IN_ROOM(actor)].people; ch; ch = ch->next_in_room) {
        ...
        if (!SCRIPT_CHECK(ch, MTRIG_QUEST))
            continue;

        for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {

            if (((IS_SET(GET_TRIG_TYPE(t), MTRIG_QUEST) && CAN_SEE(ch, actor)))) {
...
int command_mtrigger(char_data *actor, char *cmd, char *argument)
{
  ...
  for (ch = world[IN_ROOM(actor)].people; ch; ch = ch_next) {
    ...
    if (SCRIPT_CHECK(ch, MTRIG_COMMAND) && ((actor!=ch) || CONFIG_SCRIPT_PLAYERS)) {
      for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
...
int cast_mtrigger(char_data *actor, char_data *ch, int psionicnum)
{
  trig_data *t;
  char buf[MAX_INPUT_LENGTH];
  ...
  for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
    if (TRIGGER_CHECK(t, MTRIG_CAST) &&
        (rand_number(1, 100) <= GET_TRIG_NARG(t))) {
            ADD_UID_VAR(buf, t, actor, "actor", 0);
            sprintf(buf, "%d", psionicnum);
            add_var(&GET_TRIG_VARS(t), "psionic", buf, 0);
            add_var(&GET_TRIG_VARS(t), "psionicname", psionic_name(psionicnum), 0);
      return script_driver(&ch, t, MOB_TRIGGER, TRIG_NEW);
    }
  }
...
int enter_otrigger(obj_data *obj, char_data *actor)
{
  trig_data *t;
  char buf[MAX_INPUT_LENGTH];
  int ret_val;
  ...
  for (t = TRIGGERS(SCRIPT(obj)); t; t = t->next) {
    if (TRIGGER_CHECK(t, OTRIG_ENTER) && (rand_number(1, 100) <= GET_TRIG_NARG(t))) {
      ADD_UID_VAR(buf, t, actor, "actor", 0);
      ret_val = script_driver(&obj, t, OBJ_TRIGGER, TRIG_NEW);
      /* Don't allow a drop to take place, if the object is purged. */
      if (!obj)
...
int throw_otrigger(obj_data *obj, char_data *actor)
{
  trig_data *t;
  char buf[MAX_INPUT_LENGTH];
  int ret_val;

  if (!SCRIPT_CHECK(obj, OTRIG_THROW))
    return 1;

  for (t = TRIGGERS(SCRIPT(obj)); t; t = t->next) {
    if (TRIGGER_CHECK(t, OTRIG_THROW) && (rand_number(1, 100) <= GET_TRIG_NARG(t))) {
      ADD_UID_VAR(buf, t, actor, "actor", 0);
      ret_val = script_driver(&obj, t, OBJ_TRIGGER, TRIG_NEW);
      /* Don't allow a drop to take place, if the object is purged. */
      if (!obj)
...
int cast_otrigger(char_data *actor, obj_data *obj, int psionicnum)
{
  trig_data *t;
  char buf[MAX_INPUT_LENGTH];
...
int cast_wtrigger(char_data *actor, char_data *vict, obj_data *obj, int psionicnum)
{
  room_data *room;
  trig_data *t;
  char buf[MAX_INPUT_LENGTH];
